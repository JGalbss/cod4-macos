...
int32_t __cdecl FX_EffectToHandle(FxSystem *system, FxEffect *effect)
{
    ...
    return ((char *)effect - (char *)system->effects) / FxEffect::HANDLE_SCALE;
}
...
void __cdecl FX_FreePool_Generic_FxTrail_(FxTrail *item, volatile LONG *firstFreeIndex, FxPool<FxTrail> *pool)
{
    ...
    if (*firstFreeIndex != -1 && static_cast<unsigned int>(*firstFreeIndex) >= 0x80u)
        MyAssertHandler(
            ".\\EffectsCore\\fx_system.cpp",
            243,
            0,
    ...
    // KISAKHACK-AUDIT: writes a DWORD covering item->nextTrailHandle (uint16) + adjacent
    // bytes. Item is FxTrail/FxElem with the field at the same offset; assignment to that
    // half via memcpy keeps the byte semantics without strict-aliasing.
    {
        unsigned int v = static_cast<unsigned int>(*firstFreeIndex);
        memcpy(&item->nextTrailHandle, &v, sizeof(unsigned int));
    }
...
void __cdecl FX_RunGarbageCollection_FreeTrails(FxSystem *system, FxEffect *effect)
{
    ...
    while (effect->firstTrailHandle != 0xFFFF)
    {
        ...
        trail->nextFree = 0;
        // KISAKHACK-AUDIT: zero a uint32 spanning lastElemHandle (uint16) + adjacent byte.
        {
            unsigned int v = 0;
            memcpy(&trail->item.lastElemHandle, &v, sizeof(unsigned int));
        }
    ...
    volatile LONG* activeCount)
{
    ...
    {
        item = &pool[itemIndex];
        if (item->nextFree != -1 && static_cast<unsigned int>(item->nextFree) >= 0x80u)
            MyAssertHandler(
                ".\\EffectsCore\\fx_system.cpp",
                200,
                0,
    ...
    volatile LONG * activeCount)
{
    ...
    {
        item = &pool[itemIndex];
        if (item->nextFree != -1 && static_cast<unsigned int>(item->nextFree) >= 0x800u)
            MyAssertHandler(
                ".\\EffectsCore\\fx_system.cpp",
                200,
                0,
...
{
    volatile uint32_t freedIndex; // [esp+4h] [ebp-4h]
    FxPool<FxElem>* freedSlot;

    freedSlot = reinterpret_cast<FxPool<FxElem>*>(item);
    freedIndex = static_cast<uint32_t>(freedSlot - pool);
    if (freedIndex >= 0x800)
        MyAssertHandler(
            ".\\EffectsCore\\fx_system.cpp",
            228,
            ...
            "freedIndex >= 0 && freedIndex < ITEM_TYPE::POOL_SIZE");
    Sys_EnterCriticalSection(CRITSECT_FX_ALLOC);
    if (*firstFreeIndex != -1 && static_cast<unsigned int>(*firstFreeIndex) >= 0x800u)
        MyAssertHandler(
            ".\\EffectsCore\\fx_system.cpp",
            243,
            0,
            "%s",
            "*firstFreeIndex == -1 || (*firstFreeIndex >= 0 && *firstFreeIndex < ITEM_TYPE::POOL_SIZE)");
    freedSlot->nextFree = *firstFreeIndex;
    *firstFreeIndex = freedIndex;
    Sys_LeaveCriticalSection(CRITSECT_FX_ALLOC);
}
    ...
    uint32_t markEntnum)
{
    ...
    if (std::getenv("KISAK_FX_TRACE"))
    {
        ...
        const int elemCount = remoteDef->elemDefCountLooping + remoteDef->elemDefCountOneShot;
        for (int elemIndex = 0; elemIndex < elemCount && traceCount < 256; ++elemIndex)
        {
            const FxElemDef &elemDef = remoteDef->elemDefs[elemIndex];
            if (elemDef.elemType != 5)
                continue;
            const XModel *model = nullptr;
            if (elemDef.visualCount == 1)
                model = elemDef.visuals.instance.model;
            else if (elemDef.visualCount > 1 && elemDef.visuals.array)
                model = elemDef.visuals.array[0].model;
            Com_Printf(8,
                "[fx] model effect='%s' elem=%d flags=%08x visuals=%u model='%s' physics=%d\n",
                remoteDef->name ? remoteDef->name : "(unnamed)", elemIndex, elemDef.flags,
                static_cast<unsigned int>(elemDef.visualCount),
                model && model->name ? model->name : "(null)",
                (elemDef.flags & 0x8000000) != 0);
            ++traceCount;
        }
    }
    ...
    if (!isSpotLightEffect || FX_CanAllocSpotLightEffect(system))
    {
        ...
        InterlockedExchange(
            &remoteEffect->status,
            static_cast<LONG>(remoteDef->msecLoopingLife != 0 ? 0x20010002 : 0x20000001));
    ...
    FxElem* elem)
{
    ...
    if (elem->physObjId)
    {
        Phys_ObjSetCollisionFromXModel(visuals.model, PHYS_WORLD_FX, Phys_ObjFromId(elem->physObjId));
        Phys_ObjSetAngularVelocity(Phys_ObjFromId(elem->physObjId), angularVelocity);
    }
...
void __cdecl FX_FreeElem(FxSystem* system, uint16_t elemHandle, FxEffect* effect, uint32_t elemClass)
{
    ...
    {
        Sys_EnterCriticalSection(CRITSECT_PHYSICS);
        Phys_ObjDestroy(PHYS_WORLD_FX, Phys_ObjFromId(elem->item.physObjId));
        Sys_LeaveCriticalSection(CRITSECT_PHYSICS);
    }
    elem->nextFree = 0;
    { unsigned int kisak_z = 0; memcpy(&elem->item.nextElemHandleInEffect, &kisak_z, sizeof(unsigned int)); }
...
void __cdecl FX_FreeTrailElem(FxSystem *system, uint16_t trailElemHandle, FxEffect *effect, FxTrail *trail)
{
    ...
    { unsigned int kisak_z = 0; memcpy(&trailElem->item.nextTrailElemHandle, &kisak_z, sizeof(unsigned int)); }
    { unsigned int kisak_z = 0; memcpy(&trailElem->item.basis[0][0], &kisak_z, sizeof(unsigned int)); }
    { unsigned int kisak_z = 0; memcpy(&trailElem->item.basis[1][1], &kisak_z, sizeof(unsigned int)); }
...
void __cdecl FX_FreeSpotLightElem(FxSystem *system, uint16_t elemHandle, FxEffect *effect)
{
    ...
    v3->nextFree = 0;
    { unsigned int kisak_z = 0; memcpy(&v3->item.nextElemHandleInEffect, &kisak_z, sizeof(unsigned int)); }
    v3->item.msecBegin = 0;
