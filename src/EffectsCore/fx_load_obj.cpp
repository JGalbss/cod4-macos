...
{
    return Hunk_AllocAlign(size, alignof(FxEffectDef), "FX_Alloc", 8);
}
...
const FxEffectDef *__cdecl FX_LoadFailed(const char *name)
{
    ...
    const FxEffectDef *const source = fx_load.defaultEffect;
    const int elemCount = source->elemDefCountLooping
        + source->elemDefCountOneShot + source->elemDefCountEmission;
    if (elemCount < 0 || (elemCount && !source->elemDefs))
        Com_Error(ERR_FATAL, "Invalid default effect while cloning '%s'", name);

    const auto arrayBytes = [name](size_t elementSize, size_t count)
    {
        if (count && elementSize > SIZE_MAX / count)
            Com_Error(ERR_FATAL, "FX fallback allocation overflow for '%s'", name);
        return elementSize * count;
    };
    const auto appendBlock = [name](size_t &offset, size_t size, size_t alignment)
    {
        const size_t mask = alignment - 1;
        if (offset > SIZE_MAX - mask)
            Com_Error(ERR_FATAL, "FX fallback alignment overflow for '%s'", name);
    ...
    size_t totalBytesNeeded = sizeof(FxEffectDef);
    appendBlock(totalBytesNeeded,
                arrayBytes(sizeof(FxElemDef), static_cast<size_t>(elemCount)),
                alignof(FxElemDef));
    for (int elemIndex = 0; elemIndex < elemCount; ++elemIndex)
    {
        const FxElemDef &elem = source->elemDefs[elemIndex];
        if (elem.velSamples)
        {
            appendBlock(totalBytesNeeded,
                        arrayBytes(sizeof(FxElemVelStateSample),
                                   static_cast<size_t>(elem.velIntervalCount) + 1),
                        alignof(FxElemVelStateSample));
        }
        if (elem.visSamples)
        {
            appendBlock(totalBytesNeeded,
                        arrayBytes(sizeof(FxElemVisStateSample),
                                   static_cast<size_t>(elem.visStateIntervalCount) + 1),
                        alignof(FxElemVisStateSample));
        }
        if (elem.elemType == 9 && elem.visuals.markArray)
        {
            appendBlock(totalBytesNeeded,
                        arrayBytes(sizeof(FxElemMarkVisuals), elem.visualCount),
                        alignof(FxElemMarkVisuals));
        }
        else if (elem.visualCount > 1 && elem.visuals.array)
        {
            appendBlock(totalBytesNeeded,
                        arrayBytes(sizeof(FxElemVisuals), elem.visualCount),
                        alignof(FxElemVisuals));
        }
        if (elem.trailDef)
        {
            if (elem.trailDef->vertCount < 0 || elem.trailDef->indCount < 0)
                Com_Error(ERR_FATAL, "Invalid FX trail while cloning '%s'", name);
            appendBlock(totalBytesNeeded, sizeof(FxTrailDef), alignof(FxTrailDef));
            if (elem.trailDef->verts)
            {
                appendBlock(totalBytesNeeded,
                            arrayBytes(sizeof(FxTrailVertex), elem.trailDef->vertCount),
                            alignof(FxTrailVertex));
            }
            if (elem.trailDef->inds)
            {
                appendBlock(totalBytesNeeded,
                            arrayBytes(sizeof(uint16_t), elem.trailDef->indCount),
                            alignof(uint16_t));
            }
        }
    }
    const size_t nameBytes = strlen(name) + 1;
    appendBlock(totalBytesNeeded, nameBytes, alignof(char));
    if (totalBytesNeeded > static_cast<size_t>(INT_MAX)
        || totalBytesNeeded > static_cast<size_t>(UINT_MAX))
    {
        Com_Error(ERR_FATAL, "FX fallback is too large for '%s'", name);
    }

    auto *const effect = static_cast<FxEffectDef *>(
        FX_AllocMem(static_cast<unsigned int>(totalBytesNeeded)));
    memset(effect, 0, totalBytesNeeded);
    *effect = *source;
    byte *const destinationBegin = reinterpret_cast<byte *>(effect);
    size_t destinationOffset = sizeof(FxEffectDef);
    const auto reserveBlock = [destinationBegin](size_t &offset, size_t size, size_t alignment)
    {
        const size_t mask = alignment - 1;
        offset = (offset + mask) & ~mask;
        byte *const block = destinationBegin + offset;
        offset += size;
        return block;
    };

    const size_t elemBytes = arrayBytes(sizeof(FxElemDef), static_cast<size_t>(elemCount));
    auto *const elemDefs = reinterpret_cast<FxElemDef *>(
        reserveBlock(destinationOffset, elemBytes, alignof(FxElemDef)));
    memcpy(elemDefs, source->elemDefs, elemBytes);
    effect->elemDefs = elemDefs;

    for (int elemIndex = 0; elemIndex < elemCount; ++elemIndex)
    {
        const FxElemDef &sourceElem = source->elemDefs[elemIndex];
        FxElemDef &elem = elemDefs[elemIndex];
        if (sourceElem.velSamples)
        {
            const size_t bytes = arrayBytes(sizeof(FxElemVelStateSample),
                static_cast<size_t>(sourceElem.velIntervalCount) + 1);
            elem.velSamples = reinterpret_cast<FxElemVelStateSample *>(
                reserveBlock(destinationOffset, bytes, alignof(FxElemVelStateSample)));
            memcpy(elem.velSamples, sourceElem.velSamples, bytes);
        }
        if (sourceElem.visSamples)
        {
            const size_t bytes = arrayBytes(sizeof(FxElemVisStateSample),
                static_cast<size_t>(sourceElem.visStateIntervalCount) + 1);
            elem.visSamples = reinterpret_cast<FxElemVisStateSample *>(
                reserveBlock(destinationOffset, bytes, alignof(FxElemVisStateSample)));
            memcpy(elem.visSamples, sourceElem.visSamples, bytes);
        }
        if (sourceElem.elemType == 9 && sourceElem.visuals.markArray)
        {
            const size_t bytes = arrayBytes(sizeof(FxElemMarkVisuals), sourceElem.visualCount);
            elem.visuals.markArray = reinterpret_cast<FxElemMarkVisuals *>(
                reserveBlock(destinationOffset, bytes, alignof(FxElemMarkVisuals)));
            memcpy(elem.visuals.markArray, sourceElem.visuals.markArray, bytes);
        }
        else if (sourceElem.visualCount > 1 && sourceElem.visuals.array)
        {
            const size_t bytes = arrayBytes(sizeof(FxElemVisuals), sourceElem.visualCount);
            elem.visuals.array = reinterpret_cast<FxElemVisuals *>(
                reserveBlock(destinationOffset, bytes, alignof(FxElemVisuals)));
            memcpy(elem.visuals.array, sourceElem.visuals.array, bytes);
        }
        if (sourceElem.trailDef)
        {
            elem.trailDef = reinterpret_cast<FxTrailDef *>(
                reserveBlock(destinationOffset, sizeof(FxTrailDef), alignof(FxTrailDef)));
            *elem.trailDef = *sourceElem.trailDef;
            if (sourceElem.trailDef->verts)
            {
                const size_t bytes = arrayBytes(sizeof(FxTrailVertex), sourceElem.trailDef->vertCount);
                elem.trailDef->verts = reinterpret_cast<FxTrailVertex *>(
                    reserveBlock(destinationOffset, bytes, alignof(FxTrailVertex)));
                memcpy(elem.trailDef->verts, sourceElem.trailDef->verts, bytes);
            }
            if (sourceElem.trailDef->inds)
            {
                const size_t bytes = arrayBytes(sizeof(uint16_t), sourceElem.trailDef->indCount);
                elem.trailDef->inds = reinterpret_cast<uint16_t *>(
                    reserveBlock(destinationOffset, bytes, alignof(uint16_t)));
                memcpy(elem.trailDef->inds, sourceElem.trailDef->inds, bytes);
            }
        }
    }

    effect->name = reinterpret_cast<const char *>(
        reserveBlock(destinationOffset, nameBytes, alignof(char)));
    memcpy(const_cast<char *>(effect->name), name, nameBytes);
    effect->totalSize = static_cast<int>(totalBytesNeeded);
    if (destinationOffset != totalBytesNeeded)
        Com_Error(ERR_FATAL, "FX fallback clone size mismatch for '%s'", name);
    return effect;
}
...
void __cdecl FX_RegisterDefaultEffect()
{
    ...
    if (std::getenv("KISAK_FX_FALLBACK_SELFTEST"))
    {
        static bool tested = false;
        if (!tested)
        {
            tested = true;
            const char *const testName = "diagnostic/native_missing_fx";
            const FxEffectDef *const fallback = FX_LoadFailed(testName);
            bool valid = fallback && fallback->name && fallback->totalSize > 0
                && !I_strcmp(fallback->name, testName);
            int elemCount = 0;
            if (valid)
            {
                const uintptr_t begin = reinterpret_cast<uintptr_t>(fallback);
                const size_t totalSize = static_cast<size_t>(fallback->totalSize);
                const auto contains = [begin, totalSize](const void *pointer, size_t size)
                {
                    const uintptr_t address = reinterpret_cast<uintptr_t>(pointer);
                    return address >= begin && address - begin <= totalSize
                        && size <= totalSize - (address - begin);
                };

                elemCount = fallback->elemDefCountLooping
                    + fallback->elemDefCountOneShot + fallback->elemDefCountEmission;
                valid = elemCount >= 0
                    && contains(fallback->elemDefs,
                        sizeof(FxElemDef) * static_cast<size_t>(elemCount));
                for (int elemIndex = 0; valid && elemIndex < elemCount; ++elemIndex)
                {
                    const FxElemDef &elem = fallback->elemDefs[elemIndex];
                    valid = contains(elem.velSamples,
                        sizeof(FxElemVelStateSample)
                            * (static_cast<size_t>(elem.velIntervalCount) + 1));
                    if (valid && elem.visSamples)
                    {
                        valid = contains(elem.visSamples,
            ...
            if (!valid)
                Com_Error(ERR_FATAL, "Native FX fallback relocation self-test failed");
            Com_Printf(8, "[fx] native fallback self-test passed: '%s', %d elems, %d bytes\n",
                       fallback->name, elemCount, fallback->totalSize);
        }
    }
}
