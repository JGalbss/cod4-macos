...
// The original fetch-add lock assumes that no more than two threads contend
// for an effect at once.  The native renderer can have several FX workers hit
// the same freshly-published effect; enough simultaneous fetch-add attempts
// overflow the two lock bits and let a worker into a partially-built element
// list.  Acquire the bit with CAS so contention never mutates the lock state.
inline void FX_LockEffect(FxEffect *effect)
{
    volatile LONG *status = &effect->status;
    for (;;)
    {
        const LONG current = InterlockedCompareExchange(
            status, static_cast<LONG>(0), static_cast<LONG>(0));
        if ((current & FX_STATUS_IS_LOCKED_MASK) != 0)
            continue;
        if (InterlockedCompareExchange(
                status,
                static_cast<LONG>(current | FX_STATUS_IS_LOCKED),
                current) == current)
        {
            return;
        }
