        ...
        for (elemHandle = elemHandleStart; elemHandle != elemHandleStop; elemHandle = elemHandleNext)
        {
            if (elemHandle == 0xFFFF)
            {
                    ...
                    elemHandle = elem->item.nextElemHandleInEffect)
                {
                    ...
                    randomIndex = (elem->item.msecBegin + effect->randomSeed + 296 * (uint32_t)elem->item.sequence)
                        % 0x1DF;
                    const FxIntRange &lifeSpanRange =
                        effect->def->elemDefs[static_cast<uint8_t>(elem->item.defIndex)].lifeSpanMsec;
                    lifeSpan = lifeSpanRange.base
                        + (((lifeSpanRange.amplitude + 1) * LOWORD(fx_randomTable[randomIndex + 17])) >> 16);
                    Com_Printf(
                        0,
...
void __cdecl FX_Update(FxSystem* system, int32_t localClientNum, bool nonBoltedEffectsOnly)
{
    ...
    FX_BeginIteratingOverEffects_Cooperative(system);
    // Pair this acquire with FX_SpawnEffect's ordered firstNewEffect publish.
    // A plain volatile load can expose a reserved effect before its locked
    // status and initial element lists are visible on Apple Silicon.
    firstNewEffect = InterlockedCompareExchange(
        &system->firstNewEffect, static_cast<LONG>(0), static_cast<LONG>(0));
    for (activeIndex = system->firstActiveEffect; activeIndex != firstNewEffect; ++activeIndex)
    {
...
void __cdecl FX_RewindTo(int32_t localClientNum, int32_t time)
{
    ...
    if (time < system->msecNow)
    {
        ...
        firstNewEffect = InterlockedCompareExchange(
            &system->firstNewEffect, static_cast<LONG>(0), static_cast<LONG>(0));
        for (i = system->firstActiveEffect; i != firstNewEffect; ++i)
        {
        ...
        for (bitNum = 0; bitNum < v12; ++bitNum)
        {
            if (Com_BitCheckAssert(dst, bitNum, 128))
            {
                ...
                // Rewind runs on the cgame thread while an FX worker from the
                // previous render frame can still be finishing.  Killing was
                // locked above, but rebuilding the same effect used to be
                // unlocked, allowing the worker to traverse half-restarted
                // element lists during repeated killcams.
                FX_LockEffect(effectb);
                ...
                if (effectb->def->msecLoopingLife)
                {
                    ...
                    InterlockedExchangeAdd(&effectb->status, static_cast<LONG>(-536870912));
                }
                else
                {
                    FX_StartNewEffect(system, effectb);
