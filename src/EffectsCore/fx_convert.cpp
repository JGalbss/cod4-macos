...
namespace
{
constexpr size_t kFxAllocationAlignment = alignof(FxEffectDef);

int32_t FX_AlignAllocationSize(int32_t size)
{
    const size_t mask = kFxAllocationAlignment - 1;
    return static_cast<int32_t>((static_cast<size_t>(size) + mask) & ~mask);
}

void FX_AlignAllocationCursor(uint8_t **memPool)
{
    const uintptr_t mask = kFxAllocationAlignment - 1;
    *memPool = reinterpret_cast<uint8_t *>(
        (reinterpret_cast<uintptr_t>(*memPool) + mask) & ~mask);
}
}
    ...
    int32_t visualCount)
{
    ...
    bytesNeeded = sizeof(FxElemVelStateSample) * velStateSampleCount
        + sizeof(FxElemVisStateSample) * visStateSampleCount;
    if (elemType == 9)
    {
    ...
    return FX_AlignAllocationSize(bytesNeeded);
}

int32_t __cdecl FX_AdditionalBytesNeededForGeomTrail(const FxEditorElemDef *elemDef)
{
    if (elemDef->elemType == 3)
        return FX_AlignAllocationSize(sizeof(FxTrailDef)
            + (sizeof(FxTrailVertex) + sizeof(uint16_t)) * elemDef->trailDef.indCount);
    else
        return 0;
}
...
void __cdecl FX_ReserveElemDefMemory(FxElemDef *elemDef, uint8_t **memPool)
{
    ...
    *memPool += sizeof(FxElemVelStateSample) * (elemDef->velIntervalCount + 1);
    ...
    {
        elemDef->visuals.array = (FxElemVisuals *)*memPool;
        *memPool += sizeof(FxElemVisuals) * elemDef->visualCount;
    }
    FX_AlignAllocationCursor(memPool);
}
...
{
    memcpy(markVisualsArray, edElemDef->markVisuals,
...
{
    memcpy(visualsArray, edElemDef->visuals,
    ...
    uint8_t **mempool)
{
    ...
    vertBytes = sizeof(FxTrailVertex) * indCount;
    outVertPtrBegin = (FxTrailVertex *)*mempool;
    *mempool += sizeof(FxTrailVertex) * indCount;
    ...
    indBytes = sizeof(uint16_t) * indCount;
    emittedIndPtrBegin = (uint16_t *)*mempool;
    *mempool += sizeof(uint16_t) * indCount;
    ...
    if (sizeof(FxTrailVertex) * outTrailDef->vertCount > static_cast<size_t>(vertBytes))
        MyAssertHandler(
            ".\\EffectsCore\\fx_convert.cpp",
            949,
            0,
    ...
    if (sizeof(uint16_t) * outTrailDef->indCount > static_cast<size_t>(indBytes))
        MyAssertHandler(
            ".\\EffectsCore\\fx_convert.cpp",
            952,
            0,
    ...
    for (elemIndex = 0; elemIndex < editorEffect->elemCount; ++elemIndex)
    {
        ...
        if (emission && emission->elemDefCountOneShot && FX_FindEmission(emission, editorEffect) == elemIndex)
        {
            ...
            for (elemIndexEmit = emission->elemDefCountLooping; elemIndexEmit != elemIndexStop; ++elemIndexEmit)
            {
                ...
                FX_ReserveElemDefMemory(elemDef, memPool);
                memcpy(elemDef->velSamples, elemDefEmit->velSamples,
                       sizeof(FxElemVelStateSample) * (elemDef->velIntervalCount + 1));
                if (elemDef->visSamples)
                {
                    memcpy(elemDef->visSamples, elemDefEmit->visSamples,
                           sizeof(FxElemVisStateSample) * (elemDef->visStateIntervalCount + 1));
                }
                if (elemDef->elemType == 9)
                {
                    memcpy(elemDef->visuals.markArray, elemDefEmit->visuals.markArray,
                           sizeof(FxElemMarkVisuals) * elemDef->visualCount);
                }
                else if (elemDef->visualCount > 1u)
                {
                    memcpy(elemDef->visuals.array, elemDefEmit->visuals.array,
...
const FxEffectDef *__cdecl FX_Convert(const FxEditorEffectDef *editorEffect, void *(*Alloc)(uint32_t))
{
    ...
    totalBytesNeeded = sizeof(FxEffectDef) + sizeof(FxElemDef) * editorEffect->elemCount;
