    ...
    FxInsertSortElem *sortElem)
{
    ...
    {
        const int elemDefCount = def->elemDefCountLooping
            + def->elemDefCountOneShot
            + def->elemDefCountEmission;
        Com_PrintError(
            21,
            "FX sprite-list corruption: effect='%s' effect=%p def=%p elem=%p "
            "defIndex=%u defCount=%d elemType=%u first=%u sorted=%u msec=%d/%d\n",
            def->name ? def->name : "(unnamed)",
            static_cast<const void *>(effect),
            static_cast<const void *>(def),
            static_cast<const void *>(elem),
            static_cast<unsigned int>(elem->defIndex),
            elemDefCount,
            static_cast<unsigned int>(elemDef->elemType),
            static_cast<unsigned int>(effect->firstElemHandle[0]),
            static_cast<unsigned int>(effect->firstSortedElemHandle),
            effect->msecBegin,
