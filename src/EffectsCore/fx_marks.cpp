    ...
    *(uint32_t *)newMark->nativeColor = *(uint32_t *)nativeColor;
    ...
    if (newMark->pointCount != pointCount)
        MyAssertHandler(
            ...
            newMark->pointCount,
            pointCount);
    if (std::getenv("KISAK_MARK_TRACE"))
    {
        Com_Printf(8,
            "[marks] material='%s' modelType=%u model=%u tris=%d points=%d radius=%.3f "
            "origin=(%.3f %.3f %.3f) handles=(%d,%d)\n",
            material && material->info.name ? material->info.name : "(null)",
            static_cast<unsigned int>(newMark->context.modelTypeAndSurf & 0xC0),
            static_cast<unsigned int>(newMark->context.modelIndex),
            triCount, pointCount, radius, origin[0], origin[1], origin[2], tris, points);
    }
...
{
    if (handle >= sizeof(marksSystem->triGroups))
        MyAssertHandler(
            "c:\\trees\\cod3\\src\\effectscore\\fx_marks.h",
            193,
            0,
            ...
            handle,
            static_cast<int>(sizeof(marksSystem->triGroups)));
    if (!marksSystem)
...
{
    if (handle >= sizeof(marksSystem->pointGroups))
        MyAssertHandler(
            "c:\\trees\\cod3\\src\\effectscore\\fx_marks.h",
            218,
            0,
            ...
            handle,
            static_cast<int>(sizeof(marksSystem->pointGroups)));
    if (!marksSystem)
    ...
    int32_t pointCount)
{
    ...
    do
    {
        ...
        do
        {
            memcpy(&dstGroup->pointGroup.points[copyIndex],
                   &srcPoints[copyIndex], sizeof(FxMarkPoint));
            ++copyIndex;
        } while (copyIndex < v4);
...
{
    FxSystem *System; // eax
    uint32_t indexCount; // [esp+64h] [ebp-8h] BYREF
