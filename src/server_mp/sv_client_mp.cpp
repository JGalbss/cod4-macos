...
// Extended stats use the same packed 8192-byte layout uploaded by the client.
// The original 32-bit disassembly expressed this as cross-field pointer math
// from voicePackets, which does not survive the wider fields in client_t on
// 64-bit targets.
static unsigned int *Kisak_StatExtPtr(client_t *cl, int index)
{
    return reinterpret_cast<unsigned int *>(&cl->stats[4 * index - 5996]);
}
...
void __cdecl SV_AuthorizeRequest(netadr_t from, int challenge, const char *cdkeyHash)
{
    ...
    const char *integer; // [esp+Ch] [ebp-410h]
...
void __cdecl SV_GetChallenge(netadr_t from)
{
...
#ifdef KISAK_COD4X
    // CoD4x clients begin with `getchallenge 0 "<32-char guid>"`; a stock
    // IW3 server expects both a Steam ticket and numeric Steam id.  Let a
    // native LAN/listen server answer with the ordinary stock challenge.
    // The absence of the `xproto` marker makes the client deliberately stay
    // on protocol 1, so this does not pretend the stock server implements the
    // protocol-21 reliable-message extensions.  Keep the shortcut LAN-only:
    // Internet hosts must retain real ticket validation.
    if (clientSteamTicketBase64[0] && !clientSteamID64[0]
        && Sys_IsLANAddress(from))
    {
        I_strncpyz(svs.challenges[i].cdkeyHash, clientSteamTicketBase64,
                   sizeof(svs.challenges[i].cdkeyHash));
        challenge->pingTime = svs.time;
        NET_OutOfBandPrint(NS_SERVER, from,
                           va("challengeResponse %i", challenge->challenge));
        return;
    }
#endif
    ...
    //    if (!NET_StringToAdr(com_authServerName->current.string, &svs.authorizeAddress))
    //    {
