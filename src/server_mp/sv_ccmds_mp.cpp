...
void __cdecl SV_Map_f()
{
    ...
    if (*map)
    {
        ...
        if (IsFastFileLoad())
        {
            ...
            // Custom maps keep their fastfiles under usermaps/<map>/<map>.ff.
            // DB_TryLoadXFileInternal already searches that location, so the
            // command preflight must accept it too instead of rejecting the map
            // before the database loader gets a chance to open it.
            if (!DB_FileSize(mapname, 0)
                && (!fs_gameDirVar->current.string[0] || !DB_FileSize(mapname, 1))
                && !DB_FileSize(mapname, 2))
            {
                Com_PrintError(1, "Can't find map \"%s\".\n", mapname);
                return;
            }
