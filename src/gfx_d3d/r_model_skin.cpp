    ...
    for (unsigned int i = 0; i < skinCmd->surfCount; i++)
    {
        ...
        if (boneIndex != skinnedSurf->info.boneIndex)
        {
            ...
            for (unsigned int j = boneIndex; j < totalBones; j++)
            {
                ...
                // baseMat is local to this submodel.  The decompile formed a
                // pointer before the array and indexed it with the global DObj
                // bone number; that happens to work as address arithmetic on
                // x86, but is undefined C++ and unsafe under the ARM64 -O3
                // optimizer.
                ConvertQuatToInverseSkelMat(
                    &skinnedSurf->info.baseMat[j - static_cast<unsigned int>(boneIndex)],
                    &mat0);
        ...
        {
            iassert((reinterpret_cast<uintptr_t>(skinnedSurf->skinnedVert) & 15) == 0);
    ...
    GfxPackedVertex *vertices)
{
    ...
    iassert(vertices);
    iassert(!(reinterpret_cast<uintptr_t>(vertices) & 15));
