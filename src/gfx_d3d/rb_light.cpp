    ...
    *defaultGridEntry = 1;
    ...
    for (cornerIndex = 0; cornerIndex < 8; cornerIndex++, cornerTraceBit <<= 1)
    {
        ...
        else if (!honorSuppression)
        {
            ...
            // The original 32-bit executable cleared an array of four-byte
            // pointers here.  GfxLightGridEntry pointers are eight bytes on
            // arm64, so retaining the literal 4 leaves the upper half of the
            // earlier entries uninitialised and bullet FX later dereference
            // garbage addresses while calculating their packed lighting.
            memset(cornerEntry, 0, sizeof(*cornerEntry) * cornerIndex);
            continue;
        }
