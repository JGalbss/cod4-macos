...
{
    const auto *const context = static_cast<const BspOmniLightCallback *>(bspLightCallbackAsVoid);
    return context->surfaceVisData[surfIndex]
        && context->radiusSq >= PointToBoxDistSq(
            context->position,
...
{
    const auto *const context = static_cast<const BspSpotLightCallback *>(bspLightCallbackAsVoid);
...
{
    const auto *const context = static_cast<const BspSpotLightCallback *>(bspLightCallbackAsVoid);
    if (context->surfaceVisData[surfIndex])
        return R_BoxInPlanes(
            context->planes,
            rgp.world->dpvs.surfaces[surfIndex].bounds[0],
            rgp.world->dpvs.surfaces[surfIndex].bounds[1]);
