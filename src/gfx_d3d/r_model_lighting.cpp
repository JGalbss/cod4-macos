    ...
    GfxLightingInfo *lightingInfoOut)
{
    ...
    else
    {
        ...
        while (1)
        {
            // Each cache entry must have been free for the previous two frames
            // and still be free in this frame.  The original decompilation
            // accidentally used the current-frame mask as part of an array
            // subscript, so every allocation in a frame could select the same
            // entry (and could index outside the bitset on LP64 builds).
            const unsigned int freeBits =
            ...
            if (!_BitScanReverse(&v7, freeBits))
...
unsigned int R_GetModelLightingImageHeight()
{
    return modelLightGlob.imageHeight;
}

bool R_GetDynamicModelLightingInfo(unsigned __int16 handle, GfxLightingInfo *lightingInfoOut)
{
    if (!handle || !lightingInfoOut)
        return false;

    const unsigned int entryIndex = R_ModelLightingIndexFromHandle(handle);
    if (entryIndex < modelLightGlob.xmodel.baseIndex)
        return false;

    const unsigned int usedIndex = entryIndex - modelLightGlob.xmodel.baseIndex;
