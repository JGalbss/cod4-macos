...
namespace
{
bool R_AabbTreeChildrenAreValid(const GfxAabbTree *tree, const GfxAabbTree *children,
                                const unsigned int childCount)
{
    if (!rgp.world || !tree || !children || !childCount)
        return false;

    const uintptr_t treeAddress = reinterpret_cast<uintptr_t>(tree);
    const uintptr_t childrenAddress = reinterpret_cast<uintptr_t>(children);
    for (int cellIndex = 0; cellIndex < rgp.world->dpvsPlanes.cellCount; ++cellIndex)
    {
        const GfxCell &cell = rgp.world->cells[cellIndex];
        if (!cell.aabbTree || cell.aabbTreeCount <= 0)
            continue;
        const uintptr_t begin = reinterpret_cast<uintptr_t>(cell.aabbTree);
        const uintptr_t byteCount = static_cast<uintptr_t>(cell.aabbTreeCount)
            * sizeof(GfxAabbTree);
        const uintptr_t end = begin + byteCount;
        if (treeAddress < begin || treeAddress >= end)
            continue;

        const uintptr_t childrenBytes = static_cast<uintptr_t>(childCount)
            * sizeof(GfxAabbTree);
        const bool valid = childrenAddress > treeAddress
            && childrenAddress >= begin
            && childrenAddress <= end
            && childrenBytes <= end - childrenAddress
            && (childrenAddress - begin) % sizeof(GfxAabbTree) == 0;
        if (!valid)
        {
            ...
            if (!warned)
            {
                warned = true;
                Com_PrintError(8,
                               "R_AabbTree: invalid child range in cell %d "
                               "(tree=%td offset=%d children=%td count=%u trees=%d)\n",
                               cellIndex,
                               static_cast<ptrdiff_t>((treeAddress - begin) / sizeof(GfxAabbTree)),
                               tree->childrenOffset,
                               static_cast<ptrdiff_t>((childrenAddress - begin) / sizeof(GfxAabbTree)),
                               childCount, cell.aabbTreeCount);
            }
        }
        return valid;
    }

    static bool warnedOutsideCell = false;
    if (!warnedOutsideCell)
    {
        warnedOutsideCell = true;
        Com_PrintError(8, "R_AabbTree: tree is outside every world cell allocation\n");
    }
    return false;
}
}
    ...
    unsigned int *surfCount)
{
        ...
        && maxs[2] >= (double)tree->mins[2])
    {
        if (tree->childCount)
        {
            ...
            if (!R_AabbTreeChildrenAreValid(tree, children, tree->childCount))
                return;
...
void __cdecl R_MarkFragments_Begin(
    ...
    Material *material)
{
    [[maybe_unused]] int savedregs; // [esp+10h] [ebp+0h] BYREF
...
char __cdecl R_MarkFragments_WorldBrushes(MarkInfo *markInfo)
{
    ...
    [[maybe_unused]] int savedregs; // [esp+6B4h] [ebp+0h] BYREF
...
bool __cdecl R_MarkFragments_BrushSurface(
    ...
    bool *anyMarks)
{
    [[maybe_unused]] PackedUnitVec v7; // [esp+8h] [ebp-2D0h] BYREF
    [[maybe_unused]] float v8; // [esp+Ch] [ebp-2CCh]
    [[maybe_unused]] float v9; // [esp+10h] [ebp-2C8h]
    [[maybe_unused]] PackedUnitVec v10; // [esp+14h] [ebp-2C4h]
    [[maybe_unused]] float *v11; // [esp+1Ch] [ebp-2BCh]
    [[maybe_unused]] PackedUnitVec v12; // [esp+20h] [ebp-2B8h]
    [[maybe_unused]] PackedUnitVec v13; // [esp+24h] [ebp-2B4h] BYREF
    [[maybe_unused]] float v14; // [esp+28h] [ebp-2B0h]
    [[maybe_unused]] float v15; // [esp+2Ch] [ebp-2ACh]
    [[maybe_unused]] PackedUnitVec v16; // [esp+30h] [ebp-2A8h]
    [[maybe_unused]] PackedUnitVec v17; // [esp+38h] [ebp-2A0h]
    [[maybe_unused]] PackedUnitVec out; // [esp+3Ch] [ebp-29Ch] BYREF
    [[maybe_unused]] float v19; // [esp+40h] [ebp-298h]
    [[maybe_unused]] float v20; // [esp+44h] [ebp-294h]
    [[maybe_unused]] PackedUnitVec in; // [esp+48h] [ebp-290h]
    [[maybe_unused]] float *v22; // [esp+50h] [ebp-288h]
    [[maybe_unused]] float *v23; // [esp+54h] [ebp-284h]
    [[maybe_unused]] float *v24; // [esp+58h] [ebp-280h]
    [[maybe_unused]] float *v25; // [esp+5Ch] [ebp-27Ch]
    [[maybe_unused]] float *v26; // [esp+60h] [ebp-278h]
...
void __cdecl R_Mark_TransformClipPlanes(const float (*inClipPlanes)[4], float (*matrix)[3], float (*outClipPlanes)[4])
{
    ...
    for (planeIndex = 0; planeIndex != 6; ++planeIndex)
    {
        (*outClipPlanes)[4 * planeIndex] = Vec3Dot(&(*inClipPlanes)[4 * planeIndex], m);
        (*outClipPlanes)[4 * planeIndex + 1] = Vec3Dot(&(*inClipPlanes)[4 * planeIndex], m + 3);
        (*outClipPlanes)[4 * planeIndex + 2] = Vec3Dot(&(*inClipPlanes)[4 * planeIndex], m + 6);
        v3 = (float)(*inClipPlanes)[4 * planeIndex + 3] - Vec3Dot(&(*inClipPlanes)[4 * planeIndex], m + 9);
    ...
    GfxMarkContext* markContext)
{
    ...
    [[maybe_unused]] int savedregs; // [esp+558h] [ebp+0h] BYREF
    ...
    FxMarkPoint *points)
{
    ...
    [[maybe_unused]] PackedUnitVec v12; // [esp+60h] [ebp-80h]
    [[maybe_unused]] PackedUnitVec v13; // [esp+68h] [ebp-78h]
    float out2[3]; // [esp+6Ch] [ebp-74h] BYREF
    [[maybe_unused]] PackedUnitVec v15; // [esp+78h] [ebp-68h]
    [[maybe_unused]] PackedUnitVec v16; // [esp+80h] [ebp-60h]
    float out[3]; // [esp+84h] [ebp-5Ch] BYREF
    [[maybe_unused]] PackedUnitVec in; // [esp+90h] [ebp-50h]
    ...
    FxMarkPoint *points)
{
    ...
    [[maybe_unused]] PackedUnitVec v12; // [esp+14h] [ebp-CCh]
    [[maybe_unused]] float *v13; // [esp+1Ch] [ebp-C4h]
    [[maybe_unused]] PackedUnitVec v14; // [esp+20h] [ebp-C0h]
    float out2[3]; // [esp+24h] [ebp-BCh] BYREF
    [[maybe_unused]] PackedUnitVec v16; // [esp+30h] [ebp-B0h]
    [[maybe_unused]] PackedUnitVec v17; // [esp+38h] [ebp-A8h]
    float out[3]; // [esp+3Ch] [ebp-A4h] BYREF
    [[maybe_unused]] PackedUnitVec v19; // [esp+48h] [ebp-98h]
    float normal[3][3]; // [esp+A4h] [ebp-3Ch] BYREF
    FxModelMarkPoint *clipPoint; // [esp+C8h] [ebp-18h]
    int pointIndex; // [esp+CCh] [ebp-14h]
    int pingPong; // [esp+D0h] [ebp-10h]
...
bool __cdecl R_MarkModelCoreCallback_0_(
    ...
    const GfxPackedVertex **triVerts1)
{
    const MarkModelCoreContext *const context =
        static_cast<const MarkModelCoreContext *>(contextAsVoid);
    float *vertWeights; // [esp+2Ch] [ebp-1D8h]
