...
void __cdecl R_SumOfUsedImages(Image_MemUsage *usage)
{
    ...
    [[maybe_unused]] int v5; // [esp+18h] [ebp-2028h]
    [[maybe_unused]] int v6; // [esp+1Ch] [ebp-2024h]
    [[maybe_unused]] int v7; // [esp+20h] [ebp-2020h]
    [[maybe_unused]] int v8; // [esp+24h] [ebp-201Ch]
    [[maybe_unused]] int v9; // [esp+28h] [ebp-2018h]
...
void __cdecl Image_Release(GfxImage *image)
{
    ...
    // GfxTexture is a union: it holds the zone's GfxImageLoadDef until Image_Setup
    // replaces it with a real texture. With r_loadForRenderer off that never happens,
    // so the pointer here is pixel data and releasing it calls through a vtable that
    // does not exist. Upstream never had to think about it because the renderer
    // always allocated.
    if (image->texture.basemap && r_loadForRenderer->current.enabled)
    {
        ...
        if (static_cast<unsigned int>(v1) > static_cast<unsigned int>(D3DFMT_A8L8))
        {
            ...
            else
            {
                ...
                case D3DFMT_DXT1:
                    Com_Printf(8, "DXT1  ");
                    goto LABEL_36;
                // The remaining formats fall through to the unhandled-case report
                // below, which is the point of the switch. Named so that D3DFORMAT
                // being a real enum here does not make -Wswitch fail the build.
                default:
                    break;
                }
            }
