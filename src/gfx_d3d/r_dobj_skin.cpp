    ...
    DObjAnimMat *boneMatrix)
{
    ...
    // A surface descriptor may be the larger rigid variant. The original
    // 32-bit scratch allocation assumed fixed x86 layout and could overflow on
    // LP64 when several rigid surfaces were emitted.
    alignas(GfxModelRigidSurface) unsigned char surfsBuffer[150 * sizeof(GfxModelRigidSurface)]{};
        ...
        if (cullLod >= 0)
        {
            ...
            for (unsigned int i = 0; i < surfaceCount; i++)
            {
                    ...
                    | (partBitsCheck[0] & partbits[0]))
                {
                    ...
                    surfPos = (GfxModelSkinnedSurface *)((char *)surfPos + GFX_HIDDEN_MODEL_SURFACE_SIZE);
                    continue;
                }
                ...
                // Returns the native rigid or skinned descriptor size.
    ...
    if (numSkinnedVerts)
    {
        if (gfxBuf.skinCache)
        {
            ...
            for (unsigned int offset = 0; offset < totalSurfaceCount; ++offset)
            {
                ...
                {
                    // idk
                    surfPos2 = (GfxModelSkinnedSurface *)((char *)surfPos2 + GFX_HIDDEN_MODEL_SURFACE_SIZE);
                }
                else
                {
                    ++surfPos2;
        ...
        else
        {
            ...
            for (unsigned int offset = 0; offset < totalSurfaceCount; ++offset)
            {
                ...
                {
                    surfPos2->skinnedVert = reinterpret_cast<GfxPackedVertex *>(
                        &frontEndDataOut->tempSkinBuf[sizeof(GfxPackedVertex)
                            * surfPos2->skinnedCachedOffset + firstSurf]);
