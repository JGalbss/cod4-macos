...
namespace
{
float R_DpvsBoundsMaxDistance(const float *mins, const float *maxs, const DpvsPlane *plane)
{
    float distance = plane->coeffs[3];
    for (int axis = 0; axis < 3; ++axis)
        distance += (plane->coeffs[axis] > 0.0f ? maxs[axis] : mins[axis]) * plane->coeffs[axis];
    return distance;
}

float R_DpvsBoundsMinDistance(const float *mins, const float *maxs, const DpvsPlane *plane)
{
    float distance = plane->coeffs[3];
    for (int axis = 0; axis < 3; ++axis)
        distance += (plane->coeffs[axis] > 0.0f ? mins[axis] : maxs[axis]) * plane->coeffs[axis];
    return distance;
}
}
...
void __cdecl R_AddAabbTreeSurfacesInFrustum_r(const GfxAabbTree *tree, const DpvsClipPlaneSet *clipSet)
{
    ...
    for (planeIndex = 0; planeIndex < planeCount; ++planeIndex)
    {
        ...
        if (R_DpvsBoundsMaxDistance(tree->mins, tree->maxs, plane) <= 0.0f)
            return;
        if (R_DpvsBoundsMinDistance(tree->mins, tree->maxs, plane) < 0.0f)
                    ...
                    for (i = 0; i < v16; ++i)
                    {
                        ...
                        if (!g_smodelVisData[v7])
                        {
                            for (j = 0; j < clipSetChild.count; ++j)
                            {
                                ...
                                if (R_DpvsBoundsMaxDistance(v11->mins, v11->maxs, v9) <= 0.0f)
                                {
                                    v8 = 1;
                                    goto LABEL_42;
                                }
                    ...
                    for (k = 0; k < surfaceCountNoDecal; ++k)
                    {
                        ...
                        if (!g_surfaceVisData[v2])
                        {
                            ...
                            for (m = 0; m < clipSetChild.count; ++m)
                            {
                                ...
                                if (R_DpvsBoundsMaxDistance(v6->bounds[0], v6->bounds[1], v4) <= 0.0f)
                                {
                                    v3 = 1;
                                    goto LABEL_59;
                                }
...
void __cdecl R_AddCullGroupSurfacesInFrustum(int cullGroupIndex, const DpvsPlane *planes, int planeCount)
{
    ...
    {
        if (R_DpvsBoundsMaxDistance(group->mins, group->maxs, planes) <= 0.0f)
        {
            v3 = 1;
            goto LABEL_7;
        }
