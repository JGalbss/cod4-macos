...
#ifdef KISAK_METAL
    // Native Metal never creates a D3D vertex buffer. Dynamic engine meshes
    // (FX, tracers, marks and screen quads) retain CPU storage here instead.
    unsigned __int8* cpuData;
#endif
};
...
// childrenOffset is a byte offset in the IW3 asset format. Keeping that
// interpretation explicit matters on 64-bit targets, where this native struct
// is larger than the original 44-byte x86 record because it contains a pointer.
inline GfxAabbTree *GfxAabbTree_GetChildren(GfxAabbTree *tree)
{
    return reinterpret_cast<GfxAabbTree *>(reinterpret_cast<unsigned char *>(tree) + tree->childrenOffset);
}

inline const GfxAabbTree *GfxAabbTree_GetChildren(const GfxAabbTree *tree)
{
    return reinterpret_cast<const GfxAabbTree *>(reinterpret_cast<const unsigned char *>(tree) + tree->childrenOffset);
}
