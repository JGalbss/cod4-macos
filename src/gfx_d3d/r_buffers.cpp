...
void __cdecl R_InitDynamicVertexBufferState(GfxVertexBufferState *vb, int bytes)
{
    ...
    vb->buffer = nullptr;
    vb->verts = nullptr;
    vb->cpuData = reinterpret_cast<unsigned __int8 *>(
        R_AllocGlobalVariable(bytes, "Metal dynamic vertex buffer"));
    iassert(vb->cpuData);
#else
...
void __cdecl R_CreateParticleCloudBuffer()
{
    ...
    [[maybe_unused]] int indexSizeInBytes; // [esp+48h] [ebp-38h]
    int xIter; // [esp+4Ch] [ebp-34h]
    [[maybe_unused]] int vertSizeInBytes; // [esp+50h] [ebp-30h]
