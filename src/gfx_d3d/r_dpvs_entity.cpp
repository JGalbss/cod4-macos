...
void __cdecl R_AddEntitySurfacesInFrustumCmd(unsigned __int16 *data)
{
    ...
    // This worker payload contains three pointers.  The decompiled 32-bit
    // access pattern previously read planes/entVisData through unsigned int,
    // truncating native arm64 addresses as soon as another player DObj became
    // visible.  Access the actual pointer-sized command structure instead.
    const DpvsEntityCmd *const command =
        reinterpret_cast<const DpvsEntityCmd *>(data);
    sceneEnt = command->sceneEnt;
    ...
    LABEL_13:
                ...
                localSceneEnt->cull.maxs))
        {
            ...
            command->entVisData[localSceneEnt->entnum] = 1;
        }
        else
        {
