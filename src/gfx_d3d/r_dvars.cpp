...
#ifdef KISAK_METAL
     // Metal submission is owned by the engine thread.  The legacy D3D back-end
     // thread/event bridge is not part of this renderer; enabling this dvar would
     // hand a frame to a thread that does not exist.  Front-end worker commands
     // remain independently threaded through r_smp_worker.
     r_smp_backend = Dvar_RegisterBool("r_smp_backend", false, DVAR_ROM, "Metal submits the renderer back end on its owning thread");
#else
