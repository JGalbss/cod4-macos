...
static bool s_waterTablesInitialized;
...
bool __cdecl R_GenerateWaterTexturePixels(
    water_t *water,
    float floatTime,
    unsigned __int8 *pixels,
    unsigned int pixelCapacity)
{
    if (!water || !pixels || water->M <= 0 || water->N <= 0
        || water->M != water->N || water->M * water->N > HCOUNT
        || static_cast<unsigned int>(water->M * water->N) > pixelCapacity)
    {
        return false;
    }

    // The D3D backend normally initializes these lookup tables during
    // R_Init.  The native Metal renderer does not enter that backend, so keep
    // the exact CPU water simulation usable from either initialization path.
    if (!s_waterTablesInitialized)
        R_InitWater();

    water->writable.floatTime = floatTime;
    WaterFrequenciesAtTime(waterGlob.H, water, floatTime);
    WaterAmplitudesFromFrequencies(waterGlob.H, water);
    WaterPixelsFromAmplitudes(reinterpret_cast<GfxColor *>(waterGlob.pixels),
                              waterGlob.H, water);
    std::memcpy(pixels, waterGlob.pixels,
