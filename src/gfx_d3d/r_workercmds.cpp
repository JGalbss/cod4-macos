...
volatile WorkerCmdType g_waitTypeMainThread = static_cast<WorkerCmdType>(-1);
...
WorkerCmdType g_workerCmdMinType = static_cast<WorkerCmdType>(0x7FFFFFFF);
int __cdecl R_ProcessWorkerCmdsWithTimeoutInternal(int(__cdecl *timeout)())
{
    ...
    restart_2:
        ...
        for (type = minType; type < WRKCMD_COUNT; ++type)
        {
            ...
            InterlockedCompareExchange((volatile unsigned int*)&g_workerCmdMinType, static_cast<unsigned int>(0x7FFFFFFF), static_cast<unsigned int>(type));
        }
        if (timeout())
            return 1;
...
{
    if (g_workerCmdMinType > type)
        InterlockedCompareExchange((volatile unsigned int *)&g_workerCmdMinType, static_cast<unsigned int>(type), static_cast<unsigned int>(g_workerCmdMinType));
...
int __cdecl R_ProcessWorkerCmd(WorkerCmdType type)
{
    ...
    while (InterlockedExchangeAdd((LONG*)&workerCmds->outSize, static_cast<LONG>(-1)) <= 0)
    {
        if (InterlockedExchangeAdd((LONG*)&workerCmds->outSize, static_cast<LONG>(1)) < 0)
            return 0;
    }
    if (g_cmdOutputBusy[type])
    {
        while (1)
        {
            ...
            {
                InterlockedExchangeAdd((LONG*)&workerCmds->outSize, static_cast<LONG>(1));
                return 0;
            }
        ...
        InterlockedExchangeAdd((LONG*)&workerCmds->inSize, static_cast<LONG>(-1));
        if (g_workerCmdWaitCount)
            Sys_SetWorkerCmdEvent();
    }
    else
    {
        ...
        if (InterlockedExchangeAdd((LONG*)&workerCmds->outSize, static_cast<LONG>(-9)) < 9)
        {
            InterlockedExchangeAdd((LONG*)&workerCmds->outSize, static_cast<LONG>(9));
            count = 1;
        }
        else
        {
        ...
        //InterlockedExchangeAdd(&workerCmds->inSize, static_cast<LONG>(-count));
        InterlockedExchangeAdd((LONG*)&workerCmds->inSize, static_cast<LONG>(-(int)count));
        if (g_workerCmdWaitCount)
            Sys_SetWorkerCmdEvent();
    }
    return 1;
...
{
    static volatile LONG workerTraceCount = 0;
    static const bool traceWorkers = std::getenv("KISAK_WORKER_TRACE") != nullptr;
    if (traceWorkers && !Sys_IsMainThread())
    {
        const LONG traceIndex = InterlockedIncrement(&workerTraceCount);
        if (traceIndex <= 24)
            Com_Printf(8, "[worker] processed renderer command type=%d sample=%ld\n", static_cast<int>(type), traceIndex);
    }
...
int R_InitWorkerCmds()
{
    ...
    g_workerCmds[0].bufSize = sizeof(g_UpdateFxSpotLightBuf);
    g_workerCmds[0].dataSize = sizeof(g_UpdateFxSpotLightBuf[0]);

    g_workerCmds[1].buf = (unsigned __int8 *)g_UpdateFxNonDependentBuf;
    g_workerCmds[1].bufSize = sizeof(g_UpdateFxNonDependentBuf);
    g_workerCmds[1].dataSize = sizeof(g_UpdateFxNonDependentBuf[0]);

    g_workerCmds[2].buf = (unsigned __int8 *)g_UpdateFxRemainingBuf;
    g_workerCmds[2].bufSize = sizeof(g_UpdateFxRemainingBuf);
    g_workerCmds[2].dataSize = sizeof(g_UpdateFxRemainingBuf[0]);

    g_workerCmds[3].buf = (unsigned __int8 *)g_dpvsCellStaticBuf;
    g_workerCmds[3].bufSize = sizeof(g_dpvsCellStaticBuf);
    g_workerCmds[3].dataSize = sizeof(g_dpvsCellStaticBuf[0]);

    g_workerCmds[4].buf = (unsigned __int8 *)g_dpvsCellSceneEntBuf;
    g_workerCmds[4].bufSize = sizeof(g_dpvsCellSceneEntBuf);
    g_workerCmds[4].dataSize = sizeof(g_dpvsCellSceneEntBuf[0]);

    g_workerCmds[5].buf = (unsigned __int8 *)g_dpvsCellDynModelBuf;
    g_workerCmds[5].bufSize = sizeof(g_dpvsCellDynModelBuf);
    g_workerCmds[5].dataSize = sizeof(g_dpvsCellDynModelBuf[0]);

    g_workerCmds[6].buf = (unsigned __int8 *)g_dpvsCellDynBrushBuf;
    g_workerCmds[6].bufSize = sizeof(g_dpvsCellDynBrushBuf);
    g_workerCmds[6].dataSize = sizeof(g_dpvsCellDynBrushBuf[0]);

    g_workerCmds[7].buf = (unsigned __int8 *)g_dpvsEntityBuf;
    g_workerCmds[7].bufSize = sizeof(g_dpvsEntityBuf);
    g_workerCmds[7].dataSize = sizeof(g_dpvsEntityBuf[0]);

    g_workerCmds[8].buf = (unsigned __int8 *)g_addSceneEntBuf;
    g_workerCmds[8].bufSize = sizeof(g_addSceneEntBuf);
    g_workerCmds[8].dataSize = sizeof(g_addSceneEntBuf[0]);

    g_workerCmds[9].buf = (unsigned __int8 *)g_spotShadowEntBuf;
    g_workerCmds[9].bufSize = sizeof(g_spotShadowEntBuf);
    g_workerCmds[9].dataSize = sizeof(g_spotShadowEntBuf[0]);

    g_workerCmds[10].buf = (unsigned __int8 *)g_shadowCookieBuf;
    g_workerCmds[10].bufSize = sizeof(g_shadowCookieBuf);
    g_workerCmds[10].dataSize = sizeof(g_shadowCookieBuf[0]);

    g_workerCmds[11].buf = (unsigned __int8 *)g_GfxEntityBoundsBuf;
    g_workerCmds[11].bufSize = sizeof(g_GfxEntityBoundsBuf);
    g_workerCmds[11].dataSize = sizeof(g_GfxEntityBoundsBuf[0]);

    g_workerCmds[12].buf = (unsigned __int8 *)g_SkinGfxEntityBuf;
    g_workerCmds[12].bufSize = sizeof(g_SkinGfxEntityBuf);
    g_workerCmds[12].dataSize = sizeof(g_SkinGfxEntityBuf[0]);

    g_workerCmds[13].buf = (unsigned __int8 *)g_GenerateFxVertsBuf;
    g_workerCmds[13].bufSize = sizeof(g_GenerateFxVertsBuf);
    g_workerCmds[13].dataSize = sizeof(g_GenerateFxVertsBuf[0]);

    g_workerCmds[14].buf = (unsigned __int8 *)g_GenerateMarkVertsBuf;
    g_workerCmds[14].bufSize = sizeof(g_GenerateMarkVertsBuf);
    g_workerCmds[14].dataSize = sizeof(g_GenerateMarkVertsBuf[0]);

    g_workerCmds[15].buf = (unsigned __int8 *)g_skinCachedStaticModelBuf;
    g_workerCmds[15].bufSize = sizeof(g_skinCachedStaticModelBuf);
    g_workerCmds[15].dataSize = sizeof(g_skinCachedStaticModelBuf[0]);

    g_workerCmds[16].buf = (unsigned __int8 *)g_SkinXModelBuf;
    g_workerCmds[16].bufSize = sizeof(g_SkinXModelBuf);
    g_workerCmds[16].dataSize = sizeof(g_SkinXModelBuf[0]);
...
void __cdecl R_AddWorkerCmd(WorkerCmdType type, unsigned __int8 *data)
{
    ...
    if (r_smp_worker->current.enabled && sys_smp_allowed->current.enabled)
    {
        ...
        if (InterlockedExchangeAdd((LONG*)&workerCmds->inSize, static_cast<LONG>(1)) < bufCount)
        {
            endPos = InterlockedExchangeAdd((LONG*)&workerCmds->endPos, static_cast<LONG>(dataSize)) % bufSize;
            iassert( (endPos >= 0) );
            if (!endPos)
                InterlockedExchangeAdd((LONG*)&workerCmds->endPos, static_cast<LONG>(-bufSize));
            ...
            do
            {
                while (*Destination != endPos)
                    ;
            } while (InterlockedCompareExchange(Destination, static_cast<LONG>((dataSize + endPos) % bufSize), static_cast<LONG>(endPos)) != endPos);
            InterlockedExchangeAdd((LONG*)&workerCmds->outSize, static_cast<LONG>(1));
            R_NotifyWorkerCmdType(type);
            return;
        }
