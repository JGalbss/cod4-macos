...
FxImpactTable *__cdecl CG_RegisterImpactEffects_LoadObj(const char *mapname)
{
    ...
    fx = reinterpret_cast<FxImpactTable *>(Hunk_AllocAlign(
        sizeof(FxImpactTable), alignof(FxImpactTable), "CG_RegisterImpactEffects", 8));
    fx->table = reinterpret_cast<FxImpactEntry *>(Hunk_AllocAlign(
        sizeof(FxImpactEntry) * 12, alignof(FxImpactEntry), "CG_RegisterImpactEffects", 8));
    iBadCount = 0;
    for (i = 0; i < 0xC; ++i)
    {
