...
int32_t g_animRateOffsets[33] =
{
  ...
  -1,
  -1,
  static_cast<int32_t>(offsetof(WeaponDef, iHoldFireTime)),
  -1,
  -1,
  static_cast<int32_t>(offsetof(WeaponDef, iMeleeTime)),
  static_cast<int32_t>(offsetof(WeaponDef, meleeChargeTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iReloadTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iReloadEmptyTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iReloadStartTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iReloadEndTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iRaiseTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iFirstRaiseTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iDropTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iAltRaiseTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iAltDropTime)),
  static_cast<int32_t>(offsetof(WeaponDef, quickRaiseTime)),
  static_cast<int32_t>(offsetof(WeaponDef, quickDropTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iEmptyRaiseTime)),
  static_cast<int32_t>(offsetof(WeaponDef, iEmptyDropTime)),
  static_cast<int32_t>(offsetof(WeaponDef, sprintInTime)),
  static_cast<int32_t>(offsetof(WeaponDef, sprintLoopTime)),
  static_cast<int32_t>(offsetof(WeaponDef, sprintOutTime)),
  -1,
  static_cast<int32_t>(offsetof(WeaponDef, nightVisionWearTime)),
  static_cast<int32_t>(offsetof(WeaponDef, nightVisionRemoveTime)),
  -1,
  -1,
  -1,
  -1,
...
namespace
{
void CG_PrewarmViewWeaponSounds(const WeaponDef *weapDef)
{
    if (!weapDef)
        return;

    // These are the aliases that can begin synchronously from first-person
    // weapon state changes.  Decoding them when the weapon becomes active keeps
    // the first shot/reload/melee frame out of AudioToolbox and filesystem work.
    const snd_alias_list_t *const directAliases[] = {
        weapDef->pullbackSoundPlayer,
        weapDef->fireSoundPlayer,
        weapDef->fireLoopSoundPlayer,
        weapDef->fireStopSoundPlayer,
        weapDef->fireLastSoundPlayer,
        weapDef->emptyFireSoundPlayer,
        weapDef->meleeSwipeSoundPlayer,
        weapDef->meleeHitSound,
        weapDef->meleeMissSound,
        weapDef->rechamberSoundPlayer,
        weapDef->reloadSoundPlayer,
        weapDef->reloadEmptySoundPlayer,
        weapDef->reloadStartSoundPlayer,
        weapDef->reloadEndSoundPlayer,
        weapDef->detonateSoundPlayer,
        weapDef->altSwitchSoundPlayer,
        weapDef->raiseSoundPlayer,
        weapDef->firstRaiseSoundPlayer,
        weapDef->putawaySoundPlayer,
    };
    for (const snd_alias_list_t *aliasList : directAliases)
        SND_PrewarmAliasList(aliasList);

    // Reload clips and other model-authored foley are commonly dispatched by
    // animation notetracks rather than the direct WeaponDef sound fields.
    for (unsigned int mapIndex = 0; mapIndex < 16; ++mapIndex)
    {
        if (!weapDef->notetrackSoundMapValues[mapIndex])
            continue;
        const char *const aliasName =
            SL_ConvertToString(weapDef->notetrackSoundMapValues[mapIndex]);
        SND_PrewarmAliasList(Com_FindSoundAliasNoErrors(aliasName));
    }
}
} // namespace
#endif
...
void __cdecl CG_RegisterWeapon(int32_t localClientNum, uint32_t weaponNum)
{
    ...
    if (weaponNum)
    {
        ...
        if (!weapInfo->registered)
        {
            ...
            // stanceMaterials[weaponNum - 129] in the decompile: hintMaterials[133]
            // sits directly before stanceMaterials, so the original index folded into a
            // negative one on the following array. x86 quietly wrote back inside cgMedia;
            // here the negative index is zero-extended to a 4GB offset and faults.
...
void __cdecl CG_UpdateViewWeaponAnim(int32_t localClientNum)
{
    ...
    if (cgameGlob->predictedPlayerState.pm_type < PM_DEAD)
    {
        ...
        if (weaponIndex > 0)
        {
            ...
            iassert(weapInfo->viewModelDObj);
#ifndef _WIN32
            // The DObj identity changes when the cgame/weapon is rebuilt, while
            // staying stable across ordinary frames.  This avoids per-frame
            // alias walks without extending the original fixed-size weaponInfo.
            static const DObj_s *prewarmedViewModels[MAX_WEAPONS] = {};
            if (prewarmedViewModels[weaponIndex] != weapInfo->viewModelDObj)
            {
                prewarmedViewModels[weaponIndex] = weapInfo->viewModelDObj;
                CG_PrewarmViewWeaponSounds(BG_GetWeaponDef(weaponIndex));
            }
#endif
            ...
            if (std::getenv("KISAK_ANIM_TRACE"))
            {
                static int lastWeapon = -1;
                static int lastWeapAnim = -1;
                static unsigned int activeTraceFrame = 0;
                const int baseAnim = ps->weapAnim & 0xFFFFFDFF;
                const bool stateChanged = weaponIndex != lastWeapon || ps->weapAnim != lastWeapAnim;
                const bool sampleActive = baseAnim > 1 && (++activeTraceFrame % 12u) == 0;
                if (stateChanged || sampleActive)
                {
                    XAnimTree_s *const tree = DObjGetTree(weapInfo->viewModelDObj);
                    XAnim_s *const anims = tree ? XAnimGetAnims(tree) : nullptr;
                    Com_Printf(8,
                               "[weapon-anim] weapon=%d state=%d weapAnim=%d base=%d dt=%.4f",
                               weaponIndex, ps->weaponstate, ps->weapAnim, baseAnim, dtime);
                    if (tree && anims)
                    {
                        const unsigned int animCount = std::min<unsigned int>(anims->size, 33u);
                        for (unsigned int animIndex = 1; animIndex < animCount; ++animIndex)
                        {
                            const float weight = static_cast<float>(XAnimGetWeight(tree, animIndex));
                            if (weight <= 0.0001f)
                                continue;
                            Com_Printf(8, " [%u:%s t=%.3f w=%.3f%s]", animIndex,
                                       XAnimGetAnimName(anims, animIndex),
                                       XAnimGetTime(tree, animIndex), weight,
                                       XAnimHasFinished(tree, animIndex) ? " done" : "");
                        }
                    }
                    Com_Printf(8, "\n");
                }
                lastWeapon = weaponIndex;
                lastWeapAnim = ps->weapAnim;
            }
...
double __cdecl GetWeaponAnimRate(WeaponDef *weapDef, XAnim_s *anims, uint32_t animIndex)
{
    ...
    if (offset < 0)
        return 1.0;
    time = *reinterpret_cast<const int32_t *>(reinterpret_cast<const char *>(weapDef) + offset);
    if (time < 0)
...
#endif
    {
        ...
        if ((ps->eFlags & 0x300) == 0)
        {
            ...
            else
            {
                ...
                weapDef = BG_GetWeaponDef(weaponIndex);
                static unsigned int weaponTraceFrame = 0;
                if (std::getenv("KISAK_WEAPON_TRACE") && (weaponTraceFrame++ % 30u) == 0)
                {
                    Com_Printf(8,
                        "[weapon] '%s' state=%d anim=%d posFrac=%.3f pmFlags=%08x "
                        "origin=(%.3f %.3f %.3f) angles=(%.3f %.3f %.3f) "
                        "gunOfs=(%.3f %.3f %.3f) gunSpeed=(%.3f %.3f %.3f) "
                        "viewKick=(%.3f %.3f %.3f) kickVel=(%.3f %.3f %.3f)\n",
                        weapDef->szInternalName ? weapDef->szInternalName : "(null)",
                        ps->weaponstate, ps->weapAnim, ps->fWeaponPosFrac, ps->pm_flags,
                        placement.base.origin[0], placement.base.origin[1], placement.base.origin[2],
                        angles[0], angles[1], angles[2],
                        ws.vGunOffset[0], ws.vGunOffset[1], ws.vGunOffset[2],
                        ws.vGunSpeed[0], ws.vGunSpeed[1], ws.vGunSpeed[2],
                        cgameGlob->kickAngles[0], cgameGlob->kickAngles[1], cgameGlob->kickAngles[2],
                        cgameGlob->kickAVel[0], cgameGlob->kickAVel[1], cgameGlob->kickAVel[2]);
                }
    ...
    const playerState_s *ps)
{
    ...
    if (weapon)
    {
        if (weapon < BG_GetNumWeapons())
        {
            ...
            if (std::getenv("KISAK_AUDIO_TRACE"))
            {
                Com_Printf(9,
                    "[weapon-audio] weapon='%s' event=%d player=%d list=%p alias='%s' count=%d\n",
                    weaponDef->szInternalName ? weaponDef->szInternalName : "(null)",
                    event, isPlayer, static_cast<void *>(firesound),
                    firesound && firesound->aliasName ? firesound->aliasName : "",
                    firesound ? firesound->count : 0);
            }
            if (firesound)
            {
                if (isPlayer)
