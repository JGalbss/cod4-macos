...
void __cdecl CG_CreatePhysicsObject(int localClientNum, centity_s *cent)
{
    ...
    if (PhysPreset)
    {
        ...
        if (v7)
        {
            ...
            cent->pose.physObjId = Phys_ObjToId(v7);
        }
        else
        {
...
void __cdecl CG_UpdatePhysicsPose(centity_s *cent)
{
    ...
    Sys_EnterCriticalSection(CRITSECT_PHYSICS);
    Phys_ObjGetInterpolatedState(PHYS_WORLD_FX, Phys_ObjFromId(cent->pose.physObjId), cent->pose.origin, v3);
    Sys_LeaveCriticalSection(CRITSECT_PHYSICS);
...
void __cdecl CG_SaveEntityPhysics(centity_s *cent, SaveGame *save)
{
    ...
    if (v11)
    {
        Phys_ObjSave(Phys_ObjFromId(cent->pose.physObjId), SaveMemory_GetMemoryFile(save));
        iassert(obj);
...
void __cdecl CG_LoadEntityPhysics(centity_s *cent, SaveGame *save)
{
    ...
    if (v8)
    {
        cent->pose.physObjId = Phys_ObjToId(Phys_ObjLoad(PHYS_WORLD_FX, SaveMemory_GetMemoryFile(save)));
        CString = MemFile_ReadCString(SaveMemory_GetMemoryFile(save));
        xmodel = R_RegisterModel(CString);
        iassert(xmodel);
        Phys_ObjSetCollisionFromXModel(xmodel, PHYS_WORLD_FX, Phys_ObjFromId(cent->pose.physObjId));
    }
}
