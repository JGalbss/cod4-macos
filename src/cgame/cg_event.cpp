...
void __cdecl CG_Obituary(int32_t localClientNum, const entityState_s *ent)
{
    ...
    if (victimCI->infoValid)
    {
        ...
        if (playerCI->infoValid)
        {
            ...
            else if (attacker == static_cast<uint32_t>(ps->clientNum))
            {
                if (!cgameGlob->inKillCam)
                {
                    if (attackerCI->oldteam && victimCI->oldteam == attackerCI->oldteam)
                        s = va("CGAME_YOUKILLED%c%s%cCGAME_TEAMMATE", 21, targetName, 20);
                    else
                        s = va("CGAME_YOUKILLED%c%s", 21, targetName);
            ...
            {
                if (attackerCI->oldteam && victimCI->oldteam == attackerCI->oldteam)
                    s = va("CGAME_YOUWEREKILLED%c%s%cCGAME_TEAMMATE", 21, attackerName, 20);
                else
                    s = va("CGAME_YOUWEREKILLED%c%s", 21, attackerName);
                CG_PriorityCenterPrint(localClientNum, s, 0);
            }
            if (!cgameGlob->inKillCam)
