...
{
    // This builds a devgui menu, and DevGui_AddCommand walks the existing tree with
    // I_stricmp for every entry, so the cost is quadratic in the number of rawfiles.
    // With every zone's rawfiles loaded that is minutes of startup for a menu only a
    // developer opens - and every add reports the path is already in use anyway.
    // Opt in rather than keying off `developer`, which the shipped dev config sets.
    static const dvar_t *cg_visionSetDevMenu = nullptr;
    if (!cg_visionSetDevMenu)
        cg_visionSetDevMenu = Dvar_RegisterBool("cg_visionSetDevMenu", 0, DVAR_NOFLAG,
                                                "Build the devgui vision set menu. Slow: quadratic in rawfile count.");
    if (!cg_visionSetDevMenu->current.enabled)
        return;
