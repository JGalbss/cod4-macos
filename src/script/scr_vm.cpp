...
static const char *g_lastBuiltinName;
static constexpr unsigned int kOpcodeTraceLen = 32;
static unsigned char g_opcodeTrace[kOpcodeTraceLen];
static unsigned int g_opcodeTracePos;
...
char* __cdecl Scr_GetNextCodepos(VariableValue* top, const char* pos, int opcode, int mode, unsigned int* localId)
{
    ...
    unsigned int v11; // [esp+40h] [ebp-40h]
        ...
        if (mode == 2)
        {
            ...
            $LN54_3:
                if (top->type != 9 || scrVmPub.function_count >= 32)
                    goto LABEL_19;
                *localId = 0;
                result = (char*)(uintptr_t)top->u.intValue;
                break;
            default:
                goto LABEL_19;
            }
...
const char* __cdecl Scr_GetStackThreadPos(unsigned int endLocalId, VariableStackBuffer* stackValue, bool killThread)
{
    ...
    buf = &stackValue->buf[STACK_BUFFER_ENTRY * size];
    pos = stackValue->pos;
    while (size)
    {
        bufa = buf - sizeof(const char *);
        u.codePosValue = *(const char **)bufa;
        buf = bufa - 1;
        --size;
        if (*buf == 7)
        {
            ...
            next:
                ...
                if (GetValueType(notifyListEntry))
                {
                    ...
                    if (*((byte *)stackValue->pos - 1) == OP_waittillmatch)
                    {
                        ...
                        buf = &stackValue->buf[STACK_BUFFER_ENTRY * (stackValue->size - size)];
...
LABEL_30:
                    ...
                    else
                    {
                        ...
                        len = STACK_BUFFER_ENTRY * size;
                        bufLen = STACK_BUFFER_ENTRY * newSize + STACK_BUFFER_HEADER;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (stackId)
            {
                ...
                VariableValueAddress = GetVariableValueAddress(stackId);
                // VariableUnion grew with pointers on LP64. Reading intValue here
                // retained only the low 32 bits of the archived stack address and
                // crashed while cancelling endon notifications during shutdown.
...
VariableStackBuffer *__cdecl VM_ArchiveStack()
{
    ...
    if (size != (unsigned short)size)
        MyAssertHandler(".\\script\\scr_vm.cpp", 2768, 0, "%s", "size == (unsigned short)size");
    bufLen = STACK_BUFFER_ENTRY * size + STACK_BUFFER_HEADER;
    if (bufLen != (unsigned short)bufLen)
    ...
    while (size)
    {
        ...
        {
            // The whole union, not pointerValue: a vector or a stack buffer on the
            // stack is a pointer, and VM_UnarchiveStack reads all of it back.
            *(const char **)buf = top->u.codePosValue;
        }
        --buf;
        if (static_cast<unsigned int>(top->type) >= 0x100u)
...
/**
 * Read a signed jump displacement, which the compiler reserves and patches at
 * pointer width (EmitCodepos, then *(intptr_t *)). Distinct from Scr_ReadInt:
 * OP_jump's operand is four bytes only on a 32-bit build.
 */
intptr_t Scr_ReadCodeOffset(const char **pos)
{
    intptr_t value = *(reinterpret_cast<const intptr_t *>(*pos));
    *pos += sizeof(intptr_t);
    return value;
}
...
VariableStackBuffer *VM_ArchiveStack2(int size, const char *codePos, VariableValue *top, unsigned int localVarCount, unsigned int *localId)
{
    ...
    buf = &stackBuf->buf[STACK_BUFFER_ENTRY * size];

    while (size)
    {
        pos = buf - sizeof(const char *);
...
#pragma endregion
    while (2)
    {
        ...
        g_opcodeTrace[g_opcodeTracePos++ & (kOpcodeTraceLen - 1)] = opcode;
        // Log("0x%x\n", opcode);
interrupt_return:
...
thread_return:
        ...
        case OP_GetInteger:
            INC_TOP();
            fs.top->type = VAR_INTEGER;
            // EmitGetInteger widens the literal to a codepos slot, so only the low
            // half is the value but the whole slot has to be stepped over.
        ...
        case OP_GetAnimation:
            ...
            // EmitAnimation reserves this operand with EmitCodepos, because until
            // Scr_SetAnimation resolves it the slot is a link in the list of sites
            // waiting on that animation name. Reading it as an int leaves the rest
            // of the slot in the instruction stream, where its zero half decodes as
            // OP_End. Scr_SetAnimation stores a whole scr_anim_s, so the index and
            // tree that u.intValue readers want are still the low half.
...
CallBuiltIn:
            ...
            scrVmPub.top = fs.top;
            g_lastBuiltinName = scrVmDebugPub.func_table[builtinIndex].name;
            builtInTime = scrVmDebugPub.builtInTime;
            time = __rdtsc();
            ((void (*)(void))(uintptr_t)scrCompilePub.func_table[builtinIndex])();
...
CallBuiltinMethod:
            ...
            if (GetObjectType(objectId) == VAR_ENTITY)
            {
                ...
                if (scrVmDebugPub.func_table[builtinIndex].breakpointCount)
                {
                    ...
                    [[maybe_unused]] unsigned int backup = scrVmPub.outparamcount;
                ...
                builtInTime = scrVmDebugPub.builtInTime;
                time = __rdtsc();
                ((void (*)(scr_entref_t))(uintptr_t)scrCompilePub.func_table[builtinIndex])(entref);
                timeSpent = __rdtsc() - time;
                scrVmDebugPub.builtInTime = timeSpent + builtInTime;
                scrVmDebugPub.func_table[builtinIndex].prof += timeSpent;
                ++scrVmDebugPub.func_table[builtinIndex].usage;
...
ScriptFunctionCall:
        ...
        case OP_ScriptThreadCall:
            if (scrVmPub.function_count < 31)
            {
                ...
                fs.pos = Scr_ReadCodePos(&scrVmPub.function_frame->fs.pos);
                fs.startTop = fs.top - static_cast<ptrdiff_t>(
                    Scr_ReadUnsigned(&scrVmPub.function_frame->fs.pos));
                goto thread_call;
            }
            scrVarPub.error_index = 1;
            Scr_Error("script stack overflow (too many embedded function calls)");
...
function_call:
        ...
        case OP_jump:
            jumpOffset = Scr_ReadCodeOffset(&fs.pos);
            fs.pos += jumpOffset;
            continue;
...
#define INFINITE_LOOP_TIMEOUT 5000
        ...
        case OP_endswitch:
            // Falling off the end of a switch steps over the jump table that the
            // compiler parked here. Reading it as 2 * caseCount ints only skips half
            // of it once entries are pointer-width, and execution resumes in the
            // middle of the table.
            caseCount = Scr_ReadUnsignedShort(&fs.pos);
            fs.pos += caseCount * SWITCH_CASE_ENTRY_SIZE;
            continue;
...
unsigned int __cdecl VM_Execute(unsigned int localId, const char *pos, unsigned int paramcount)
{
    ...
    Scr_ClearOutParams();
    // paramcount is unsigned. On the original 32-bit target `-paramcount`
    // wrapped in the same address width and happened to subtract. On a 64-bit
    // host it is zero-extended before pointer arithmetic, advancing the stack
    // by almost 64 GiB for every non-zero callback argument count.
...
unsigned int __cdecl Scr_GetConstString(unsigned int index)
{
    ...
    {
        Com_Printf(16, "[scr-diag] builtin='%s' GetConstString idx=%u outparam=%u | ",
                   g_lastBuiltinName ? g_lastBuiltinName : "?", index, scrVmPub.outparamcount);
        for (int slot = -3; slot <= 1; ++slot)
            Com_Printf(16, "top[%d]=%s(%llx) ", slot, var_typename[scrVmPub.top[slot].type],
                       (unsigned long long)(uintptr_t)scrVmPub.top[slot].u.codePosValue);
        Com_Printf(16, "\n[scr-diag] opcodes:");
        for (unsigned int back = kOpcodeTraceLen; back > 0; --back)
            Com_Printf(16, " %u", g_opcodeTrace[(g_opcodeTracePos - back) & (kOpcodeTraceLen - 1)]);
        Com_Printf(16, "\n");
        scrVarPub.error_index = index + 1;
        Scr_ErrorInternal();
    }
...
void IncInParam()
{
    const uintptr_t topAddr = reinterpret_cast<uintptr_t>(scrVmPub.top);
    const uintptr_t evalLow = reinterpret_cast<uintptr_t>(scrVmGlob.eval_stack) - sizeof(VariableValue);
    const uintptr_t evalHigh = reinterpret_cast<uintptr_t>(&scrVmGlob.eval_stack[1]);
    // eval_stack is the first member of scrVmGlob, which the linker places right
    // after scrVmPub - whose last member is stack[2048]. So eval_stack - 1, where
    // GetEntityFieldValue parks top, is the same address as maxstack, and the
    // main-stack overflow test below reads as full on every entity field read.
    const bool onEvalStack = topAddr >= evalLow && topAddr <= evalHigh;
    if (!onEvalStack
...
VariableValue __cdecl GetEntityFieldValue(unsigned int classnum, int entnum, int offset)
{
    ...
    scrVmPub.top = reinterpret_cast<VariableValue *>(
        reinterpret_cast<uintptr_t>(scrVmGlob.eval_stack) - sizeof(VariableValue));
...
void __cdecl VM_Resume(unsigned int timeId)
{
    ...
    [[maybe_unused]] function_stack_t stack;
...
void VM_UnarchiveStack2(unsigned int startLocalId, function_stack_t *stack, VariableStackBuffer *stackValue)
{
    ...
    while (size)
    {
        ...
        {
            startTop->u.codePosValue = *(const char **)pos;
        }

        buf = pos + sizeof(const char *);
    }
...
unsigned int Scr_GetFunc(unsigned int index)
{
    ...
    if (index < scrVmPub.outparamcount)
    {
        value = scrVmPub.top - static_cast<ptrdiff_t>(index);
        if (value->type == 9)
        {
...
XAnim_s * Scr_GetAnimTree(unsigned int index)
{
    ...
    [[maybe_unused]] int v7; // r4
    [[maybe_unused]] int v8; // r3
    const char *v9; // r3
    [[maybe_unused]] const char *v10; // r4

    if (index < scrVmPub.outparamcount)
    {
        v3 = scrVmPub.top - static_cast<ptrdiff_t>(index);
        type = v3->type;
        if (type == 6)
        {
            if (static_cast<unsigned int>(v3->u.intValue) <= scrAnimPub.xanim_num[1])
            {
                v5 = (VariableUnion *)(uintptr_t)(4 * v3->u.intValue);
                if (*(unsigned int *)((char *)(scrAnimPub.xanim_num - 128) + (uintptr_t)v5))
                    return *(XAnim_s **)((char *)(scrAnimPub.xanim_num - 128) + (uintptr_t)v5);
            }
