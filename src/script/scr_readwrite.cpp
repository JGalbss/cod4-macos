...
// Rebuilt through named members rather than the decompiler's word offsets into
// the block: those were the x86 layout, and pos is twice as wide here, so every
// field after it landed somewhere else - and pos itself was stored through an
// unsigned int, keeping half of a code pointer.
VariableStackBuffer *__cdecl Scr_ReadStack(MemoryFile *memFile)
{
    unsigned int size; // r28
    int bufLen; // r31
    VariableStackBuffer *stackBuf; // r31
    char *entry; // r29
    unsigned __int8 v11[8]; // [sp+50h] [-40h] BYREF
    VariableValue v12; // [sp+58h] [-38h] BYREF

    MemFile_ReadData(memFile, 2, v11);
    size = *(unsigned __int16 *)v11;
    ...
    stackBuf->size = static_cast<unsigned __int16>(size);
    stackBuf->bufLen = static_cast<unsigned __int16>(bufLen);
    stackBuf->pos = Scr_ReadCodepos(memFile);
    MemFile_ReadData(memFile, 1, v11);
    stackBuf->localId = Scr_ReadId(memFile, v11[0]);
    MemFile_ReadData(memFile, 1, v11);
    stackBuf->time = v11[0];
    entry = stackBuf->buf;
    for (unsigned int i = 0; i < size; ++i)
    {
        ...
        *entry = v12.type;
        *(const char **)(entry + 1) = v12.u.codePosValue;
