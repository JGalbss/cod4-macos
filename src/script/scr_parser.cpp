...
{
    const OpcodeLookup *lookup = Scr_GetPrevSourcePosOpcodeLookup(codePos);
    if (!lookup)
        return 0;
    return scrParserGlob.sourcePosLookup[index + lookup->sourcePosIndex].sourcePos;
}
...
OpcodeLookup *__cdecl Scr_GetPrevSourcePosOpcodeLookup(const char *codePos)
{
    ...
    // Half-open range: an inclusive one underflows `high` to 0xFFFFFFFF the first
    // time codePos sorts below an entry, and the loop then indexes far off the end.
    // Callers reach here from the runtime-error reporter, which is exactly when
    // codePos may be junk, so the search has to stay in bounds on bad input.
    low = 0;
    high = scrParserGlob.opcodeLookupLen;
    ...
    return &scrParserGlob.opcodeLookup[low - 1];
}
