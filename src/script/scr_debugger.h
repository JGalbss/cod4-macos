...
union sval_u // sizeof=0x4 on x86, 0x8 here - it holds a pointer
{                                       // ...
    // No hand-written operator=. The two that used to live here assigned only
    // `type`, which copied the whole union on x86 because it was 4 bytes wide.
    // Here the union is 8 bytes, so that copied the low half of a pointer and
    // left the high half as whatever the destination already held: `node[0] = val2`
    // in append_node handed the parser a truncated node address and the GSC
    // compiler faulted on it while loading a map. The implicit copy assignment
    // copies the whole object, which is what every caller means.
