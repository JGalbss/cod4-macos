...
int __cdecl yylex()
{
	...
	{
	LABEL_11:
		// The token actions below write one 4-byte member of yylval.val - an int, a
		// float or a string id - and yylval is a global that outlives the token. On
		// x86 that wrote the whole union; here it leaves the other 4 bytes holding
		// the previous token's, so anything that later reads the slot as a pointer
		// sees a live high half stitched onto an unrelated low one. Start each token
		// from a defined value.
		yylval.val.node = nullptr;
		yy_cp = yy_c_buf_p;
...
//	yybackup:
		...
		yyreduce:
			...
			// The actions below stage values in valstack, and several of them set only
			// a slot's 4-byte `sourcePosValue` before passing the slot on whole. That
			// was the entire union on x86; here the other half is left over from an
			// earlier reduction, so a slot meant to carry a source position arrives as
			// a plausible-looking pointer. valstack is function-scope and reused by
			// every reduction, so clear it per reduction rather than per action.
			std::memset(valstack, 0, sizeof(valstack));

			switch (yyn)
			{
			case 1:
				yaccResult = node_pair(yyvsp[-1].val, yyvsp->val);
				break;
			case 2:
