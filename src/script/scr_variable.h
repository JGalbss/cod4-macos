...
// Bytes before buf, which is where the entries start. The callers that size
// these buffers were written as "5 * size + 11" - 11 being this offset when pos
// was 4 bytes wide. It is 15 here, so those allocations came up five bytes short
// and the entries ran off the end of the block the script memory tree handed
// out, over the free-list links of whatever node came next. The tree then
// handed out a node it had already allocated.
#define STACK_BUFFER_HEADER (offsetof(VariableStackBuffer, buf))

// One archived stack slot: a Vartype_t byte followed by the whole VariableUnion,
// which for a codepos or a vector is a pointer. The callers spelled this as 5,
// so on this target every entry wrote eight bytes into four and the payloads
// landed one on top of the next.
#define STACK_BUFFER_ENTRY (1 + sizeof(const char *))
