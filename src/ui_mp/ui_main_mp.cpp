    ...
    if (cstate.connState == CA_ACTIVE)
    {
        ...
        if (*info)
        {
            ...
            for (i = 0; i < sharedUiInfo.mapCount; ++i)
            {
                ...
                {
                    if (!I_stricmp(szMap, sharedUiInfo.mapList[i].mapLoadName))
                    {
                                                ...
                                                if (I_stricmp(out, "RefreshServers"))
                                                {
                                                    if (I_stricmp(out, "RefreshFilter"))
                                                    {
                                                        if (I_stricmp(out, "addPlayerProfiles"))
                                                        {
                                                            ...
                                                            {
                                                                uiInfoArray.playerProfileStatus.sortDir = !uiInfoArray.playerProfileStatus.sortDir;
                                                                UI_SortPlayerProfiles(0);
                                                            }
                                                        }
                                                        else
...
Material *__cdecl UI_GetLevelShot(int index)
{
    ...
    if (!sharedUiInfo.mapList[index].levelShot)
        sharedUiInfo.mapList[index].levelShot = Material_RegisterHandle(sharedUiInfo.mapList[index].imageName, 3);
