...
{
    auto *const io = static_cast<InputOutput *>(userData);
    const objInfo *const input = io->Input;
    Results *const results = io->Output;
...
{
    [[maybe_unused]] BrushInfo *ClassData; // eax
    ...
    if (!geom)
        return 0;
    ClassData = (BrushInfo *)dGeomGetClassData(geom);
