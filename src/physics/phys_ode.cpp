...
namespace
{
constexpr uint32_t PHYS_OBJ_HANDLE_SLOT_BITS = 10;
constexpr uint32_t PHYS_OBJ_HANDLE_SLOT_MASK = (1u << PHYS_OBJ_HANDLE_SLOT_BITS) - 1u;
constexpr uint32_t PHYS_OBJ_HANDLE_GENERATION_MASK = 0x000FFFFFu;

struct PhysObjHandleSlot
{
    dxBody *body;
    uint32_t generation;
};

PhysObjHandleSlot g_physObjHandles[512];

void Phys_ReleaseObjId(dxBody *body)
{
    if (!body)
        return;

    for (uint32_t slot = 0; slot < ARRAY_COUNT(g_physObjHandles); ++slot)
    {
        if (g_physObjHandles[slot].body == body)
        {
            g_physObjHandles[slot].body = nullptr;
            g_physObjHandles[slot].generation =
                (g_physObjHandles[slot].generation + 1u) & PHYS_OBJ_HANDLE_GENERATION_MASK;
            return;
        }
    }
}
...
PhysObjId __cdecl Phys_ObjToId(dxBody *body)
{
    if (!body)
        return PHYS_OBJ_ID_NULL;

    uint32_t freeSlot = ARRAY_COUNT(g_physObjHandles);
    for (uint32_t slot = 0; slot < ARRAY_COUNT(g_physObjHandles); ++slot)
    {
        if (g_physObjHandles[slot].body == body)
        {
            return static_cast<PhysObjId>(
                (g_physObjHandles[slot].generation << PHYS_OBJ_HANDLE_SLOT_BITS) | (slot + 1u));
        }
        if (!g_physObjHandles[slot].body && freeSlot == ARRAY_COUNT(g_physObjHandles))
            freeSlot = slot;
    }

    if (freeSlot == ARRAY_COUNT(g_physObjHandles))
    {
        MyAssertHandler(__FILE__, __LINE__, 0, "%s", "freeSlot < ARRAY_COUNT(g_physObjHandles)");
        return PHYS_OBJ_ID_NULL;
    }

    g_physObjHandles[freeSlot].body = body;
    return static_cast<PhysObjId>(
        (g_physObjHandles[freeSlot].generation << PHYS_OBJ_HANDLE_SLOT_BITS) | (freeSlot + 1u));
}

dxBody *__cdecl Phys_ObjFromId(PhysObjId id)
{
    if (id == PHYS_OBJ_ID_NULL || id == PHYS_OBJ_ID_DEAD)
        return nullptr;

    const uint32_t encoded = static_cast<uint32_t>(id);
    const uint32_t slotToken = encoded & PHYS_OBJ_HANDLE_SLOT_MASK;
    if (!slotToken || slotToken > ARRAY_COUNT(g_physObjHandles))
    {
        MyAssertHandler(__FILE__, __LINE__, 0, "%s", "valid physics object handle slot");
        return nullptr;
    }

    PhysObjHandleSlot &slot = g_physObjHandles[slotToken - 1u];
    const uint32_t generation = encoded >> PHYS_OBJ_HANDLE_SLOT_BITS;
    if (!slot.body || slot.generation != generation)
    {
        MyAssertHandler(__FILE__, __LINE__, 0, "%s", "live physics object handle");
        return nullptr;
    }
    return slot.body;
}
...
{
    track_static_alloc_internal(&physGlob, sizeof(physGlob), "physGlob", 9);
}
...
void __cdecl Phys_Init()
{
    ...
    if (!physInited)
    {
        ...
        memset(g_physObjHandles, 0, sizeof(g_physObjHandles));
        Pool_Init(
            reinterpret_cast<char *>(physGlob.userData),
            &physGlob.userDataPool,
            sizeof(PhysObjUserData),
            ARRAY_COUNT(physGlob.userData));
        ODE_Init();
...
dxBody *__cdecl Phys_CreateBodyFromState(PhysWorld worldIndex, const BodyState *state)
{
    ...
    if (body)
    {
        ...
        if (!LOBYTE(state->underwater))
            dBodyDisable(body);
        const PhysObjId bodyId = Phys_ObjToId(body);
        static unsigned int createTraceCount = 0;
        if (std::getenv("KISAK_PHYS_TRACE") && createTraceCount++ < 128)
        {
            Com_Printf(
                20,
                "[physics] create world=%d id=%d bodies=%d pos=(%.3f %.3f %.3f) "
                "vel=(%.3f %.3f %.3f) mass=%.3f\n",
                static_cast<int>(worldIndex),
                bodyId,
                physGlob.world[worldIndex]->nb,
                state->position[0],
                state->position[1],
                state->position[2],
                state->velocity[0],
    ...
    const PhysMass *physMass)
{
    ...
    // On x86 the brush union's pointer member was four bytes wide, so the
    // decompile could address these two floats through cylinderState. A
    // native pointer is eight bytes wide: retaining those aliases either
    // leaves an inertia component uninitialized (brush models) or overwrites
    // the upper half of a live cbrush_t pointer (brushes).
    ...
    iassert(geomState.u.brushState.u.brushModel == brushModel);
    geomState.isOriented = 0;
...
void __cdecl Phys_ObjAddGeomBrush(PhysWorld worldIndex, dxBody *id, const cbrush_t *brush, const PhysMass *physMass)
{
    ...
    geomState.u.brushState.productsOfInertia[1] = physMass->productsOfInertia[1];
    geomState.u.brushState.productsOfInertia[2] = physMass->productsOfInertia[2];
    iassert(geomState.u.brushState.u.brush == brush);
    geomState.isOriented = 0;
    dBodyGetMass(id, &mass);
    Phys_BodyAddGeomAndSetMass(worldIndex, id, mass.mass, &geomState, physMass->centerOfMass);
}
...
void __cdecl Phys_ObjDestroy(PhysWorld worldIndex, dxBody *id)
{
    ...
    const PhysObjId bodyId = Phys_ObjToId(id);
    static unsigned int destroyTraceCount = 0;
    if (std::getenv("KISAK_PHYS_TRACE") && destroyTraceCount++ < 128)
    {
        Com_Printf(
            20,
            "[physics] destroy world=%d id=%d bodies-before=%d\n",
            static_cast<int>(worldIndex),
            bodyId,
            physGlob.world[worldIndex]->nb);
    }
    Phys_ReleaseObjId(id);
    dBodyDestroy(id);
#ifdef USE_POOL_ALLOCATOR
    Sys_EnterCriticalSection(CRITSECT_PHYSICS);
    ...
    const ScreenPlacement *scrPlace)
{
    ...
    *y = *y + charHeight;
    // The x86 decompile reconstructed this integer subtraction as pointer
    // arithmetic.  On 64-bit macOS that also passes a pointer to the %i
    // formatter, producing undefined varargs behaviour whenever physics debug
...
void __cdecl Phys_RunToTime(int localClientNum, PhysWorld worldIndex, int timeNow)
{
    ...
    world = physGlob.world[worldIndex];
    int timeBeforeUpdate = data->timeLastUpdate;

    // FX and ragdoll clocks use the current game/server-time epoch, which can be
    // millions of milliseconds away from zero.  A world has no elapsed simulation
    // to catch up on before its first RunToTime call; integrating that entire epoch
    // in two giant steps launches the first spawned body to invalid coordinates.
    // Establish the initial snapshot at the requested time, then let subsequent
    // calls advance through the normal fixed-step path.
    if (data->timeLastSnapshot == 0 && data->timeLastUpdate == 0)
    {
        data->timeLastSnapshot = timeNow;
        data->timeLastUpdate = timeNow;
        data->timeNowLerpFrac = 1.0f;
        ODE_ForEachBody(world, Phys_BodyGrabSnapshot);
        timeBeforeUpdate = timeNow;
    }
    else if (timeNow < data->timeLastSnapshot)
    {
    ...
    static unsigned int idleTraceCount[PHYS_WORLD_COUNT] = {};
    static unsigned int activeTraceCount[PHYS_WORLD_COUNT] = {};
    const bool tracePhysics = std::getenv("KISAK_PHYS_TRACE") != nullptr;
    const bool traceActive = world->nb > 0 && activeTraceCount[worldIndex]++ < 120;
    const bool traceIdle = world->nb == 0
        && idleTraceCount[worldIndex]++ < 120
        && (idleTraceCount[worldIndex] % 30u) == 1u;
    if (tracePhysics && (traceActive || traceIdle))
    {
        Com_Printf(
            20,
            "[physics] step world=%d bodies=%d requested=%d advanced=%d snapshot=%d update=%d lerp=%.3f\n",
            static_cast<int>(worldIndex),
            world->nb,
            timeNow,
            data->timeLastUpdate - timeBeforeUpdate,
        ...
        {
            float center[3];
            Phys_BodyGetCenterOfMass(world->firstbody, center);
            const float *const velocity = dBodyGetLinearVel(world->firstbody);
            const float *const angularVelocity = dBodyGetAngularVel(world->firstbody);
            Com_Printf(
                20,
                "[physics] body world=%d pos=(%.3f %.3f %.3f) "
                "vel=(%.3f %.3f %.3f) angular=(%.3f %.3f %.3f) enabled=%d\n",
                static_cast<int>(worldIndex),
                center[0], center[1], center[2],
                velocity[0], velocity[1], velocity[2],
                angularVelocity[0], angularVelocity[1], angularVelocity[2],
                dBodyIsEnabled(world->firstbody));
        }
    }
    ...
    {
        // These timestamps are integers, but the render state must interpolate
        // continuously between the two fixed ODE snapshots.  Integer division
        // collapsed the fraction to only 0 or 1, making native dynamic props,
        // debris and physics-driven effects visibly snap at the 17 ms step.
        data->timeNowLerpFrac = static_cast<float>(timeNow - data->timeLastSnapshot)
            / static_cast<float>(data->timeLastUpdate - data->timeLastSnapshot);
...
void __cdecl Phys_GetStateFromBody(dxBody *body, BodyState *state)
{
    ...
    state->underwater = dBodyIsEnabled(body);
}
