...
struct dxUserGeom : public dxGeom {
    // The largest custom payload is BrushInfo.  Its brush pointer makes the
    // original 16-byte x86 layout 24 bytes on a 64-bit target (including the
    // pointer-alignment tail padding).
    char user_data[sizeof(void *) == 8 ? 24 : 16]; // MOD
