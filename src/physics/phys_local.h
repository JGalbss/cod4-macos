...
// Retail stored ODE body pointers in 32-bit integer fields.  Those fields are
// part of several fixed-size runtime/serialized structures (cpose_t, FxElem,
// DynEntityClient), so widening them would break far more than it fixes on
// LP64.  Keep the ABI-sized token and resolve it through a generation-checked
// native handle table instead of truncating an arm64 pointer.
using PhysObjId = int32_t;
constexpr PhysObjId PHYS_OBJ_ID_NULL = 0;
constexpr PhysObjId PHYS_OBJ_ID_DEAD = -1;
