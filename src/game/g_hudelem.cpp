...
{
    // VariableUnion grows to pointer width on LP64, but this field is an int.
    // An eight-byte union assignment corrupts the next HUD element's type.
    *reinterpret_cast<int32_t *>((char *)&hud->elem.type + fields_0[offset].ofs) = Scr_GetInt(0);
}
...
void __cdecl HECmd_ScaleOverTime(scr_entref_t entref)
{
    const char *v1; // eax
    const char *v2; // eax
    [[maybe_unused]] float v3; // [esp+Ch] [ebp-20h]
...
void __cdecl HECmd_MoveOverTime(scr_entref_t entref)
{
    const char *v1; // eax
    const char *v2; // eax
    [[maybe_unused]] float v3; // [esp+Ch] [ebp-18h]
...
void __cdecl HECmd_SetPulseFX(scr_entref_t entref)
{
    ...
    // The original decompilation indexed the structure through VariableUnion.
    // That happened to be a four-byte type in the 32-bit Windows binary, but it
    // contains pointers and is eight bytes on LP64. As a result these writes
    // ran into the following hudelem on macOS (fxBirthTime landed on the next
    // element's fromAlignScreen and fxDecayDuration landed on its text field).
