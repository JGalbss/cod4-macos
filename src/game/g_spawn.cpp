...
void __cdecl Scr_SetGenericField(unsigned __int8 *b, fieldtype_t type, int ofs)
{
    ...
    {
    case F_INT:
        // The original 32-bit binary used a pointer-sized store here because
        // int and pointers were both four bytes. On LP64 that overwrites the
        // following field (HUD align/text values were the first visible victim).
