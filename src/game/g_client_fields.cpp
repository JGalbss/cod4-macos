...
void __cdecl ClientScr_SetSessionState(gclient_s *pSelf, const client_fields_s *pField)
{
    ...
    const sessionState_t oldState = pSelf->sess.sessionState;
    ...
    if (std::getenv("KISAK_GAMEPLAY_TRACE") && pSelf->sess.sessionState != oldState)
    {
        Com_Printf(8, "[gameplay] client %td session state: %d -> %d (%s)\n",
                   pSelf - level.clients, oldState, pSelf->sess.sessionState,
                   SL_ConvertToString(newState));
    }
}
