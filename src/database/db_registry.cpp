...
#else
    // Native fastfiles are OAT zones, whose sample bytes are borrowed from zone
    // storage.  SND_FreeData distinguishes those from loose-file buffers that
    // SND_SetData really allocated and prevents an invalid free during quit.
    SND_FreeData(&header.loadSnd->sound);
#endif
}
...
{
    // Reached only for a stub asset with no existing entry, so it is where a missing
    // cross-zone reference surfaces. KISAK_DB_TRACE prints what was asked for.
    if (std::getenv("KISAK_DB_TRACE"))
        Com_Printf(8, "[db] DB_CreateDefaultEntry type=%d name='%s'\n", (int)type, name ? name : "?");
    ...
    // Hash links store an entry index, not a byte offset.  XAssetEntry was 16
    // bytes in the original 32-bit build, but is 24 bytes once its header pointer
    // is native-width.  Dividing by the old stride silently linked a different,
    // usually still-free entry into the hash table on arm64.
    db_hashTable[DB_HashForName(name, type)] =
        static_cast<uint16_t>(reinterpret_cast<XAssetEntryPoolEntry *>(newEntry) - g_assetEntryPool);
...
XAssetHeader __cdecl DB_AllocXAssetHeader(XAssetType type)
{
    ...
    {
        Sys_UnlockWrite(&db_hashCritSect);
        // Report before Com_PrintError: the console path is not up during zone
        // loading and formatting through it faults in Con_UpdateNotifyMessage.
        std::fprintf(stderr, "[db] pool exhausted: type=%d '%s' limit=%d\n",
                     type, g_assetNames[type], g_poolSize[type]);
        std::fflush(stderr);
...
// OpenAssetTools owns the assets it loads, and they point at one another directly:
// a MenuList holds the very menuDef_t objects the zone produced. Copying an asset
// into a pool slot would make a DB lookup hand back a different object than the one
// the referencing asset holds, and UI_AddMenu checks exactly that identity. So the
// entry keeps the zone's pointer and allocates no asset storage. The pools go unused
// on this path, which also puts their fixed capacities out of the picture.
static XAssetEntryPoolEntry *DB_AllocZoneOwnedEntry(const XAsset &asset, uint8_t zoneIndex)
{
...
XAssetEntryPoolEntry *__cdecl DB_LinkXAssetEntry(XAssetEntryPoolEntry *newEntry, int32_t allowOverride)
{
    ...
    [[maybe_unused]] uint8_t zoneIndex; // [esp+23h] [ebp-889h]
...
#if defined(KISAK_OAT_ZONES)
        newEntry = DB_AllocZoneOwnedEntry(asset, g_zoneIndex);
#else
...
#if defined(KISAK_OAT_ZONES)
            // OAT entries own pointers to complete zone objects.  The stock path
            // keeps fixed pool addresses and swaps their bytes, but doing that here
            // overwrites the retained old map with the new map's asset contents.
            // Swap the entry payload pointers and owners instead; both allocations
            // stay intact and unloading the override can restore the old pointer.
            const XAsset oldAsset = existingEntry->entry.asset;
            const uint8_t oldZoneIndex = existingEntry->entry.zoneIndex;
            existingEntry->entry.asset = newEntry->entry.asset;
            existingEntry->entry.zoneIndex = newEntry->entry.zoneIndex;
            newEntry->entry.asset = oldAsset;
            newEntry->entry.zoneIndex = oldZoneIndex;
#else
...
#if defined(KISAK_OAT_ZONES)
    // Replace a generated pool-owned default entry with the actual zone pointer.
    // There is no reason to clone into either object: freeing the default payload
    // and adopting the incoming header preserves OAT's internal pointer identity.
    const XAssetHeader oldDefaultHeader = existingEntry->entry.asset.header;
    existingEntry->entry.asset = newEntry->entry.asset;
    existingEntry->entry.zoneIndex = newEntry->entry.zoneIndex;
    DB_FreeXAssetHeader(type, oldDefaultHeader);
#else
    DB_CloneXAssetEntry(&newEntry->entry, &existingEntry->entry);
#endif
...
void __cdecl DB_FreeXAssetHeader(XAssetType type, XAssetHeader header)
{
    ...
    // Freeing pushes the header onto the pool's intrusive free list, which is only
    // valid for a header the pool handed out. An asset that still points at the
    // memory its zone owns would put a foreign address on the free list, and the
    // next allocation would follow it and read whatever happens to be there.
    //
    // The engine reaches here with such a header whenever a zone re-registers a
    // name that already exists: DB_AddXAsset links with allowOverride 0, lands in
    // DB_DelayedCloneXAsset, and with g_sync set that re-enters DB_LinkXAssetEntry
    // with allowOverride 1, which frees the entry before it has been cloned into
    // the pool. It never showed up while zones failed to load at all.
    //
    // XAssetPool<T, N> is { freeHead; entries[N] }, so the pool owns exactly the
    // bytes after that first pointer.
    const size_t stride = DB_GetXAssetTypeSize(type);
    const char *base = static_cast<const char *>(pool) + sizeof(void *);
    const char *end = base + stride * static_cast<size_t>(g_poolSize[type]);
    const char *data = static_cast<const char *>(header.data);
    if (data < base || data >= end)
        return;

    DB_FreeXAssetHeaderHandler[type](pool, header);
}
...
#if defined(KISAK_OAT_ZONES)
    // Entries are the indirection layer on the OAT path.  Zone assets are already
    // fully constructed and cross-reference their exact allocation, so cloning
    // bytes would corrupt the source zone's object graph.
    to->asset = from->asset;
#else
...
void __cdecl DB_GetXAsset(XAssetType type, XAssetHeader header)
{
    ...
    XAssetEntry *sameNameEntry = nullptr;
    ...
    for (assetEntryIndex = db_hashTable[DB_HashForName(name, type)]; ; assetEntryIndex = assetEntry->nextHash)
    {
        if (!assetEntryIndex)
        {
            // Cross-zone OAT references can retain their small reference object while
            // the registry owns the canonical object with the same type and name.
            // Treat that canonical entry as the asset being touched; otherwise a zone
            // unload can release a technique set that live materials still reference.
            if (sameNameEntry)
            {
                sameNameEntry->inuse = 1;
                return;
            }
            ...
            {
                ++reported;
                Com_PrintWarning(10, "WARNING: DB_GetXAsset: '%s' (type %d) not in the hash chain\n",
                                 name ? name : "?", (int)type);
            }
            return;
        }
        ...
        {
            if (assetEntry->asset.header.xmodelPieces == header.xmodelPieces)
                break;
            const char *entryName = DB_GetXAssetName(&assetEntry->asset);
            if (entryName && name && !I_stricmp(entryName, name))
                sameNameEntry = assetEntry;
        }
    }
    assetEntry->inuse = 1;
}
...
void DB_Init()
{
    ...
    // Entry zero is the null/sentinel index used by every hash and override link.
    // The old literal 16 was the Win32 byte stride of one entry, not 16 entries.
...
void DB_TryLoadXFile()
{
    ...
    if (g_zoneInfoCount)
    {
        ...
        {
            const int32_t loaded = DB_TryLoadXFileInternal(g_zoneInfo[j].name, g_zoneInfo[j].flags);
#if defined(KISAK_OAT_ZONES)
            // g_loadingAssets counts zones still in flight and has to reach zero
            // before the next batch. Upstream only decrements it here on failure
            // because success is signalled later by the database worker, which this
            // build does not have - loading runs inline, so returning from
            // DB_TryLoadXFileInternal is the completion, whatever the outcome.
            (void)loaded;
...
int32_t __cdecl DB_TryLoadXFileInternal(char *zoneName, int32_t zoneFlags)
{
    ...
    else
    {
        ...
        // Only worth waiting for when another thread can finish $init. Loading
        // synchronously means we are that thread - Com_Init sets g_initializing,
        // calls CL_InitRenderer, and only clears it after this returns - so the
        // wait would never end. The warning it guards is advisory: it says some
        // code_post_gfx assets may be missing, not that the load is invalid.
        if (g_zoneAllocType == 1 && g_initializing && !g_sync)
        {
...
#if defined(KISAK_OAT_ZONES)
        // The stock reader cannot run here: it rebuilds the zone from x86 struct
        // layouts and 4-byte pointers, and db_stream_load.cpp needs the blocks in
        // the low 4 GB, which macOS arm64 never grants. OpenAssetTools reads the
        // same file into these very structs at LP64. See src/posix/posix_zone_oat.cpp.
        std::fclose(static_cast<FILE *>(zoneFile));
        if (!Posix_LoadZoneWithOat(filename, zone->name))
        {
...
#else
    // POSIX open() treats '\\' as an ordinary filename character, so the Windows-style
    // path made every zone lookup fail with "Could not find zone".
    Com_sprintf(filename, size, "%s/zone/%s/%s.ff", v3, Language, zoneName);
#endif
}
...
void __cdecl DB_UnloadXZone(uint32_t zoneIndex, bool createDefault)
{
    ...
    iassert(zoneIndex);
#if defined(KISAK_OAT_ZONES)
    // A stock default placeholder is made by copying its bytes over the unloaded
    // pool object.  OAT zone objects are retained for pointer stability and must
    // remain immutable across a map change, so remove an unoverridden entry instead.
    // A later lookup can still create a separate pool-owned default normally.
    createDefault = false;
#endif
    hash = 0;
...
#if defined(KISAK_OAT_ZONES)
    // The engine's entries for this zone are gone; let go of the memory they pointed
    // at, or a later reload leaves two live copies of every asset.
    Posix_ReleaseOatZone(g_zones[zoneIndex].name);
#endif
}
...
void DB_FreeDefaultEntries()
{
    ...
    for (hash = 0; hash < 0x8000; ++hash)
    {
        for (assetEntryIndex = db_hashTable[hash]; assetEntryIndex; assetEntryIndex = nextAssetEntryIndex)
        {
            ...
            {
                // OAT resolves some cross-zone reference stubs after the normal
                // unload pass.  Their entry can retain the last loading zone even
                // though all live zones are already gone.  Final shutdown owns the
                // whole database, so drain such entries here; they are not defaults
                // and must not consume g_defaultAssetCount.
                Com_PrintWarning(16,
                    "DB_FreeDefaultEntries: draining late zone asset type=%d name='%s' zone=%u\n",
                    assetEntry->entry.asset.type,
                    DB_GetXAssetName(&assetEntry->entry.asset),
                    static_cast<unsigned int>(assetEntry->entry.zoneIndex));
                DB_RemoveXAsset(&assetEntry->entry.asset);
...
int32_t __cdecl DB_FileSize(const char *zoneName, int32_t isMod)
{
    ...
    if (isMod == 2)
        DB_BuildOSPath_UserMap(zoneName, 0x100u, filename);
    else if (isMod)
