...
#else
    // A zone is usually smaller than one 0x40000 chunk (code_post_gfx_mp.ff is 87 KB), so
    // adding the full chunk size hands zlib ~175 KB of buffer past the end of the file and
    // inflate fails. Add what the read actually delivered.
    extern unsigned long kb_last_read_bytes;
    g_load.stream.avail_in += kb_last_read_bytes ? kb_last_read_bytes : 0x40000;
#endif
    return result;
}
