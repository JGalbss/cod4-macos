...
void __cdecl PrefetchArray_XSurfaceCollisionNode_(const XSurfaceCollisionNode *mem, unsigned int elementCount)
{
    ...
    for (memIter = (const unsigned __int8 *)((uintptr_t)mem & ~uintptr_t(0x7F));
        memIter <= (const unsigned __int8 *)(((uintptr_t) & mem[elementCount - 1].childCount + 1) & ~uintptr_t(0x7F));
        memIter += 128)
        ;
}
...
void __cdecl PrefetchArray_GfxPackedVertex_(const GfxPackedVertex *mem, unsigned int elementCount)
{
    ...
    if (!elementCount)
        MyAssertHandler(".\\xanim\\xmodel.cpp", 883, 0, "%s", "elementCount");
    for (memIter = (const unsigned __int8*)((uintptr_t)mem & ~uintptr_t(0x7F));
        memIter <= (const unsigned __int8*)(((uintptr_t) & mem[elementCount - 1].tangent + 3) & ~uintptr_t(0x7F));
        memIter += 128)
        ;
}
...
void __cdecl PrefetchArray_XSurfaceCollisionLeaf_(const XSurfaceCollisionLeaf *mem, unsigned int elementCount)
{
    ...
    if (!elementCount)
        MyAssertHandler(".\\xanim\\xmodel.cpp", 883, 0, "%s", "elementCount");
    for (memIter = (const unsigned __int8*)((uintptr_t)mem & ~uintptr_t(0x7F));
        memIter <= (const unsigned __int8*)(((uintptr_t) & mem[elementCount - 1].triangleBeginIndex + 1) & ~uintptr_t(0x7F));
        memIter += 128)
    {
        ;
    }
