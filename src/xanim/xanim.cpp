...
void __cdecl XAnimCreate(XAnim_s* anims, unsigned int animIndex, const char* name)
{
    ...
    {
        // A leaf left null is not noticed here: it surfaces frames later inside
        // XAnimGetAnimMap, with nothing to say which animation was missing.
        if (!parts)
            Com_PrintWarning(10, "WARNING: xanim '%s' is not loaded\n", name);
...
XAnim_s* __cdecl XAnimCreateAnims(const char* debugName, unsigned int size, void* (__cdecl* Alloc)(int))
{
    ...
    // 8 * size + 12 in the decompile: an x86 XAnimEntry and the three header words
    // before the entries. XAnimEntry holds an XAnimParts*, so it is 16 bytes here and
    // the header is 24, and every leaf past the first ninety or so was being written
    // into whatever the hunk handed out next. A missing one only shows up much later,
    // when XAnimGetAnimMap dereferences the null it read back.
    anims = (XAnim_s*)Alloc(offsetof(XAnim_s, entries) + size * sizeof(XAnimEntry));
    anims->size = size;
    ...
    if (g_anim_developer)
    {
        ...
        anims->debugAnimNames =
            (const char**)Hunk_AllocDebugMem(size * sizeof(*anims->debugAnimNames), "XAnimCreateAnims");
    }
