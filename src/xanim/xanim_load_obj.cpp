...
void* XAnimTempAlloc(unsigned int size)
{
    ...
    // The temporary decode structs contain native pointers.  Four-byte
    // alignment was sufficient for the retail x86 layouts, but is not a valid
    // pointer alignment on arm64/LP64.
...
{
template <typename T>
T *XAnimAllocCleared(void *(__cdecl *Alloc)(int), const size_t size = sizeof(T))
{
    T *const value = static_cast<T *>(Alloc(static_cast<int>(size)));
    iassert(value);
...
template <typename T>
T *XAnimTempAllocCleared(const size_t size = sizeof(T))
{
    T *const value = static_cast<T *>(XAnimTempAlloc(static_cast<unsigned int>(size)));
    iassert(value);
    std::memset(value, 0, size);
    return value;
}

size_t XAnimQuatStorageSize(const bool smallIndices, const unsigned int count)
{
    return offsetof(XAnimPartQuat, u.frames.indices)
        + count * (smallIndices ? sizeof(unsigned char) : sizeof(unsigned short));
}

size_t XAnimDeltaQuatStorageSize(const bool smallIndices, const unsigned int count)
{
    return offsetof(XAnimDeltaPartQuat, u.frames.indices)
        + count * (smallIndices ? sizeof(unsigned char) : sizeof(unsigned short));
}

size_t XAnimTransStorageSize(const bool smallIndices, const unsigned int count)
{
    return offsetof(XAnimPartTrans, u.frames.indices)
        + count * (smallIndices ? sizeof(unsigned char) : sizeof(unsigned short));
}
}
...
XModelPieces *__cdecl XModelPiecesLoadFile(const char *name, void *(__cdecl *Alloc)(int))
{
    ...
    if (version == 1)
    {
        xmodelPieces = XAnimAllocCleared<XModelPieces>(Alloc);
...
void __cdecl ConsumeQuat(unsigned __int8 **pos, __int16 *out)
{
    ...
    // Hex-rays decompiled an iQ[3] == (short)iQ[3] truncation check into a
    // tautological v2 != v2. The intent was to catch out-of-range writes;
    // already enforced via the (short) cast. Drop the dead branch.
...
void __cdecl ConsumeQuatNoSwap(unsigned __int8 **pos, __int16 *out)
{
    ...
    [[maybe_unused]] __int16 v6; // [esp+1Ch] [ebp-20h]
    [[maybe_unused]] __int16 v7; // [esp+20h] [ebp-1Ch]
    [[maybe_unused]] __int16 v8; // [esp+24h] [ebp-18h]
    int temp; // [esp+38h] [ebp-4h]
    ...
    bool useSmallIndices)
{
    ...
    if (numQuatIndices)
    {
        if (numQuatIndices == 1)
        {
            ...
            deltaPart->quat = XAnimAllocCleared<XAnimDeltaPartQuat>(Alloc);
        ...
        else
        {
            ...
            if (useSmallIndices)
            {
                deltaPart->quat = XAnimAllocCleared<XAnimDeltaPartQuat>(
                    Alloc, XAnimDeltaQuatStorageSize(true, numQuatIndices));
            ...
            {
                deltaPart->quat = XAnimAllocCleared<XAnimDeltaPartQuat>(
                    Alloc, XAnimDeltaQuatStorageSize(false, numQuatIndices));
                if (numQuatIndices >= numloopframes)
                {
    ...
    bool useSmallIndices)
{
    ...
    if (numTransIndices)
    {
        if (numTransIndices == 1)
        {
            ...
            deltaPart->trans = XAnimAllocCleared<XAnimPartTrans>(Alloc);
        ...
        else
        {
            ...
            if (useSmallIndices)
            {
                deltaPart->trans = XAnimAllocCleared<XAnimPartTrans>(
                    Alloc, XAnimTransStorageSize(true, numTransIndices));
            ...
            {
                deltaPart->trans = XAnimAllocCleared<XAnimPartTrans>(
                    Alloc, XAnimTransStorageSize(false, numTransIndices));
                if (numTransIndices >= numloopframes)
                {
...
unsigned __int8 *__cdecl GetQuaternions(
    ...
    bool useSmallIndices)
{
    [[maybe_unused]] unsigned __int16 v7; // [esp+0h] [ebp-48h]
    ...
    if (numQuatIndices)
    {
        if (numQuatIndices == 1)
        {
            if (bSimpleQuat)
            {
                ...
                part->quat = XAnimTempAllocCleared<XAnimPartQuat>();
        ...
        else
        {
            ...
            {
                part->quat = XAnimTempAllocCleared<XAnimPartQuat>(
                    XAnimQuatStorageSize(true, numQuatIndices));
                if (numQuatIndices >= numloopframes)
                {
                    for (i = 0; i < numQuatIndices; ++i)
            ...
            {
                part->quat = XAnimTempAllocCleared<XAnimPartQuat>(
                    XAnimQuatStorageSize(false, numQuatIndices));
                if (numQuatIndices >= numloopframes)
                {
                    for (j = 0; j < numQuatIndices; ++j)
    ...
    bool useSmallIndices)
{
    ...
    if (numTransIndices)
    {
        if (numTransIndices == 1)
        {
            ...
            part->trans = XAnimTempAllocCleared<XAnimPartTrans>();
            part->trans->size = 0;
            p_u = &part->trans->u;
            p_u->frames.mins[0] = mins[0];
            p_u->frames.mins[1] = mins[1];
        ...
        else
        {
            ...
            {
                part->trans = XAnimTempAllocCleared<XAnimPartTrans>(
                    XAnimTransStorageSize(true, numTransIndices));
                if (numTransIndices >= numloopframes)
                {
                    for (i = 0; i < numTransIndices; ++i)
            ...
            {
                part->trans = XAnimTempAllocCleared<XAnimPartTrans>(
                    XAnimTransStorageSize(false, numTransIndices));
                if (numTransIndices >= numloopframes)
                {
                    for (j = 0; j < numTransIndices; ++j)
...
void __cdecl ReadNoteTracks(const char *name, unsigned char **pos, XAnimParts *parts, void *(__cdecl *Alloc)(int))
{
    ...
    [[maybe_unused]] double v5; // [esp+8h] [ebp-30h]
    [[maybe_unused]] unsigned __int16 v6; // [esp+24h] [ebp-14h]
...
XAnimParts *__cdecl XAnimLoadFile(char *name, void *(__cdecl *Alloc)(int))
{
    ...
    XAnimPartQuatPtr sortedQuat[128]{};
    ...
    parts = XAnimAllocCleared<XAnimParts>(Alloc);
    ...
    if (parts->bDelta)
    {
        parts->deltaPart = XAnimAllocCleared<XAnimDeltaPart>(Alloc);
    ...
    if (numBones)
    {
        ...
        memset(v73, 0, sizeof(v73[0]) * numBones);
    ...
    if (numBones)
    {
        ...
        for (i = 0; i < numBones; ++i)
        {
            ...
            sortedQuat[i] = part[animPartIndex];
        ...
        {
            tableSize = sortedQuat[animPartIndex].quat->size;
            ++dataShortCount;
            if (useSmallIndices)
            {
        ...
        animPartIndex = parts->boneCount[PART_TYPE_NO_QUAT];
        v56 = animPartIndex + parts->boneCount[PART_TYPE_HALF_QUAT];
        while (animPartIndex < v56)
        {
            XAnimPartQuat *const quat = sortedQuat[animPartIndex].quat;
            iassert(quat);
            tableSize = quat->size;
            *dataShort++ = static_cast<unsigned short>(tableSize);
            const size_t frameBytes = sizeof(short) * 2u * (tableSize + 1u);
            memcpy(randomDataShort, quat->u.frames.u.frames2, frameBytes);
            randomDataShort += frameBytes;
            XAnimEmitFrameIndices(
                tableSize,
                &dataByte,
                &dataShort,
                &indices,
                &quat->u.frames.indices,
                useSmallIndices);
            ++animPartIndex;
        }
        v56 += parts->boneCount[PART_TYPE_FULL_QUAT];
        while (animPartIndex < v56)
        {
            ...
            tableSize = quat->size;
            *dataShort++ = static_cast<unsigned short>(tableSize);
            const size_t frameBytes = sizeof(short) * 4u * (tableSize + 1u);
            memcpy(randomDataShort, quat->u.frames.u.frames, frameBytes);
            randomDataShort += frameBytes;
            XAnimEmitFrameIndices(
                tableSize,
                &dataByte,
        ...
        while (animPartIndex < v56)
        {
            XAnimPartQuat *const quat = sortedQuat[animPartIndex++].quat;
            iassert(quat);
            memcpy(dataShort, quat->u.frame02, sizeof(quat->u.frame02));
            dataShort += 2;
        }
        v56 += parts->boneCount[PART_TYPE_FULL_QUAT_NO_SIZE];
        while (animPartIndex < v56)
        {
            XAnimPartQuat *const quat = sortedQuat[animPartIndex++].quat;
            iassert(quat);
            memcpy(dataShort, quat->u.frame0, sizeof(quat->u.frame0));
            dataShort += 4;
        }
        ...
        while (animPartIndex < v56)
        {
            XAnimPartTransPtr &entry = sortedTrans[animPartIndex];
            XAnimPartTrans *const trans = entry.trans;
            iassert(trans);
            *dataByte++ = entry.partIndex;
            tableSize = trans->size;
            *dataShort++ = static_cast<unsigned short>(tableSize);
            memcpy(dataInt, trans->u.frames.mins, sizeof(float) * 6u);
            dataInt += 6;
            const size_t frameBytes = 3u * (tableSize + 1u);
            memcpy(randomDataByte, trans->u.frames.frames._1, frameBytes);
            randomDataByte += frameBytes;
            XAnimEmitFrameIndices(
                tableSize,
                &dataByte,
                &dataShort,
                &indices,
                &trans->u.frames.indices,
                useSmallIndices);
            ++animPartIndex;
        }
        v56 += parts->boneCount[PART_TYPE_TRANS];
        while (animPartIndex < v56)
        {
            ...
            *dataShort++ = static_cast<unsigned short>(tableSize);
            memcpy(dataInt, trans->u.frames.mins, sizeof(float) * 6u);
            dataInt += 6;
            const size_t frameBytes = sizeof(short) * 3u * (tableSize + 1u);
            memcpy(randomDataShort, trans->u.frames.frames._2, frameBytes);
            randomDataShort += frameBytes;
            XAnimEmitFrameIndices(
                tableSize,
                &dataByte,
        ...
        while (animPartIndex < v56)
        {
            ...
            iassert(entry.trans);
            *dataByte++ = entry.partIndex;
            memcpy(dataInt, entry.trans->u.frame0, sizeof(entry.trans->u.frame0));
            dataInt += 3;
        }
        v56 += parts->boneCount[PART_TYPE_NO_TRANS];
        while (animPartIndex < v56)
