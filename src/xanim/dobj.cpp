...
/**
 * Byte size of a DObj's models block: numModels XModel pointers followed by one
 * parent-bone byte each, in a single allocation. The x86 build wrote this as
 * 5 * numModels, which only holds while a pointer is four bytes.
 */
static unsigned int DObjGetModelsBlockSize(unsigned int numModels)
{
    return numModels * (sizeof(XModel *) + sizeof(byte));
}
...
void __cdecl DObjCreateDuplicateParts(DObj_s *obj, DObjModel_s *dobjModels, unsigned int numModels)
{
    ...
    iassert(numModels > 0);
    obj->models = (XModel **)MT_Alloc(DObjGetModelsBlockSize(numModels), 13);
...
void __cdecl DObjFree(DObj_s *obj)
{
    ...
    if (models)
    {
        MT_Free((byte*)models, DObjGetModelsBlockSize(obj->numModels));
        obj->models = 0;
    }
...
void __cdecl DObjUnarchive(DObj_s *obj)
{
    ...
    MT_Free((_BYTE *)savedObj.models, DObjGetModelsBlockSize(savedObj.numModels));
...
void DObjClone(const DObj_s *from, DObj_s *obj)
{
    ...
    obj->tree = 0;
    v5 = (XModel **)MT_Alloc(DObjGetModelsBlockSize(from->numModels), 13);
    obj->models = v5;
    memcpy(v5, from->models, DObjGetModelsBlockSize(from->numModels));
}
