...
double __cdecl XModelGetLodOutDist(const XModel *model)
{
    ...
    const int lodCount = XModelGetNumLods(model);
    iassert(lodCount > 0);
    iassert(lodCount <= MAX_LODS);

    // The original x86 expression walked from parentList to the last
    // XModelLodInfo using the 32-bit XModel layout.  Pointer members are twice
    // as wide on LP64, so that arithmetic reads an unrelated field and gives
    // static models a bogus cull distance.  Use the typed member directly.
    return model->lodInfo[lodCount - 1].dist;
}
