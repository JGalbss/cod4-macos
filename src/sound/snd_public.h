...
#endif
// On POSIX/Switch the Miles Sound System is unavailable (proprietary).
// snd_public.h's declarations that take MSS types compile via the
// upstream forward references — those code paths are never executed
// outside Windows. Audio is replaced by an OpenAL-soft layer (TBD).
#include <gfx_d3d/fxprimitives.h>
#include <universal/memfile.h>

// snd

[[maybe_unused]] static const char *snd_eqTypeStrings[6] = { "lowpass", "highpass", "lowshelf", "highshelf", "bell", NULL }; // idb
...
int __cdecl SND_PlayLocalSoundAliasByName(
    ...
    snd_alias_system_t system);
#ifndef _WIN32
// Decode resident aliases before their first latency-sensitive use.  The native
// backend leaves streamed assets alone so map music/ambience are not pulled into
// memory just because a weapon references them indirectly.
...
#ifndef _WIN32
// Releases only sample buffers allocated by the native SND_SetData path.  OAT
// fastfile samples point into zone-owned storage and must not be passed to free().
void SND_FreeData(MssSoundCOD4 *mssSound);
#endif
