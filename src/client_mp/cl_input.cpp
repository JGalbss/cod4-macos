...
void __cdecl CL_WritePacket(int localClientNum)
{
    ...
    // Match the CoD4x client state gate.  In particular, do not let the
    // connection/challenge states consume netchan sequence numbers and
    // outPacket ring entries before connectResponse has set the channel up.
    if (clc->demoplaying || connstate < CA_CONNECTED || connstate == CA_SENDINGSTATS)
    {
    ...
    else
    {
        ...
        if (Cod4x_UseExtendedProtocol())
            MSG_WriteLong(&buf, LocalClientGlobals->serverId);
        else
#endif
...
#ifdef KISAK_COD4X
        if (Cod4x_UseExtendedProtocol())
            MSG_WriteLong(&buf, Cod4x_GetServerConfigDataSequence());
#endif
        ...
        if (count > 32)
        {
            ...
            if (Cod4x_UseExtendedProtocol())
            {
                static int cod4xMaxPacketTraceCount;
                if (cod4xMaxPacketTraceCount++ < 12)
                {
                    const auto &previousPacket = LocalClientGlobals->outPackets[oldPacketNum];
                    Com_Printf(14,
                        "CoD4x: MAX_PACKET_USERCMDS cmd=%d previous=%d slot=%d outseq=%d dup=%d state=%d now=%d sent=%d\n",
                        LocalClientGlobals->cmdNumber, previousPacket.p_cmdNumber, oldPacketNum,
                        clc->netchan.outgoingSequence, cl_packetdup->current.integer,
                        static_cast<int>(connstate), cls.realtime, previousPacket.p_realtime);
                }
            }
            else
#endif
...
#ifdef KISAK_COD4X
        compressionStart = Cod4x_UseExtendedProtocol() ? 16 : 9;
#else
        constexpr int compressionStart = 9;
#endif
        if (buf.cursize < compressionStart)
            MyAssertHandler(".\\client_mp\\cl_input.cpp", 2063, 0, "%s", "buf.cursize >= CL_ENCODE_START");
        v3 = buf.data;
        v4 = (unsigned __int8 *)compressedBuf;
        memcpy(v4, v3, compressionStart);
        ...
        compressedSize = MSG_WriteBitsCompress(
            0,
            (const unsigned __int8 *)buf.data + compressionStart,
            &(*compressedBuf)[compressionStart],
            buf.cursize - compressionStart)
            + compressionStart;
        ...
        if (Cod4x_UseExtendedProtocol())
        {
            static int cod4xPacketTraceCount;
            if (cod4xPacketTraceCount++ < 4)
                Com_Printf(14,
                    "CoD4x: netchan tx qport=%u serverId=%d messageAck=%d commandAck=%d configAck=%d payload=%d\n",
                    static_cast<unsigned int>(static_cast<unsigned short>(clc->qport)),
                    LocalClientGlobals->serverId, clc->serverMessageSequence,
                    clc->serverCommandSequence, Cod4x_GetServerConfigDataSequence(), compressedSize);
        }
#endif
...
bool __cdecl CL_ReadyToSendPacket(int localClientNum)
{
    ...
    if (clientUIActives[0].connectionState != CA_ACTIVE
        ...
        && cls.realtime - clc->lastPacketSentTime
#ifdef KISAK_COD4X
            < (Cod4x_UseExtendedProtocol() ? 50 : 1000))
#else
            < 1000)
#endif
    {
