...
void __cdecl CL_ServerInfoPacket(netadr_t from, msg_t *msg, int time)
{
    ...
    if (
#ifdef KISAK_COD4X
        Cod4x_AcceptsServerProtocol(prot, v8)
#else
        prot == v8
#endif
    )
...
void __cdecl CL_GlobalServers_f()
{
    ...
    {
#ifdef KISAK_COD4X
        char cod4xKeywords[1024]{};
        for (int arg = 3; arg < Cmd_Argc(); ++arg)
        {
            if (cod4xKeywords[0])
                I_strncat(cod4xKeywords, sizeof(cod4xKeywords), " ");
            I_strncat(cod4xKeywords, sizeof(cod4xKeywords), (char *)Cmd_Argv(arg));
        }
        if (Cod4x_QueryMasterServers(cod4xKeywords))
            return;
#endif
