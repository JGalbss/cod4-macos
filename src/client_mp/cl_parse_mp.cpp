...
void __cdecl CL_ParseSnapshot(int localClientNum, msg_t *msg)
{
    ...
    if (msg->overflowed)
    {
        ...
        if (Cod4x_UseExtendedProtocol())
            Com_PrintWarning(14,
                "CoD4x: snapshot %d overflowed at byte=%d bit=%d size=%d\n",
                newSnap.messageNum, msg->readcount, msg->bit, msg->cursize);
#endif
    }
...
END_LOOP:
    ...
    // Stock/local gamestates need the same custom-map IWD mount as the
    // extended CoD4x parser below. Without it the map fastfiles can load while
    // usermaps/<map>/<map>.iwd remains absent from the virtual filesystem.
    const char *serverInfo = CL_GetConfigString(localClientNum, 0);
    FS_AddUserMapDirIWD(Info_ValueForKey(serverInfo, "mapname"));
...
#ifdef KISAK_COD4X
void CL_ParseGamestateCod4x(netsrc_t localClientNum, msg_t *msg)
{
    constexpr int kStockConfigStrings = 2442;
    constexpr int kExtendedConfigStrings = kStockConfigStrings * 2;
    constexpr int kSvcConfigClient = 11;
    ...
    client->gameState.dataCount = 1;
    memset(&extendedGameState, 0, sizeof(extendedGameState));
    extendedGameState.dataCount = 1;

    auto storeConfigString = [&](const int index, const char *value)
    {
        gameState_t *state = &client->gameState;
        int localIndex = index;
        if (index >= kStockConfigStrings)
        {
            state = &extendedGameState;
            localIndex -= kStockConfigStrings;
        }

        const size_t length = strlen(value) + 1;
        if (localIndex < 0 || localIndex >= kStockConfigStrings
            || length > sizeof(state->stringData)
            || state->dataCount > static_cast<int>(sizeof(state->stringData) - length))
        {
            Com_Error(ERR_DROP, "CoD4x: MAX_GAMESTATE_CHARS exceeded at configstring %d", index);
            return;
        }
        state->stringOffsets[localIndex] = state->dataCount;
        memcpy(&state->stringData[state->dataCount], value, length);
        state->dataCount += static_cast<int>(length);
    };

    bool haveConfigStrings = false;
    int baselineCount = 0;
    int configClientCount = 0;
    while (!msg->overflowed)
    {
        const int commandOffset = msg->readcount;
        ...
        case svc_configstring:
        {
            const int count = MSG_ReadLong(msg);
            if (count <= 0 || count > kExtendedConfigStrings)
                Com_Error(ERR_DROP, "CoD4x: invalid configstring count %d", count);
            ...
            for (int item = 0; item < count; ++item)
            {
                ...
                if (index <= previousIndex || index < 0 || index >= kExtendedConfigStrings)
                    Com_Error(ERR_DROP, "CoD4x: invalid configstring index %d", index);

                while (constantConfigStrings[constantIndex].configStringNum
                    && constantConfigStrings[constantIndex].configStringNum < index)
                {
                    storeConfigString(
                        constantConfigStrings[constantIndex].configStringNum,
                        constantConfigStrings[constantIndex].configString);
                    ++constantIndex;
                }
                if (constantConfigStrings[constantIndex].configStringNum == index)
                    ++constantIndex;

                const char *value = MSG_ReadBigString(msg);
                if (msg->overflowed)
                    Com_Error(ERR_DROP, "CoD4x: truncated configstring %d", index);
                storeConfigString(index, value);
                previousIndex = index;
            }
            ...
            haveConfigStrings = true;
            CL_ParseMapCenter(localClientNum);
            Com_Printf(14, "CoD4x: parsed %d configstrings through byte %d\n", count, msg->readcount);
            break;
        }
        case svc_baseline:
        {
            const unsigned int entityNumber = MSG_ReadEntityIndex(msg, 10);
            if (entityNumber >= 1024)
                Com_Error(ERR_DROP, "CoD4x: baseline number out of range: %u", entityNumber);
            entityState_s nullState{};
            MSG_ReadDeltaEntity(msg, 0, &nullState, &client->entityBaselines[entityNumber], entityNumber);
            ++baselineCount;
            break;
        }
        case kSvcConfigClient:
        {
            const int clientNumber = MSG_ReadByte(msg);
            if (clientNumber < 0 || clientNumber >= 64)
                Com_Error(ERR_DROP, "CoD4x: invalid config client %d", clientNumber);
        ...
        default:
            Com_Error(ERR_DROP,
                "CoD4x: bad extended gamestate command %d at byte %d after %d baselines and %d clients",
                command, commandOffset, baselineCount, configClientCount);
            return;
        }
    }

    if (msg->overflowed || !haveConfigStrings)
        Com_Error(ERR_DROP, "CoD4x: truncated gamestate");

    Cod4x_SetServerConfigDataSequence(MSG_ReadLong(msg));
    clc->clientNum = MSG_ReadLong(msg);
    if (clc->clientNum < 0 || clc->clientNum >= 64)
        Com_Error(ERR_DROP, "CoD4x: bad own client number %d", clc->clientNum);
    clc->checksumFeed = MSG_ReadLong(msg);
    if (msg->readcount + 4 <= msg->cursize)
        MSG_ReadLong(msg); // protocol-21 reserved/database checksum feed
