...
#include <cod4x/cod4x_reliable.h>
#endif
...
void __cdecl CL_RequestAuthorization(netsrc_t localClientNum)
{
    ...
    //    if (!NET_StringToAdr(com_authServerName->current.string, &cls.authorizeServer))
    //    {
...
void __cdecl CL_DownloadsComplete(int32_t localClientNum)
{
    ...
    if (!CL_WasMapAlreadyLoaded())
    {
        ...
        CL_StartHunkUsers();
#ifdef KISAK_COD4X
        // Protocol 21 uses this controlled restart in place of the stock
        // nested video restart above.  CoD4x clears the flag here so later
        // map loads do not tear down an already registered world.
        if (Cod4x_UseExtendedProtocol())
            cls.gameDirChanged = 0;
#endif
        SCR_UpdateScreen();
    LABEL_25:
        ...
        || connectionState == CA_SENDINGSTATS)
    {
        ...
        if (!clc->demoplaying)
        {
...
#ifdef KISAK_COD4X
                if (Cod4x_IsEnabled())
                    v2 = va("getchallenge 0 \"%s\"", Cod4x_GetGuid());
                else
#endif
            ...
            case CA_CHALLENGING:
                ...
                if (Cod4x_UseExtendedProtocol())
                {
                    Info_SetValueForKey(dest, "xver", KISAK_COD4X_CLIENT_VERSION);
                    Info_SetValueForKey(dest, "pbguid", Cod4x_GetGuid());
                }
#endif
        ...
        else if (connstate < CA_CONNECTED)
        {
            if (connstate == CA_CHALLENGING)
            {
                ...
                if (NET_CompareBaseAdr(from, clc->serverAddress))
                {
                    ...
                    if (Cod4x_UseExtendedProtocol())
                    {
                        Cod4x_SetServerConfigDataSequence(0);
                        Cod4x_ReliableSetup(localClientNum, localClientNum + g_qport, from);
                    }
#endif
                    if (Cmd_Argc() <= 1)
                        v18 = (char *)"";
    ...
    if (clientUIActives[0].connectionState >= CA_CONNECTED)
    {
        ...
        if (msg->cursize >= 4)
        {
            if (NET_CompareAdr(from, clc->netchan.remoteAddress))
            {
                ...
                if (Cod4x_UseExtendedProtocol()
                    && msg->cursize >= 6
                    && *(uint32_t *)msg->data == 0xfffffff0u)
                {
                    MSG_BeginReading(msg);
                    MSG_ReadLong(msg);
                    MSG_ReadShort(msg); // qport
                    Cod4x_ReliableReceivePacket(msg);
                    return 1;
                }
#endif
                if (Netchan_Process(&clc->netchan, msg))
                {
                    ...
                    if (Cod4x_UseExtendedProtocol())
                    {
                        static int cod4xNetchanReceiveTraceCount;
                        if (cod4xNetchanReceiveTraceCount++ < 40)
                            Com_Printf(14,
                                "CoD4x: netchan rx sequence=%d reliableAck=%d bytes=%d payloadAt=%d state=%d\n",
                                clc->serverMessageSequence, clc->reliableAcknowledge,
                                msg->cursize, msg->readcount,
                                static_cast<int>(clientUIActives[0].connectionState));
                    }
#endif
...
void CL_InitDevGui()
{
    // The commands are registered either way. devgui_aimassist.cfg is exec'd during
    // startup and holds a couple of hundred devgui_dvar lines; with no command by that
    // name the console forwards each one to the server as a reliable command, and the
    // first client packet after that overflows its 2 KB send buffer. The devgui add
    // functions are inert until DevGui_Init has run, so the lines land nowhere.
    ...
    // The menu tree itself is a developer overlay; retail has no equivalent and nothing
    // in the game depends on it. It still carries 32-bit layout assumptions - the handle
    // stride, the intrusive free list through DevMenuItem::label, the sort invariant in
    // DevGui_CompareMenus - and building it costs startup time quadratically, so it is
    // opt-in with KISAK_DEV=1.
    if (!std::getenv("KISAK_DEV"))
        return;
                ...
                if (parentMenuName)
                {
                    if (menuResponse)
                    {
                        ...
                        if (menuIndex == 32)
                            menuIndex = -1;
#ifdef KISAK_COD4X
                        Com_Printf(14, "CoD4x: script menu '%s' resolved to %d, response '%s'\n",
                                   parentMenuName, menuIndex, menuResponse);
                        if (menuIndex < 0)
                        {
                            Com_Printf(14, "CoD4x: active script menus:");
                            for (int32_t configIndex = 0; configIndex < 32; ++configIndex)
                            {
                                const char *const configMenu = CL_GetConfigString(0, configIndex + 1970);
                                if (*configMenu)
                                    Com_Printf(14, " %d='%s'", configIndex, configMenu);
                            }
                            Com_Printf(14, "\n");
                        }
#endif
