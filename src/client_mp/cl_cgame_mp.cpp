...
void __cdecl CL_FirstSnapshot(int32_t localClientNum)
{
    ...
    if ((LocalClientGlobals->snap.snapFlags & 2) == 0)
    {
        ...
        if (Cod4x_UseExtendedProtocol())
            Com_Printf(14, "CoD4x: first snapshot time=%d message=%d; entering active play\n",
                LocalClientGlobals->snap.serverTime, LocalClientGlobals->snap.messageNum);
#endif
        CG_RegisterSounds();
        clc = CL_GetLocalClientConnection(localClientNum);
        if (localClientNum)
        ...
        if (cl_activeAction->current.string[0])
        {
