    ...
    while (*text)
    {
        ...
        else
        {
            ...
            if (!text || *text == 94 || *text < 48 || *text > 57)
            {
                ...
                if (con.lineOffset + CONTXTCMD_TOTAL_HUDICON > sizeof(con.textTempLine))
                    MyAssertHandler(
                        ".\\client\\cl_console.cpp",
                        1236,
                        0,
                        ...
                        con.lineOffset,
                        static_cast<int>(sizeof(con.textTempLine) - CONTXTCMD_TOTAL_HUDICON + 1));
                Material *iconMaterial = nullptr;
                memcpy(&iconMaterial, text + CONTXTCMD_ARG_HUDICON_MATERIAL, sizeof(iconMaterial));
                ...
                memcpy(v9, text, CONTXTCMD_LEN_HUDICON);
                con.lineOffset += CONTXTCMD_LEN_HUDICON;
                text += CONTXTCMD_LEN_HUDICON;
                if (con.lineOffset >= 0x201)
                    MyAssertHandler(
                        ".\\client\\cl_console.cpp",
                        1244,
...
{
    // These were indices into con.color with strides baked for the x86 layout of
    // Console: color sits last, so the decompiler expressed every window as a negative
    // float offset from it, and 4630 is sizeof(MessageBuffer) in floats. Every one of
    // those numbers moves at LP64, where the pointers inside MessageWindow grow, so
    // they produced addresses like 0x40000000001. Name the members instead.
    switch (dest)
    {
    ...
    while (idx < line->textBufSize)
    {
        ...
        if (letter == 94)
        {
            ...
            if (c[0] != 94 && c[0] >= 48 && c[0] <= 57)
                ++idx;
            else if (c[0] == CONTXTCMD_TYPE_HUDICON || c[0] == CONTXTCMD_TYPE_HUDICON_FLIP)
                idx += CONTXTCMD_LEN_HUDICON;
        }
    }
    return printedCnt;
}
    ...
    bool horzFlipIcon)
{
    ...
    if (deathMsgLend + sizeof(iconShader) > deathMsgMaxLen)
        MyAssertHandler(".\\client\\cl_console.cpp", 1449, 0, "%s", "deathMsgLen + sizeof( iconShader ) <= deathMsgMaxLen");
    memcpy(&deathMsg[deathMsgLend], &iconShader, sizeof(iconShader));
    deathMsgLene = deathMsgLend + sizeof(iconShader);
    if (deathMsgLene - deathMsgLen != CONTXTCMD_TOTAL_HUDICON)
        MyAssertHandler(
            ".\\client\\cl_console.cpp",
            1455,
            0,
    ...
    msgwnd_mode_t mode)
{
    ...
    if (!cg_paused->current.integer)
    {
        ...
        Con_DrawMessageWindow(
            ...
            xPos,
            yPos,
            SnapFloatToInt(fontScale * 48.0f),            horzAlign,
            vertAlign,
            font,
            color,
            textStyle,
...
bool __cdecl Con_IsGameMessageWindowActive(int32_t localClientNum, uint32_t windowIndex)
{
    if (windowIndex >= 4)
        MyAssertHandler(
            ...
            windowIndex,
            4);
    // Float offset 11 into a MessageWindow is activeLineCount: the three leading
    // pointers are 4 bytes each on x86, so the field indices shift at LP64.
