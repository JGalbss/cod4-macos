...
void __cdecl CL_DestroyDevGui()
{
    ...
    // Tearing down a devgui that was never created is a no-op, not a fault: the whole
    // subsystem is opt-in (KISAK_DEV), so on a normal run there is nothing to destroy.
    if (!clGuiGlob.inited)
        return;
    clGuiGlob.inited = 0;
