...
void __cdecl PMem_FreeIndex(PhysicalMemoryPrim *prim, unsigned int allocIndex)
{
    ...
    if (allocIndex == prim->allocListCount - 1)
    {
        ...
        do
        {
            ...
            if (!--prim->allocListCount)
                break;
            // Was pointer arithmetic from &allocListCount scaled by 2 ints per entry,
            // which is sizeof(PhysicalMemoryAllocation) on x86. The struct holds a
            // pointer so it is 16 bytes at LP64, and the expression only lands on the
            // right entry when the count is 1 - past that the walk read between
            // entries and reported a false "memory hole" when freeing a zone.
    ...
    else
    {
        ...
        // Only a region with a length can leave a hole. Zones loaded through
        // OpenAssetTools own their memory outside PMem, so they bracket a
        // BeginAlloc/EndAlloc pair that reserves nothing - freeing one out of order
        // moves no boundary and strands nothing.
        if (v2 != 0)
        {
