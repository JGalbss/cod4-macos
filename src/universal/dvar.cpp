...
// bit is passed by address to a callback that reads it as unsigned int. As a char
// that read picks up three bytes of neighbouring stack, so dvars whose flags
// happened to overlap the garbage were pulled into the info string and overflowed
// it - r_outdoorFeather and friends in a serverinfo that should only hold bit 4.
...
void Dvar_Sort()
{
    if (InterlockedCompareExchange<LONG>(&isSortingDvars, 1, 0))
    {
    ...
    case 6u:
        if ((value.integer < 0 || value.integer >= dvar->domain.enumeration.stringCount) && value.integer)
            MyAssertHandler(
                ...
                value.integer);
        if (dvar->domain.enumeration.stringCount)
            result = dvar->domain.enumeration.strings[value.integer];   // see Dvar_RegisterEnum
        else
            result = "";
        break;
    case 7u:
        // Testing .integer here only checks the low 32 bits of the string pointer; a
        // perfectly valid pointer whose low half happens to be zero would assert.
        if (!value.string)
        ...
        if (charsWritten >= 0)
        {
            ...
            for (stringIndex = 0; stringIndex < domain.enumeration.stringCount; ++stringIndex)
            {
                charsWrittena = _snprintf(
                    ...
                    "\n  %2i: %s",
                    stringIndex,
                    domain.enumeration.strings[stringIndex]);   // see Dvar_StringToEnum
                if (charsWrittena < 0)
                    break;
                if (outLineCount)
                    ++*outLineCount;
...
void __cdecl Dvar_UpdateEnumDomain(dvar_s *dvar, const char **stringTable)
{
    ...
    // LP64: the string table is a pointer and integer.max is an int, so this truncated it
    // and every later read through enumeration.strings faulted. The union has the correctly
    // typed member - use it.
...
void __cdecl Dvar_SetVariant(dvar_s *dvar, DvarValue value, DvarSetSource source)
{
    const char *v3; // eax
    char *v4; // eax
    (void)v3; (void)v4; // hex-rays scratches; unread
    const char *v5; // eax
    const char *v6; // eax
...
int __cdecl Dvar_StringToEnum(const DvarLimits *domain, const char *string)
{
    ...
    for (stringIndex = 0; stringIndex < domain->enumeration.stringCount; ++stringIndex)
    {
        // LP64: this reached the string array through domain->integer.max - an int, so
        // the pointer was truncated - and stepped 4 bytes per entry. The union already
        // carries the array as `enumeration.strings`; index it directly and let the
        // compiler use the right 8-byte stride.
    ...
    {
        if (!I_strnicmp(string, domain->enumeration.strings[stringIndexb], v3))   // see above
            return stringIndexb;
    }
    return -1337;
}
    ...
    const char *description)
{
    ...
    // LP64: on x86, DvarLimits_Enumeration::strings and DvarLimits_Integer::max both sat at
    // offset 4, so writing the table through integer.max happened to work. At LP64 `strings`
    // moves to offset 8 and no longer aliases max, and the int cast truncates the pointer
    // besides - so the enum table came back null and Dvar_StringToEnum faulted while
    // registering the renderer dvars. Write the correctly typed member.
...
void __cdecl Dvar_SetDomainFunc(dvar_s *dvar, bool(__cdecl *customFunc)(dvar_s *, DvarValue))
{
    ...
    if (customFunc)
    {
        // LP64 ABI: the decompiler rendered "pass the 16-byte DvarValue by value" as four
        // separate 32-bit arguments, because that is how x86 cdecl lays a union out on the
        // stack. Calling through that cast on AArch64 puts the string pointer's low half in
        // one register and garbage in the next, so the callee dereferences a truncated
        // address (FS_GameDirDomainFunc faulted at 0x1a46f94). domainFunc already has the
        // correct type - call it directly and let the compiler pass the union properly.
