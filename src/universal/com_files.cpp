...
#include <io.h>
#else
#include <unistd.h>     // POSIX equivalent: read/write/close, lseek, access
#include <sys/stat.h>
#endif
...
{
    [[maybe_unused]] DWORD oldAttributes; // [esp+0h] [ebp-10Ch]
    char ospath[256]; // [esp+4h] [ebp-108h] BYREF
    [[maybe_unused]] unsigned int attributes; // [esp+108h] [ebp-4h]
...
#else
        // POSIX: chmod existing file to read+write owner so open-for-write
        // succeeds even when the previous owner left it read-only.
        (void)attributes;  // unused on POSIX
        chmod(ospath, 0644);
#endif
...
int __cdecl FS_CreatePath(char *OSPath)
{
    ...
    {
        // Split on either separator. The original only looked for '\\', so on POSIX -
        // where every path uses '/' - it walked the whole string, found nothing, and
        // created no directories at all. Every write the game makes goes through here,
        // so nothing it saved was ever persisted: no players/, no profiles/, and so
        // Com_IsValidPlayerProfileDir could never find the profile the menu had just
        // created.
...
unsigned int __cdecl FS_FOpenFileReadForThread(const char *filename, int *file, FsThread thread)
{
    ...
    *file = FS_HandleForFile(thread);
    ...
    for (search = fs_searchpaths; ; search = search->next)
    {
        if (!search)
        {
            if (fs_debug->current.integer == 1 && thread == FS_THREAD_MAIN)
    ...
    if (!iwd->referenced && !FS_FilesAreLoadedGlobally(sanitizedName))
        iwd->referenced = 1;
    if (InterlockedCompareExchange(&iwd->hasOpenFile, 1u, 0u) == 1)
    {
    ...
    // Mode 2 is the in-match disk-access sentinel installed by cgame after its
    // initial snapshot.  It drives com_fileAccessed/statmon; it is not verbose
    // tracing.  Treating every non-zero mode as verbose made each lazily loaded
    // texture synchronously format and flush a console line in live gameplay.
    if (fs_debug->current.integer == 1 && thread == FS_THREAD_MAIN)
...
bool __cdecl FS_GameDirDomainFunc(dvar_s *dvar, DvarValue newValue)
{
    ...
    // LP64: this read the string through newValue.integer, the 4-byte member of the
    // DvarValue union. On x86 that aliased the pointer exactly; here it truncates it to
    // its low 32 bits and the first dereference faults (observed: 0x1a46f94). The union
    // already has a `string` member of the right width - use it, and keep the strstr
    // results as pointers rather than round-tripping them through int.
    if (!dvar)
        MyAssertHandler(".\\universal\\com_files.cpp", 4241, 0, "%s", "dvar");
    const char *val = newValue.string;
    if (!val || !*val)
        return 1;
    if (I_strnicmp(val, "mods", 4))
        return 0;
    if (strlen(val) < 6 || (val[4] != '/' && val[4] != '\\'))
        return 0;
    result = 0;
    if (!strstr(val, ".."))
    {
...
void FS_RegisterDvars()
{
    char *v1; // eax
    char *v2; // eax
    [[maybe_unused]] const dvar_s *result; // eax
...
void __cdecl FS_AddGameDirectory(char *path, char *dir, int bLanguageDirectory, int iLanguage)
{
    ...
    if (bLanguageDirectory)
    {
        ...
        ospath[&ospath[strlen(ospath) + 1] - &ospath[1] - 1] = 0;
        // Sys_DirectoryHasContents: Win32-specific empty-dir check. POSIX
        // doesn't have it; assume the dir has contents (worst case: we add
        // an empty search path, which is harmless).
...
void __cdecl FS_Startup(char *gameName)
{
    ...
    if ((fs_homepath->current.string && *fs_homepath->current.string) && I_stricmp(fs_basepath->current.string, fs_homepath->current.string))
    {
        ...
        // Profiles and mpdata are written below fs_homepath/players.  The
        // original startup only searched basepath/players, so a separate
        // writable home directory silently reloaded the retail installation's
        // stale stats on the next launch.  Add the home players directory
        // after basepath so it has the same read priority as our writes.
        FS_AddLocalizedGameDirectory((char *)fs_homepath->current.string, (char*)"players");
    }
