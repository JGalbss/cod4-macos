...
// `long` is 32-bit on Windows but 64-bit on macOS arm64. This name is part of
// the engine's fixed-width vocabulary, so keep its actual width portable.
...
typedef uint64_t __attribute__((__may_alias__)) _QWORD_alias;
