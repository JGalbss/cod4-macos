...
void __cdecl Hunk_AddAsset(XAssetHeader header, _DWORD *data)
{
    ...
    *(XAssetHeader *)(uintptr_t)(data[2] + 4 * (*data)++) = header;
}
...
void FreeString(const char* str)
{
	...
	// A string that is not interned has no reference to drop, and id 0 is not a
	// valid node - SL_RemoveRefToString(0) would decrement a refcount at the very
	// start of the memory tree. Callers reach here with a stale pointer, which is
	// its own bug, but turning that into a wild write makes it far harder to find.
	if (!out)
	{
		Com_PrintWarning(0, "WARNING: FreeString: '%s' is not an interned string\n", str);
		return;
	}
...
{
    // This backs XAnim_s, whose entries hold an XAnimParts*, so four is no longer
    // enough to align what gets stored here.
    return Hunk_AllocAlign(size, alignof(XAnim_s), "XAnimPrecache", 11);
}
...
char* __cdecl Z_TryVirtualAlloc(int32_t size, const char* name, int32_t type)
{
    ...
    if (buf)
        track_z_commit((size + 4095) & ~static_cast<uintptr_t>(0xFFF), type);
    return buf;
}
...
void __cdecl Hunk_ClearToMarkLow(int32_t mark)
{
    ...
    endBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[hunk_low.temp + 4095] & ~static_cast<uintptr_t>(0xFFF));
    hunk_low.temp = mark;
    hunk_low.permanent = mark;
    Hunk_ClearData();
    beginBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[hunk_low.temp + 4095] & ~static_cast<uintptr_t>(0xFFF));
    if (endBuf != beginBuf)
...
uint8_t* __cdecl Hunk_AllocAlign(uint32_t size, int32_t alignment, const char* name, int32_t type)
{
    ...
    endBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.permanent] & ~static_cast<uintptr_t>(0xFFF));
    ...
    // Commit the pages between the aligned allocation and where the hunk previously
    // reached. The decompiler expressed the length as pointer arithmetic through an
    // (int) cast of the address itself, which truncates at LP64 and produced a
    // negative size.
    {
        uint8_t *const alignedBuf = (uint8_t *)((uintptr_t)buf & ~static_cast<uintptr_t>(0xFFF));
        if (endBuf != alignedBuf)
            Z_VirtualCommit(alignedBuf, static_cast<int>(endBuf - alignedBuf));
    }
...
// Returns a pointer, not the unsigned int upstream declared. `buf` holds
// &s_hunkData[...], and the hunk lives well above 4 GB here, so returning it as 32
// bits handed every caller the low half of a real address. They all bridge back
// through (uintptr_t), which faithfully widened the truncation - the script compiler
// wrote its block table through one and faulted while loading a map.
void* __cdecl Hunk_AllocateTempMemoryHigh(int32_t size, const char* name)
{
    ...
    endBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.temp] & ~static_cast<uintptr_t>(0xFFF));
    hunk_high.temp += size;
    ...
    if ((((_BYTE)s_hunkTotal + (_BYTE)(uintptr_t)s_hunkData - LOBYTE(hunk_high.temp)) & 0xF) != 0)
        MyAssertHandler(".\\universal\\com_memory.cpp", 2074, 0, "%s", "!(((psize_int)buf) & 15)");
    {
        uint8_t *const alignedBuf = (uint8_t *)(buf & ~static_cast<uintptr_t>(0xFFF));
        if (endBuf != alignedBuf)
            Z_VirtualCommit(alignedBuf, static_cast<int>(endBuf - alignedBuf));
    }
...
void Hunk_ClearTempMemoryHigh()
{
    ...
    beginBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.temp] & ~static_cast<uintptr_t>(0xFFF));
    hunk_high.temp = hunk_high.permanent;
    commitSize = ((uintptr_t)&s_hunkData[s_hunkTotal - hunk_high.permanent] & ~static_cast<uintptr_t>(0xFFF)) - (uintptr_t)beginBuf;
    if (commitSize)
...
uint8_t* __cdecl Hunk_AllocLowAlign(uint32_t size, int32_t alignment, const char* name, int32_t type)
{
    ...
    beginBuf = (uint8_t *)(uintptr_t)((uintptr_t)&s_hunkData[hunk_low.permanent + 4095] & ~static_cast<uintptr_t>(0xFFF));
    ...
    commitSize = ((uintptr_t)&s_hunkData[hunk_low.permanent + 4095] & ~static_cast<uintptr_t>(0xFFF)) - (uintptr_t)beginBuf;
    if (commitSize)
...
uint32_t* __cdecl Hunk_AllocateTempMemory(int32_t size, const char* name)
{
    ...
    bufa = buf + 16;
    if (((uint8_t)(uintptr_t)bufa & 0xF) != 0)
        MyAssertHandler(".\\universal\\com_memory.cpp", 2303, 0, "%s", "!(((psize_int)buf) & 15)");
    commitSize = ((uintptr_t)&s_hunkData[hunk_low.temp + 4095] & ~static_cast<uintptr_t>(0xFFF)) - (uintptr_t)beginBuf;
    if (commitSize)
        Z_VirtualCommit(beginBuf, commitSize);
...
void __cdecl Hunk_UserReset(HunkUser* user)
{
    ...
    // Give back every page above the user's inline buffer. The decompiler reached
    // that boundary as `&user[114].name + 3` - an offset built from the 32-bit
    // sizeof(HunkUser) - and then subtracted addresses through (int), so at LP64 both
    // the base and the length came out wrong and Z_VirtualDecommit saw a negative
    // size. buf is the flexible array at the end of the struct, so say that.
    const uintptr_t resetTop = ((uintptr_t)user->buf + 4095u) & ~static_cast<uintptr_t>(0xFFF);
    const uintptr_t currentTop = (user->pos + 4095u) & ~static_cast<uintptr_t>(0xFFF);
    if (resetTop != currentTop)
    {
        if (currentTop <= resetTop)
            MyAssertHandler(".\\universal\\com_memory.cpp", 3019, 0, "%s", "user->pos - pos > 0");
        Z_VirtualDecommit((void *)resetTop, static_cast<int>(currentTop - resetTop));
    }
    user->pos = (uintptr_t)user->buf;
...
[[maybe_unused]] static void __cdecl Z_MallocFailed(int32_t size)
{
