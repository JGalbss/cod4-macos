...
void __cdecl DynEntCl_InitEntities(int32_t localClientNum)
{
    ...
    {
        if (std::getenv("KISAK_PHYS_TRACE"))
        {
            Com_Printf(
                20,
                "[physics] dynent init models=%u brushes=%u def-size=%zu pose-size=%zu client-size=%zu\n",
                static_cast<unsigned int>(DynEnt_GetEntityCount(DYNENT_COLL_CLIENT_MODEL)),
                static_cast<unsigned int>(DynEnt_GetEntityCount(DYNENT_COLL_CLIENT_BRUSH)),
                sizeof(DynEntityDef),
                sizeof(DynEntityPose),
                sizeof(DynEntityClient));
        }
        ...
        for (dynEntId = 0; dynEntId < (int)dynEntCount; ++dynEntId)
        {
            ...
            if ((dynEntClient->flags & 1) != 0 && dynEntClient->physObjId)
            {
                ...
                Phys_ObjGetInterpolatedState(
                    PHYS_WORLD_DYNENT,
                    Phys_ObjFromId(dynEntClient->physObjId),
                    origin,
                    dynEntPose->pose.quat);
                if (Phys_ObjIsAsleep(Phys_ObjFromId(dynEntClient->physObjId)))
                {
                    Phys_ObjDestroy(PHYS_WORLD_DYNENT, Phys_ObjFromId(dynEntClient->physObjId));
                    dynEntClient->physObjId = 0;
                }
...
void __cdecl DynEntCl_Shutdown(int32_t localClientNum)
{
    ...
    if (CL_GetLocalClientActiveCount() && localClientNum == RETURN_ZERO32())
    {
        ...
        for (dynEntIda = 0; dynEntIda < (int)dynEntCounta; ++dynEntIda)
        {
            ...
            {
                Phys_ObjDestroy(PHYS_WORLD_DYNENT, Phys_ObjFromId(dynEntClienta->physObjId));
    ...
    if (trace->hitType == TRACE_HITTYPE_ENTITY && DynEntCl_EventNeedsProcessed(localClientNum, sourceEntityNum))
    {
        ...
        if (cent->pose.physObjId != -1 && cent->pose.physObjId)
        {
            ...
            Phys_ObjBulletImpact(
                PHYS_WORLD_DYNENT,
                Phys_ObjFromId(cent->pose.physObjId),
                hitPos,
                hitDir,
    ...
    bool isMelee)
{
    ...
    if (DynEnt_GetEntityProps(dynEntDef->type)->usePhysics)
    {
        ...
        if (!dynEntClient->physObjId)
        {
            ...
            dynEntClient->physObjId = Phys_ObjToId(PhysObj);
        }
        if (dynEntClient->physObjId)
            Phys_ObjBulletImpact(
                PHYS_WORLD_DYNENT,
            ...
            for (i = 0; ; ++i)
            {
                ...
                if (outerRadiusSqr > (double)CylindricalRadiusDistSqr)
                {
                    ...
                    if (DynEnt_GetEntityProps(dynEntDef->type)->usePhysics)
                    {
                        ...
                        if (dynEntClient->physObjId)
                        {
                            Phys_ObjGetCenterOfMass(Phys_ObjFromId(dynEntClient->physObjId), outPosition);
                            v10 = flrand(-1.0, 1.0);
                            ...
                            Phys_ObjAddForce(PHYS_WORLD_DYNENT, Phys_ObjFromId(dynEntClient->physObjId), outPosition, result);
                        }
                    }
                    if (DynEnt_GetEntityProps(dynEntDef->type)->destroyable)
                    {
        ...
        for (drawType = DYNENT_DRAW_MODEL; (uint32_t)drawType < DYNENT_DRAW_COUNT; ++drawType)
        {
            ...
            for (i = 0; i < (int)unsignedInt; ++i)
            {
                ...
                if (DynEnt_GetEntityProps(dynEntDef->type)->usePhysics && !ClientEntity->physObjId)
                {
                    ...
                    ClientEntity->physObjId = Phys_ObjToId(PhysObj);
                }
            }
        }
            ...
            if (ClosestEntities)
            {
                ...
                do
                {
                    ...
                    if (DynEnt_GetEntityProps(EntityDef->type)->usePhysics && !ClientEntity->physObjId)
                    {
                        ...
                        ClientEntity->physObjId = Phys_ObjToId(DynEntCl_CreatePhysObj(EntityDef, &ClientPose->pose));
                    }
                    --v8;
                    ++v7;
                } while (v8);
