...
char __cdecl DynEnt_Create(DynEntityDef *dynEntDef, const DynEntityCreateParams *params)
{
    ...
    Com_Memset(reinterpret_cast<uint32_t *>(dynEntDef), 0, sizeof(*dynEntDef));
...
int32_t __cdecl DynEnt_CompareEntities(const void *lhs, const void *rhs)
{
    ...
    const auto *arg0 = static_cast<const DynEntityDef *>(lhs);
    const auto *arg1 = static_cast<const DynEntityDef *>(rhs);
    ...
    if (static_cast<uint32_t>(arg0->type) >= DYNENT_TYPE_COUNT)
        MyAssertHandler(
            ".\\DynEntity\\DynEntity_load_obj.cpp",
            367,
            0,
            ...
            arg0->type,
            3);
    if (static_cast<uint32_t>(arg1->type) >= DYNENT_TYPE_COUNT)
        MyAssertHandler(
            ".\\DynEntity\\DynEntity_load_obj.cpp",
            368,
            0,
            ...
            arg1->type,
            3);
    hasModel0 = arg0->xModel != nullptr;
    hasModel1 = arg1->xModel != nullptr;
    ...
    else
    {
        ...
        return static_cast<int32_t>(arg0->brushModel) - static_cast<int32_t>(arg1->brushModel);
    }
}
...
void __cdecl DynEnt_LoadEntities()
{
    ...
    DynEntityDef *dynEntDefList; // [esp+6Ch] [ebp-268h]
    ...
    {
        dynEntDefList = reinterpret_cast<DynEntityDef *>(
            Hunk_Alloc(sizeof(DynEntityDef) * dynEntStringCount, "DynEnt_LoadDefs", 9));
        dynEntCount = 0;
        ...
        while (1)
        {
            ...
            if (isDynEnt)
            {
                ...
                if (DynEnt_Create(&dynEntDefList[dynEntCount], &params))
                    ++dynEntCount;
            }
        }
        if (dynEntCount)
        {
            qsort(dynEntDefList, dynEntCount, sizeof(*dynEntDefList), DynEnt_CompareEntities);
            for (dynEntIndex = 0; dynEntIndex < dynEntCount; ++dynEntIndex)
            {
                dynEntDef = &dynEntDefList[dynEntIndex];
                if (dynEntDef->xModel)
                {
                    drawType = 0;
                }
            ...
            for (drawTypea = 0; drawTypea < 2; ++drawTypea)
            {
                ...
                {
                    cm.dynEntPoseList[drawTypea] = reinterpret_cast<DynEntityPose *>(
                        DynEnt_Alloc(cm.dynEntCount[drawTypea], sizeof(DynEntityPose)));
                    cm.dynEntClientList[drawTypea] =
                        reinterpret_cast<DynEntityClient *>(DynEnt_Alloc(cm.dynEntCount[drawTypea], sizeof(DynEntityClient)));
                    cm.dynEntCollList[drawTypea] = reinterpret_cast<DynEntityColl *>(
                        DynEnt_Alloc(cm.dynEntCount[drawTypea], sizeof(DynEntityColl)));
                }
            }
        }
    }
...
void DynEnt_SaveEntities(MemoryFile *memFile)
{
    ...
    dynEntPoseList = cm.dynEntPoseList;
    do
    {
        MemFile_WriteData(memFile, 2, dynEntCount);
        if (*dynEntCount)
        {
            MemFile_WriteData(memFile, sizeof(DynEntityPose) * *dynEntCount, *dynEntPoseList);
            MemFile_WriteData(memFile, sizeof(DynEntityClient) * *dynEntCount, *dynEntClientList);
            if (*dynEntCount)
            {
                v5 = 0;
                do
                {
                    ...
                    if (v6)
                        Phys_ObjSave(Phys_ObjFromId((*dynEntClientList)[v5].physObjId), memFile);
