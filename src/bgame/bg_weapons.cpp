...
double __cdecl BG_GetSurfacePenetrationDepth(const WeaponDef *weapDef, uint32_t surfaceType)
{
    ...
    // Malformed trace data must not turn a weapon impact into an arbitrary
    // table read.  Keep the original assertions for debug builds, but also
    // enforce the bounds in release builds where bcassert compiles away.
    if (weapDef->penetrateType <= PENETRATE_TYPE_NONE
        || weapDef->penetrateType >= PENETRATE_TYPE_COUNT
        || surfaceType == 0
        || surfaceType >= SURF_TYPECOUNT)
    {
        return 0.0;
    }
...
void __cdecl PM_Weapon_FinishWeaponChange(pmove_t *pm, bool quick)
{
    ...
    WeaponDef *weapDef; // [esp+2Ch] [ebp-10h]
...
void __cdecl BG_WeaponFireRecoil(const playerState_s *ps, float *vGunSpeed, float *kickAVel)
{
    ...
    *vGunSpeed = *vGunSpeed + fPitchKickc;
    ...
    static unsigned int recoilTraceCount = 0;
    if (std::getenv("KISAK_RECOIL_TRACE") && recoilTraceCount++ < 64)
    {
        Com_Printf(
            8,
            "[recoil] '%s' ads=%.3f reduced=%.3f "
            "viewHipP=(%.3f %.3f) viewHipY=(%.3f %.3f) "
            "viewAdsP=(%.3f %.3f) viewAdsY=(%.3f %.3f) "
            "gunHipP=(%.3f %.3f) gunHipY=(%.3f %.3f) "
            "gunAdsP=(%.3f %.3f) gunAdsY=(%.3f %.3f) "
            "gunHipMotion=(%.3f %.3f %.3f %.3f) "
            "gunAdsMotion=(%.3f %.3f %.3f %.3f) "
            "sampleView=(%.3f %.3f) sampleGun=(%.3f %.3f) "
            "resultGunSpeed=(%.3f %.3f) resultKickVel=(%.3f %.3f %.3f)\n",
            ...
            weapDef->fAdsViewKickPitchMax,
            weapDef->fAdsViewKickYawMin,
            weapDef->fAdsViewKickYawMax,
            ...
            weapDef->fAdsGunKickPitchMax,
            weapDef->fAdsGunKickYawMin,
            weapDef->fAdsGunKickYawMax,
            weapDef->fHipGunKickAccel,
            weapDef->fHipGunKickSpeedMax,
            weapDef->fHipGunKickSpeedDecay,
            weapDef->fHipGunKickStaticDecay,
            weapDef->fAdsGunKickAccel,
            weapDef->fAdsGunKickSpeedMax,
            weapDef->fAdsGunKickSpeedDecay,
            weapDef->fAdsGunKickStaticDecay,
            fPitchKickb,
            fYawKickb,
            fPitchKickc,
            fYawKickc,
    ...
    float fGunKickStaticDecay)
{
    ...
    const float inputOffset = *fOffset;
    const float inputSpeed = *speed;
    ...
    else
    {
        ...
        *speed = 0.0;
        ...
        static unsigned int settledTraceCount = 0;
        if (std::getenv("KISAK_RECOIL_TRACE")
            && (inputOffset != 0.0f || inputSpeed != 0.0f)
            && settledTraceCount++ < 128)
        {
            Com_Printf(
                8,
                "[recoil-step] dt=%.6f cap=%.3f accel=%.3f vmax=%.3f decay=%.3f static=%.3f "
                "in=(%.6f %.6f) out=(0 0) settled=1\n",
                fTimeStep,
                fOfsCap,
                fGunKickAccel,
                fGunKickSpeedMax,
    ...
    static unsigned int activeTraceCount = 0;
    if (std::getenv("KISAK_RECOIL_TRACE")
        && (inputOffset != 0.0f || inputSpeed != 0.0f)
        && activeTraceCount++ < 256)
    {
        Com_Printf(
            8,
            "[recoil-step] dt=%.6f cap=%.3f accel=%.3f vmax=%.3f decay=%.3f static=%.3f "
            "in=(%.6f %.6f) out=(%.6f %.6f) settled=0\n",
            fTimeStep,
            fOfsCap,
            fGunKickAccel,
            fGunKickSpeedMax,
