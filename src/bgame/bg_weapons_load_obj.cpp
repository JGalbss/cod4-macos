...
static char g_playerAnimTypeNamePool[64 * 64];
static size_t g_playerAnimTypeNameBytes;
...
void __cdecl BG_LoadPlayerAnimTypes()
{
    ...
    while (1)
    {
        ...
        // These names outlive the hunk they used to come from. The server loads them
        // during G_InitGame and the client only reads them back in BG_InitWeaponStrings
        // while parsing playeranim.script, by which point its own map load has recycled
        // that hunk and every name reads as garbage.
        if (g_playerAnimTypeNameBytes + strlen(token) + 1 > sizeof(g_playerAnimTypeNamePool))
            Com_Error(ERR_DROP, "Player anim type name pool exceeded");
        g_playerAnimTypeNames[g_playerAnimTypeNamesCount] = &g_playerAnimTypeNamePool[g_playerAnimTypeNameBytes];
        g_playerAnimTypeNameBytes += strlen(token) + 1;
        v2 = token;
...
// weaponDefFields holds x86 byte offsets, which name different members once
// pointers are 8 bytes wide. Translate them the first time anything reads one -
// both users of the table, this and ParseConfigStringToStructCustomSize, come
// through here first.
static void TranslateWeaponDefFieldsOnce()
{
    static bool hasTranslated = false;
    if (hasTranslated)
        return;
    hasTranslated = true;
    Com_RemapFieldOffsets(weaponDefFields, ARRAYSIZE(weaponDefFields), kWeaponDefLayout,
                          ARRAYSIZE(kWeaponDefLayout), "weaponDefFields");
}
...
snd_alias_list_t **__cdecl BG_RegisterSurfaceTypeSounds(const char *surfaceSoundBase)
{
    ...
    // Retail x86 used 29 * 4 bytes here.  On LP64 this is an array of 29
    // pointers, so retaining the literal size lets the following base-name
    // allocation overwrite entries 14..28 (and grenade bounces eventually
    // treat ASCII such as "WPN_FRAGGRENADE" as a snd_alias_list_t pointer).
    result = reinterpret_cast<snd_alias_list_t **>(
        Hunk_AllocLow(sizeof(*result) * 29u, "BG_RegisterSurfaceTypeSounds", 15));
...
WeaponDef *__cdecl BG_LoadWeaponDefInternal(const char *one, const char *two)
{
    ...
    // 0x878 is the x86 sizeof; the struct is larger here, and the shortfall was
    // being written over whatever the hunk handed out next.
