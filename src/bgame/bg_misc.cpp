...
void __cdecl BG_EvaluateTrajectoryDelta(const trajectory_t *tr, int32_t atTime, float *result)
{
    ...
    case TR_ACCELERATE:
        ...
        // trDelta is the velocity reached at the end of the acceleration
        // interval.  Differentiate the matching position equation above:
        // v(t) = trDelta * t / duration.  The recovered code multiplied by
        // t^2, which is neither the derivative nor dimensionally a velocity.
        scale = deltaTimeb / ((double)tr->trDuration * EQUAL_EPSILON);
    ...
    case TR_DECELERATE:
        ...
        {
            deltaTimec = (double)(atTime - tr->trTime) * EQUAL_EPSILON;
            // Differentiate base + trDelta*t - 0.5*a*t^2.  Deceleration must
            // begin at trDelta and reach zero at trDuration; the old t factor
            // instead started at zero and sped the entity up.
            scale = 1.0f - deltaTimec / ((double)tr->trDuration * EQUAL_EPSILON);
            Vec3Scale(tr->trDelta, scale, result);
        LABEL_22:
            iassert(!IS_NAN((tr->trBase)[0]) && !IS_NAN((tr->trBase)[1]) && !IS_NAN((tr->trBase)[2]));
            iassert(!IS_NAN((tr->trDelta)[0]) && !IS_NAN((tr->trDelta)[1]) && !IS_NAN((tr->trDelta)[2]));
    ...
    float prone_feet_dist)
{
    ...
    [[maybe_unused]] float fTraceRealHeight; // [esp+D4h] [ebp-38h]
...
void __cdecl BG_SetShellShockParmsFromDvars(shellshock_parms_t *parms)
{
    ...
    [[maybe_unused]] float v7; // [esp+20h] [ebp-D4h]
    float v8; // [esp+30h] [ebp-C4h]
    float v9; // [esp+34h] [ebp-C0h]
    [[maybe_unused]] float v10; // [esp+3Ch] [ebp-B8h]
    [[maybe_unused]] float v11; // [esp+50h] [ebp-A4h]
    [[maybe_unused]] float v12; // [esp+64h] [ebp-90h]
    [[maybe_unused]] float v13; // [esp+78h] [ebp-7Ch]
    [[maybe_unused]] float v14; // [esp+8Ch] [ebp-68h]
    float value; // [esp+9Ch] [ebp-58h]
    [[maybe_unused]] float v16; // [esp+A4h] [ebp-50h]
    [[maybe_unused]] float v17; // [esp+B8h] [ebp-3Ch]
    [[maybe_unused]] float v18; // [esp+CCh] [ebp-28h]
    [[maybe_unused]] float v19; // [esp+E0h] [ebp-14h]
