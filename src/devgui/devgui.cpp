...
// CL_InitDevGui is opt-in behind KISAK_DEV, but the engine registers devgui paths from
// all over (cgame media, dvar registration, graphs) with no idea whether the menu tree
// exists. Without the free list those registrations walk a null nextFreeMenu and abort
// with "Too many devgui entries", so treat an uninitialised devgui as having no menus
// and let every add fall away.
static bool devguiInitialized;
...
{
    // Handles are 1-based indices into devguiGlob.menus. The decompiler wrote the
    // stride as a literal 40 - sizeof(DevMenuItem) on x86 - and DevMenuItem holds
    // pointers, so at LP64 every handle past the first addressed the wrong entry and
    // DevGui_FreeMenu_r asserted on the null it eventually found.
    if (!handle || handle > ARRAY_COUNT(devguiGlob.menus))
        return nullptr;
    return (devguiGlob_t *)((char *)devguiGlob.menus + sizeof(DevMenuItem) * (handle - 1));
}
...
void __cdecl DevGui_FreeMenu_r(uint16_t handle)
{
    ...
    if (handle)
    {
        ...
        *(uintptr_t *)menu->menus[0].label = (uintptr_t)devguiGlob.nextFreeMenu;
...
void __cdecl DevGui_Init()
{
    ...
    for (menuIndex = 0; menuIndex < 0x257; ++menuIndex)
        *(uintptr_t *)devguiGlob.menus[menuIndex].label = (uintptr_t)&devguiGlob.menus[menuIndex + 1];
    *(uintptr_t *)devguiGlob.menus[menuIndex].label = 0;
    devguiGlob.nextFreeMenu = (DevMenuItem *)&devguiGlob;
    devguiInitialized = true;
