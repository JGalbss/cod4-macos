...
char *__cdecl GetSourceString(Operand operand)
{
    ...
    if (operand.dataType == VAL_STRING)
        return const_cast<char *>(operand.internals.string);
    ...
    if (source->dataType == VAL_STRING)
    {
        ...
        if (dvar)
        {
            if (dvar->type == 7)
                ...
                VariantString = (char *)(uintptr_t)Dvar_GetVariantString(source->internals.string);
...
int __cdecl GetKeyBindingLocalizedString(int localClientNum, const char *command, char *keys, bool single)
{
    ...
    if (bindCount)
    {
        if (bindCount == 1)
        {
            translation = (char *)(uintptr_t)SEH_StringEd_GetString(bindings[0]);
            if (translation)
...
void __cdecl SecondsToTimeDisplay(int localClientNum, Operand *source, Operand *result)
{
    ...
    v3 = SnapFloatToInt((float)GetSourceInt(source).intVal / 60.0f);    _snprintf(
        resultString_0,
        0x80u,
        "%id %ih %im",
        v3 / 1440,
