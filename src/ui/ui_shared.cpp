...
void __cdecl Item_RunScript(UiContext *dc, itemDef_s *item, char *s)
{
    ...
    if (item && s && *s)
    {
        ...
        while (String_Parse((const char **)&p, out, 1024))
        {
            ...
            {
                v3 = 0;
                for (i = 0; i < ARRAY_COUNT(commandList); ++i)
                {
...
char __cdecl Menu_Paint(UiContext *dc, menuDef_t *menu)
{
    ...
    // ui_showMenuOnly is a string dvar. On x86 `string` and `integer` alias at offset
    // 0 of the value union, so the decompiler emitted the "is it non-empty" test as a
    // byte read through `current.integer`. At LP64 that truncates the pointer to 32
    // bits and dereferences the remains.
...
void __cdecl UI_AddMenu(UiContext *dc, menuDef_t *menu)
{
    ...
    {
        std::fprintf(stderr, "[uimenu] '%s' incoming=%p db=%p menuCount=%d\n",
                     menu->window.name ? menu->window.name : "?", (void *)menu,
                     (void *)DB_FindXAssetHeader(ASSET_TYPE_MENU, menu->window.name).menu,
                     dc->menuCount);
        std::fflush(stderr);
