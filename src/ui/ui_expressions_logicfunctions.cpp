...
{
    // qsort hands over hudelem_s *const *. The decompile read each one as a 32-bit
    // int and reached the sort key by its x86 byte offset, so at LP64 it compared two
    // truncated pointers. hudelem_s holds no pointers, so only the indirection was
    // ever wrong - offset 128 is hudelem_s::sort in both layouts.
    const float delta = (*static_cast<hudelem_s *const *>(pe0))->sort
        - (*static_cast<hudelem_s *const *>(pe1))->sort;
    if (delta >= 0.0f)
        return delta > 0.0f;
    else
        return -1;
...
void __cdecl compare_doesStringEqualString(Operand *leftSide, Operand *rightSide, Operand *result)
{
    ...
    if (std::getenv("KISAK_EXPR_TRACE"))
    {
        std::fprintf(stderr, "[expr] streq left=%p right=%p\n",
                     (const void *)leftSide->internals.string, (const void *)rightSide->internals.string);
        std::fflush(stderr);
    }
