...
void __cdecl CheckVote()
{
    ...
    if (level.voteExecuteTime)
    {
        if (level.voteExecuteTime < level.time)
        {
            ...
            if (std::getenv("KISAK_GAMEPLAY_TRACE"))
                Com_Printf(8, "[gameplay] vote executing: %s\n", level.voteString);
            v0 = va("%s\n", level.voteString);
            Cbuf_AddText(0, v0);
        }
    }
    if (level.voteTime)
    {
        ...
        LABEL_9:
            ...
            if (std::getenv("KISAK_GAMEPLAY_TRACE"))
            {
                Com_Printf(8, "[gameplay] vote passed: yes=%d no=%d eligible=%d\n",
        ...
        LABEL_11:
            ...
            if (std::getenv("KISAK_GAMEPLAY_TRACE"))
            {
                Com_Printf(8, "[gameplay] vote failed: yes=%d no=%d eligible=%d\n",
                           level.voteYes, level.voteNo, level.numVotingClients);
            }
            goto LABEL_13;
        }
...
void __cdecl G_RunFrame(int32_t levelTime)
{
    ...
    // Authoritative, opt-in state transitions for native multiplayer lifecycle
    // tests. Logging only on change keeps long killcam/respawn runs readable.
    static const bool traceCombat = std::getenv("KISAK_COMBAT_TRACE") != nullptr;
    static int lastConnected[2] = {-1, -1};
    static int lastSession[2] = {-1, -1};
    static int lastPmType[2] = {-1, -1};
    static int lastHealth[2] = {-1, -1};
    static int lastScore[2] = {-1, -1};
    if (traceCombat)
    {
        for (int clientNum = 0; clientNum < std::min(level.maxclients, 2); ++clientNum)
        {
            const gclient_s &client = level.clients[clientNum];
            const int connected = client.sess.connected;
            const int session = client.sess.sessionState;
            const int pmType = client.ps.pm_type;
            const int health = g_entities[clientNum].health;
            const int score = client.sess.score;
            if (connected != lastConnected[clientNum]
                || session != lastSession[clientNum]
                || pmType != lastPmType[clientNum]
                || health != lastHealth[clientNum]
                || score != lastScore[clientNum])
            {
                Com_Printf(15,
                    "[combat-test] state client=%d connected=%d session=%d pm=%d "
                    "health=%d score=%d time=%d\n",
                    clientNum, connected, session, pmType, health, score, level.time);
                lastConnected[clientNum] = connected;
                lastSession[clientNum] = session;
                lastPmType[clientNum] = pmType;
                lastHealth[clientNum] = health;
                lastScore[clientNum] = score;
            }
        }
    }
    iassert(bgs == 0);
