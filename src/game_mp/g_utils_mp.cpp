...
int __cdecl G_ModelIndex(const char *name)
{
    ...
    haveIndex:
        ...
        if (i == MAX_MODELS)
        {
            Com_Printf(14, "[diag] overflow for '%s' nameString=%u empty=%u\n", name, nameString, scr_const._);
            for (int d = 1; d < 8; ++d)
                Com_Printf(14, "[diag]   cs[%d]=%u '%s'\n", d + 830, SV_GetConfigstringConst(d + 830),
                           SL_ConvertToString(SV_GetConfigstringConst(d + 830)));
...
int __cdecl G_EntLinkToInternal(gentity_s *ent, gentity_s *parent, unsigned int tagName)
{
    ...
    tagInfo = (tagInfo_s *)MT_Alloc(sizeof(tagInfo_s), 17);
