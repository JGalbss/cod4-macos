...
void __cdecl Cmd_CallVote_f(gentity_s *ent)
{
    ...
    LABEL_91:
        ...
        {
            Com_Printf(8,
                       "[gameplay] vote called: client=%td command='%s' yes=%d no=%d eligible=%d\n",
                       ent - g_entities, level.voteString, level.voteYes, level.voteNo,
...
void __cdecl Cmd_Vote_f(gentity_s *ent)
{
    ...
    if (msg[0] == 121 || msg[1] == 89 || msg[1] == 49)
    {
        if (g_oldVoting->current.enabled)
        {
            ...
            if (std::getenv("KISAK_GAMEPLAY_TRACE"))
            {
                Com_Printf(8, "[gameplay] vote cast: client=%td choice=yes yes=%d no=%d eligible=%d\n",
                           ent - g_entities, level.voteYes, level.voteNo,
                           level.numVotingClients);
            }
        }
        else
    ...
    else if (g_oldVoting->current.enabled)
    {
        ...
        if (std::getenv("KISAK_GAMEPLAY_TRACE"))
        {
            Com_Printf(8, "[gameplay] vote cast: client=%td choice=no yes=%d no=%d eligible=%d\n",
                       ent - g_entities, level.voteYes, level.voteNo,
                       level.numVotingClients);
        }
    }
