...
{
    const scr_anim_s anim = Scr_GetAnim(0, 0);
    const uint32_t name = Scr_GetConstString(1);
    const XAnim_s *anims = Scr_GetAnims(anim.tree);
    Scr_AddBool(XAnimNotetrackExists(anims, anim.index, name));
}
...
void GScr_GetNotetrackTimes()
{
    ...
    Scr_MakeArray();
    const XAnim_s *anims = Scr_GetAnims(anim.tree);
    XAnimAddNotetrackTimesToScriptArray(anims, anim.index, name);
}
