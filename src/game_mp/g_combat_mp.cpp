...
static bool CombatLifecycleTraceEnabled()
{
    static const bool enabled = std::getenv("KISAK_COMBAT_TRACE") != nullptr;
    return enabled;
}
    ...
    int32_t psTimeOffset)
{
    ...
    SV_CheckThread();
    if (CombatLifecycleTraceEnabled())
    {
        Com_Printf(15,
            "[combat-test] death victim=%d attacker=%d damage=%d mod=%d weapon=%d "
            "victimScore=%d attackerScore=%d time=%d\n",
            self ? self->s.number : -1,
            attacker ? attacker->s.number : -1,
            damage, meansOfDeath, iWeapon,
            self && self->client ? self->client->sess.score : 0,
            attacker && attacker->client ? attacker->client->sess.score : 0,
            level.time);
    }
    if (!self->client)
    ...
    int32_t timeOffset)
{
        ...
        && targ->client->ps.pm_type != PM_DEAD)
    {
        ...
        if (damage <= 0)
            damage = 1;
        const int healthBefore = targ->health;
        Scr_PlayerDamage(targ, inflictor, attacker, damage, dflags, mod, weapon, point, dir, hitLoc, timeOffset);
        if (CombatLifecycleTraceEnabled())
        {
            Com_Printf(15,
                "[combat-test] damage target=%d attacker=%d amount=%d hitloc=%d "
                "health=%d->%d targetPm=%d targetScore=%d attackerScore=%d time=%d\n",
                targ->s.number,
                attacker ? attacker->s.number : -1,
                damage, static_cast<int>(hitLoc), healthBefore, targ->health,
                targ->client->ps.pm_type, targ->client->sess.score,
                attacker && attacker->client ? attacker->client->sess.score : 0,
                level.time);
        }
    }
