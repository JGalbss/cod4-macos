...
int __cdecl G_GetFreePlayerCorpseIndex()
{
    ...
    bestDistSq = -1.0;
    bestIndex = 0;
    // The decompiled constant was offsetof(gentity_s, classname) in the
    // 32-bit layout. Pointers widen the entity on arm64, so using 368 reads a
    // different field and makes every death assert while allocating a corpse.
...
void __cdecl G_RunCorpseMove(gentity_s *ent)
{
    ...
    [[maybe_unused]] bool v6; // [esp+1Ch] [ebp-D8h]
...
{
    [[maybe_unused]] bool v4; // [esp+0h] [ebp-58h]
