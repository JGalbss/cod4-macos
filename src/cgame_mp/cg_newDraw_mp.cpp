...
char *__cdecl CG_GetUseString(int32_t localClientNum)
{
    ...
    static char concisePlantHint[260];
    const cg_s *cgameGlob;
    ...
    char *resolved = UI_ReplaceConversionString(
        SEH_LocalizeTextMessage(displayString, "Hint String", LOCMSG_SAFE), binding);

    // The stock S&D hint is unnecessarily wordy on screen.  Preserve the
    // localized key binding and its inline color markup, but show the concise
    // action requested by the native client UI: "Hold F to plant".
    const char *plantWord = resolved ? strstr(resolved, "plant") : nullptr;
    const char *explosivesWord = plantWord ? strstr(plantWord + strlen("plant"), "explosives") : nullptr;
    if (plantWord && explosivesWord)
    {
        // Color controls can appear between the visible words, so locate the
        // two words independently and retain the source through "plant".
        const size_t keepLength = static_cast<size_t>(plantWord - resolved) + strlen("plant");
        if (keepLength < sizeof(concisePlantHint))
        {
            memcpy(concisePlantHint, resolved, keepLength);
            concisePlantHint[keepLength] = '\0';
            return concisePlantHint;
        }
    }
