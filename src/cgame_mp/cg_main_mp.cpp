...
void __cdecl CG_RestartSmokeGrenades(int32_t localClientNum)
{
    ...
    if (cgs->smokeGrenadeFx)
    {
        ...
        {
            const entityState_s &entity = nextSnap->entities[i];
            if ((entity.lerp.eFlags & 0x10000) != 0
                && entity.time2 >= cgameGlob->time
                && entity.lerp.u.customExplode.startTime <= cgameGlob->time)
            {
                if (entity.eType != ET_GENERAL)
                    MyAssertHandler(
                        ...
                        "(es->eType == ET_GENERAL)",
                        entity.eType);
                eventIndex = (static_cast<uint8_t>(entity.eventSequence) - 1) & 3;
                const int32_t event = entity.events[eventIndex];
                if (event < 45 || event > 50)
                    MyAssertHandler(
                        ".\\cgame_mp\\cg_main_mp.cpp",
                        1586,
                        0,
...
void __cdecl CG_Init(int32_t localClientNum, int32_t serverMessageNum, int32_t serverCommandSequence, int32_t clientNum)
{
    ...
    {
        const char *expected = va("cod%d", KISAK_COD4X_PROTOCOL_VERSION);
        const size_t expectedLength = strlen(expected);
        if (strncmp(s, expected, expectedLength)
            || (s[expectedLength] != '\0' && s[expectedLength] != ' '))
        {
            Com_Error(ERR_DROP, "Client/Server game mismatch: %s/%s", expected, s);
        }
    }
    else
#endif
...
void __cdecl CG_Shutdown(int32_t localClientNum)
{
    ...
    for (entnum = 0; entnum < 1024; ++entnum)
    {
        ...
        if (cent->pose.physObjId)
        {
            ...
            {
                Phys_ObjDestroy(PHYS_WORLD_FX, Phys_ObjFromId(cent->pose.physObjId));
