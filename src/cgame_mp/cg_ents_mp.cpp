...
void __cdecl CG_Player_PreControllers(DObj_s *obj, centity_s *cent)
{
    ...
    {
        cent->pose.player.control = nullptr;
    }
}
...
void __cdecl CG_CreatePhysicsObject(int32_t localClientNum, centity_s *cent)
{
    ...
    PhysObjId physObjId; // [esp+28h] [ebp-38h]
    ...
    if (physPreset)
    {
        ...
        physObjId = Phys_ObjToId(Phys_ObjCreate(PHYS_WORLD_FX, position, quat, velocity, physPreset));
        if (physObjId)
        {
            DObjPhysicsSetCollisionFromXModel(obj, PHYS_WORLD_FX, Phys_ObjFromId(physObjId));
...
void __cdecl CG_UpdatePhysicsPose(centity_s *cent)
{
    ...
    Sys_EnterCriticalSection(CRITSECT_PHYSICS);
    Phys_ObjGetInterpolatedState(PHYS_WORLD_FX, Phys_ObjFromId(cent->pose.physObjId), cent->pose.origin, quat);
    Sys_LeaveCriticalSection(CRITSECT_PHYSICS);
