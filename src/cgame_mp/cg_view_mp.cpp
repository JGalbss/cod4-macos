...
// One local client occupies the complete render target.  The original binary
// supplied this static initializer; leaving the reconstructed table zeroed
// turns shellshock save/blend commands into 0x0 rectangles.
ClientViewParams clientViewParamsArray[1][1] = {{{0.0f, 0.0f, 1.0f, 1.0f}}};
...
void __cdecl CG_CalcViewValues(int32_t localClientNum)
{
    ...
    // Renderer regression captures can place the camera at a known map feature
    // without asking a live multiplayer server for noclip/cheats.  This is inert
    // in normal play and still drives the complete engine DPVS and lighting path.
    const char *const viewOverride = std::getenv("KISAK_VIEW_OVERRIDE");
    if (viewOverride && viewOverride[0])
    {
        float origin[3] = {};
        float pitch = 0.0f;
        float yaw = 0.0f;
        if (std::sscanf(viewOverride, "%f,%f,%f,%f,%f", &origin[0], &origin[1],
                        &origin[2], &pitch, &yaw) == 5)
        {
            std::memcpy(cgameGlob->refdef.vieworg, origin, sizeof(origin));
            cgameGlob->refdefViewAngles[0] = pitch;
            cgameGlob->refdefViewAngles[1] = yaw;
            cgameGlob->refdefViewAngles[2] = 0.0f;
            AnglesToAxis(cgameGlob->refdefViewAngles, cgameGlob->refdef.viewaxis);
            static bool reportedOverride = false;
            if (!reportedOverride)
            {
                reportedOverride = true;
                Com_Printf(8, "[view] diagnostic override: origin=(%.1f %.1f %.1f) "
                              "angles=(%.1f %.1f 0.0)\n",
                           origin[0], origin[1], origin[2], pitch, yaw);
            }
        }
    }
}
...
{
    // A deterministic, console-free hook for validating the native rigid-body FX
    // path.  It is deliberately inert in normal play; the macOS smoke test sets the
    // effect name after the first snapshot is live, when the ordinary `fxTest`
    // command is safe to run as well.
    static bool nativeFxTestSpawned = false;
    const char *const nativeFxTest = std::getenv("KISAK_FX_AUTOTEST");
    const cg_s *const cgameGlob = CG_GetLocalClientGlobals(localClientNum);
    if (!nativeFxTestSpawned && nativeFxTest && nativeFxTest[0] && cgameGlob->nextSnap)
    {
        Vec3Mad(cgameGlob->refdef.vieworg, 100.0f, cgameGlob->refdef.viewaxis[0],
                s_testEffect[localClientNum].pos);
        I_strncpyz(s_testEffect[localClientNum].name, nativeFxTest,
                   sizeof(s_testEffect[localClientNum].name));
        s_testEffect[localClientNum].respawnTime = 0;
        nativeFxTestSpawned = true;
        Com_Printf(21, "[fx] native autotest spawning '%s'\n", s_testEffect[localClientNum].name);
        CG_PlayTestFx(localClientNum);
    }

    if (s_testEffect[localClientNum].respawnTime >= 1)
    {
        if (cgameGlob->time > s_testEffect[localClientNum].respawnTime + s_testEffect[localClientNum].time)
            CG_PlayTestFx(localClientNum);
    }
}
