...
// The recovered x86 vehicle-wheel routine below stores pointers in float/uint32
// scratch lanes. Keep it for reference, but do not compile it on the native
// 64-bit target.
#if 0
...
void CG_VehPoseControllers(const cpose_t *pose, const DObj_s *obj, int32_t *partBits)
{
    ...
    float bodyAngles[3] = {
        pose->vehicle.pitch * (360.0f / 65536.0f),
        0.0f,
        pose->vehicle.roll * (360.0f / 65536.0f),
    };
    float turretAngles[3] = {
        0.0f,
        pose->vehicle.yaw * (360.0f / 65536.0f),
        0.0f,
    };
    float barrelAngles[3] = {
        pose->vehicle.barrelPitch * (360.0f / 65536.0f),
        0.0f,
        pose->eType == ET_HELICOPTER ? pose->turret.barrelPitch : 0.0f,
    };

    DObjSetLocalTag((DObj_s *)obj, partBits, pose->vehicle.tag_body,
                    vec3_origin, bodyAngles);
    DObjSetLocalTag((DObj_s *)obj, partBits, pose->vehicle.tag_turret,
                    vec3_origin, turretAngles);
    DObjSetLocalTag((DObj_s *)obj, partBits, pose->vehicle.tag_barrel,
                    vec3_origin, barrelAngles);
}
