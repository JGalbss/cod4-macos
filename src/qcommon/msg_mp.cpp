...
double __cdecl MSG_ReadOriginFloat(int bits, msg_t *msg, float oldValue)
{
    ...
    {
        const int encoded = MSG_ReadLong(msg);
        float value;
...
int __cdecl MSG_ReadLastChangedField(msg_t *msg, int totalFields)
{
    ...
    {
        v2 = va("lastChanged was %i, totalFields is %i", lastChanged, totalFields);
...
static void __cdecl MSG_ReadDeltaHudElems(msg_t *msg, int time, const hudelem_s *from, hudelem_s *to, int count)
{
    ...
    if (inuse < 0 || inuse > count)
    {
        Com_PrintWarning(14, "CoD4x: invalid HUD element count %d (maximum %d)\n", inuse, count);
        msg->overflowed = 1;
        return;
    }
    ...
    for (int i = 0; i < inuse; ++i)
    {
        ...
        if (lc >= numHudElemFields)
        {
            Com_PrintWarning(14,
                "CoD4x: invalid HUD last-changed field %u for element %d at bit=%d byte=%d\n",
                lc, i, msg->bit, msg->readcount);
            msg->overflowed = 1;
            return;
        }
