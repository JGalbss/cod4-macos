...
#ifdef KISAK_MP
    // Com_ChangePlayerProfile pairs every profile switch with a stat read; the
    // startup path has to do the same or stats stay unfetched until something
    // restarts the filesystem, which never happens when booting into a map
