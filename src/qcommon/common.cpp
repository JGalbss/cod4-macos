...
{
    // The config's own header says it "automatically executes in development builds".
    // Retail does not run it, and its `set developer 1` turns on the devgui, whose
    // menu tree still has 32-bit layout assumptions and asserts while being torn down.
    // Opt in with KISAK_DEV=1 when that is what you want.
...
extern void Posix_LoadDeferredZones();
#endif
...
#if defined(KISAK_OAT_ZONES)
        // prim[1] is free now, so the startup zones that allocate from it can load.
        Posix_LoadDeferredZones();
#endif
...
void Com_InitDvars()
{
    ...
    // Fresh native-Metal profiles should not inherit the original 85 FPS PC
    // default.  250 preserves CoD4's established high-FPS behavior while
    // remaining user-configurable (0 is still uncapped).
#ifdef KISAK_METAL
    constexpr int defaultMaxFps = 250;
#else
    constexpr int defaultMaxFps = 85;
#endif
    com_maxfps = Dvar_RegisterInt(
        "com_maxfps", defaultMaxFps, 0, 1000, DVAR_ARCHIVE, "Cap frames per second");
...
void __cdecl Com_Frame_Try_Block_Function()
{
    ...
    const bool traceFrame = std::getenv("KISAK_FRAME_TRACE") && kisakFrameTrace < 3;
...
int __cdecl Com_ModifyMsec(int msec)
{
    ...
    {
        msec = SnapFloatToInt(dev_timescale->current.value * (com_codeTimeScale * (com_timescale->current.value * (double)msec)));        useTimescale = 1;
    }
