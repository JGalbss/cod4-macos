...
function render(data){latest=data;$('health').textContent=`LINK UP // ${data.hostname} // ${data.server}`;$('health').className='live';$('current-map').textContent=data.map;$('current-mode').textContent=data.modeLabel;$('player-count').textContent=`${data.players.length} / ${data.maxPlayers}`;$('uptime').textContent=data.uptime;
  ...
  const target=$('player-slot'),selected=target.value;target.replaceChildren();const controllable=data.players.filter(player=>player.slot!==null);if(!controllable.length)option(target,'','NO ACTIVE CLIENTS');else controllable.forEach(player=>option(target,String(player.slot),`[${player.slot}] ${player.name}`));if([...target.options].some(node=>node.value===selected))target.value=selected;const disabled=!controllable.length;['level-player','unlock-cac','max-player','kick'].forEach(id=>$(id).disabled=disabled)}
...
$('announce').onclick=()=>action('/api/say',{message:$('message').value});$('level-player').onclick=()=>playerAction('level');$('unlock-cac').onclick=()=>playerAction('cac');$('max-player').onclick=()=>playerAction('max');$('kick').onclick=()=>action('/api/kick',{slot:$('player-slot').value});$('send').onclick=()=>action('/api/command',{command:$('command').value});$('command').onkeydown=event=>{if(event.key==='Enter')$('send').click()};$('message').onkeydown=event=>{if(event.key==='Enter')$('announce').click()};refresh().then(refreshSettings);setInterval(refresh,5000);
</script></body></html>"""
